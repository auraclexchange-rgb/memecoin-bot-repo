...
def token_card(info: dict, contract: str, ud: dict, sc: dict = None) -> str:
    ...
    try:
        ...
        if chain_raw in ("solana", "sol"):
            ext_line = (
                "─────────────────────────\n"
                "📊 [GeckoTerminal](" + gt_url + ")  [DexTools](" + dt_url + ")  [DexScreener](" + ds_url + ")\n"
                "🔎 [DexView](" + dv_url + ")  [Birdeye](" + be_url + ")  [PumpFun](" + pf_url + ")\n"
            )
        else:
            ext_line = (
                "─────────────────────────\n"
                "📊 [GeckoTerminal](" + gt_url + ")  [DexTools](" + dt_url + ")\n"
                "🔎 [DexScreener](" + ds_url + ")  [DexView](" + dv_url + ")  [Birdeye](" + be_url + ")\n"
            )
        # ── assemble ──────────────────────────────────────────────────────────
        card = (
            "🪙 *" + name + "* ($" + symbol + ")\n"
        ...
        for uid, ud in active_users:
            try:
                ...
                if ud.get("sniper_advisory") and ud.get("sniper_adv_notify", True):
                    # Send for SNIPE always; send WAIT/SKIP only if confidence >= 5
                    # Also always send if confidence is 0 (API key issue) so user sees the error
