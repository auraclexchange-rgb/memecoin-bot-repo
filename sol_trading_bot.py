...
APEX_PROFILES = {
    ...
    "hunter": {
        ...
        "min_mc":        150_000,    # Sajad S1-01: MC > $150K
        "min_liq":        80_000,    # Sajad S1-01: Liq > $80K
...
def _sajad_lower_high_check(h: dict, current_price: float) -> bool:
    ...
    Uses sr_history candle peaks (3 required peaks minimum).
    ...
    # Lower high: most recent peak < previous peak
    recent_peak   = peaks[-1]
...
def _sajad_staircase_break(h: dict, current_price: float) -> bool:
    ...
    Only fires when position is below break-even (cx < 1.0) to avoid
    exiting profitable positions on shallow pullbacks.
    """
    avg = h.get("avg_price", 0)
    if avg <= 0 or current_price <= 0:
        return False

    # Only check staircase break when position is losing or flat
    # (Sajad: if already profitable, use trail — staircase exit is for loss protection)
    ...
    # Staircase break: current price below most recent higher low
    prev_low   = lows[-2]
    recent_low = lows[-1]

    # Only a "higher low staircase" is worth protecting
    if recent_low <= prev_low:
        return False   # no staircase in the first place

    # Price broke below the most recent higher low = staircase broken
    if current_price < recent_low * 0.98:   # 2% buffer for noise
        return True
    return False
...
def _sajad_fib_zone_check(h: dict, current_price: float) -> bool:
    ...
    # Only relevant when position is profitable (we've had a pump)
    cx = current_price / avg
    if cx < 1.2:
        return False

    # Calculate 62% retrace from peak
    # Fib 0.618 retrace: price = peak - (peak - avg) * 0.618
    fib_level = peak - (peak - avg) * 0.618
    if fib_level <= 0:
        return False
...
async def btn(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # ── APEX VAULT MENU ───────────────────────────────────────────────────────
        ...
        await q.edit_message_text(
            ...
            "🎯 *Hunter*\n"
            "Built from 3 Sajad TA masterclass videos.\n\n"
            "*Entry filters:*\n"
