...
def apex_self_calibrate(ud: dict, uid: int) -> dict:
    ...
    if losing:
        ...
        # Only ever RAISE the threshold — never lower it via this path
    ...
    async with _get_user_lock(uid):
        for contract, h in list(ud.get("holdings", {}).items()):
            ...
            try:
                ...
                # The general run_checker skips APEX/AI-Sniper positions — we own this.
                # Fires when cx drops below stop_loss_pct BEFORE trail activates (cx<1.5)
                sl_pct = h.get("stop_loss_pct")
                if sl_pct and cx < APEX_TRAIL_ACTIVATE_X:
                        ...
                        if cv_sl >= 0.01:
                            ...
                            try:
                                await app.bot.send_message(
                                    ...
                                    text=(
                                        "🛑 *APEX — STOP LOSS HIT*\n\n"
                                        "*$" + h["symbol"] + "* dropped *" + str(round(drop_pct, 1)) + "%*\n"
