...
APEX_PROFILES = {
    ...
    # ══════════════════════════════════════════════════════════════════════════
    # HUNTER MODE — Sajad TA Strategy (Season 1)
    # ══════════════════════════════════════════════════════════════════════════
    # Built from 3 Sajad strategy videos:
    #   S1-01: Bounce Framework (logical exits, volume confirmation, Fib 62%)
    #   S1-02: Rug Pull Detection (bundle %, SOL balances, pro traders, fees)
    #   S1-03: Mental Game (capital preservation, green candle trap, coin segments)
    #
    # PHILOSOPHY: Small consistent wins. Capital preservation first.
    # The ratchet model stops you losing on winners. Wide trails let winners run.
    # Sajad filters block the rugs before they even enter the position.
    # ══════════════════════════════════════════════════════════════════════════
    "hunter": {
        # ── Entry protection — Phase 0 (before token proves itself) ──────────
        # Sajad S1-02: tight entry SL aligned with rug risk assessment
        "sl_low":            18.0,   # LOW rug risk:    -18% initial stop
        "sl_med":            14.0,   # MEDIUM rug risk: -14% initial stop
        "sl_high":           10.0,   # HIGH rug risk:   -10% initial stop

        # ── Ratchet thresholds (capital protection as position gains) ────────
        # Sajad S1-03: "Stay in the game." Never lose on a winner.
        "ratchet_1x":        1.25,   # at 1.25x: SL moves to -4% from entry
        "ratchet_2x":        1.50,   # at 1.50x: SL locks to entry price (break-even)
        "red_2x_start":      1.25,   # after 1.25x: require 2x RED cycles before exit

        # ── Trail (only activates once break-even is locked at 1.5x) ─────────
        # Sajad S1-01: wide trail lets winners run to full resistance level
        "trail_activate_x":  1.50,   # MUST match ratchet_2x
        "trail_1_6":         0.22,   # 1.5x-2x:  22% trail
        "trail_2_0":         0.28,   # 2x-5x:    28% trail (riding the staircase)
        "trail_5_0":         0.20,   # 5x-10x:   20% trail
        "trail_10":          0.14,   # 10x+:     14% trail

        # ── Sajad Entry Gate ─────────────────────────────────────────────────
        # S1-01 + S1-02 hard minimums to filter rugs and low-quality setups
        "min_confidence":     6,     # only 6+/10 confidence tokens
        "min_score":         58,     # only 58+/100 sniper score
        "min_mc":         30_000,    # was 150K — lowered to match real sniper range ($30K+)
        "min_liq":        15_000,    # was 80K — lowered to match real sniper range ($15K+)
        "max_top10_pct":      20,    # Sajad S1-02: Top10 < 20%
        "max_age_h":          40,    # Sajad S1-01: Age < 40h (bounce trades only)
        "max_bundle_pct":     20,    # Sajad S1-02: Bundle < 20%
        "daily_loss_limit":   0.20,  # 20% daily loss halt
        "heat_stop":          0.75,  # tighter capital heat cap than Default

        # ── Sajad Entry Block Flags ──────────────────────────────────────────
        # S1-02: block rug signals at entry. S1-03: block green candle chasing.
        "block_green_candle_pct": 25.0,   # S1-03: block if 5m candle > 25% AND near ATH
        "block_farmed_vol_ratio": 0.15,   # S1-02: block if vol_m5 < 15% of hourly avg
        "block_instant_pump_age": 3.0,    # S1-02: block if age < 3 min AND MC > $50K
        "block_instant_pump_mc":  50_000, # S1-02: MC threshold for instant pump block

        # ── Sajad Exit Signals ───────────────────────────────────────────────
        # S1-01: lower high rule — exit if bounce high < previous high
        "lower_high_exit":    True,
        # S1-01: staircase break — exit if price closes below previous higher low
        "staircase_exit":     True,
        # S1-01: Fibonacci logical target — trail tightens near 62% retrace of ATH
        "fib_trail_tighten":  True,
        "fib_tighten_pct":    0.05,   # trail at 5% near fib zone
...
# False = Hunter is LIVE. Set True via /admin hunter off to emergency suspend.
APEX_HUNTER_SUSPENDED = False
...
# ══════════════════════════════════════════════════════════════════════════════
# SAJAD STRATEGY SIGNAL FUNCTIONS (Season 1)
# Used exclusively by Hunter mode for entry filtering and exit detection.
# All functions are pure (no side effects) — they return bool or float.
# ══════════════════════════════════════════════════════════════════════════════

def _sajad_entry_checks(info: dict, p: dict) -> tuple:
    """
    Run all Sajad S1 entry filters against live token data.
    Returns (passed: bool, reason: str).
    All checks must pass for Hunter to enter.

    Checks (from strategy playbook):
      S1-01: MC > min_mc, Liq > min_liq, Top10 < max_top10_pct, Age < max_age_h
      S1-02: Bundle < max_bundle_pct
      S1-02: Block instant launch pump (age < 3min AND MC > $50K)
      S1-02: Block farmed/dead coin (vol_m5 < 15% of hourly avg)
      S1-03: Block green candle chasing (5m candle > 25% AND near ATH)
    """
    mc       = info.get("mc", 0)
    liq      = info.get("liq", 0)
    ...
    ath_mc   = info.get("ath_mc", mc)
    avg_5m   = vol_h1 / 12.0 if vol_h1 > 0 else 0
    rug_risks = info.get("rug_risks", []) or []

    # S1-01: Market cap minimum
    min_mc = p.get("min_mc", 30_000)
    if mc < min_mc:
        return False, f"Sajad MC gate: ${mc:,.0f} < ${min_mc:,.0f} min"

    # S1-01: Liquidity minimum
    min_liq = p.get("min_liq", 15_000)
    if liq < min_liq:
        return False, f"Sajad Liq gate: ${liq:,.0f} < ${min_liq:,.0f} min"

    # S1-01: Top10 holder concentration
    max_top10 = p.get("max_top10_pct", 20)
    if top10 is not None and top10 > max_top10:
        return False, f"Sajad Top10 gate: {top10:.1f}% > {max_top10}% max"

    # S1-01: Token age (bounce trades only — fresh tokens)
    max_age = p.get("max_age_h", 40)
    if age_h > max_age:
        return False, f"Sajad Age gate: {round(age_h,1)}h > {max_age}h max"

    # S1-02: Bundle detection — use rug_risks flag from RugCheck AND top10 threshold
    # RugCheck explicitly flags bundle sniping in risks array.
    # Top10 > max_bundle_pct is the numerical proxy when RugCheck data isn't available.
    max_bundle = p.get("max_bundle_pct", 20)
    bundle_flagged = any("bundle" in r.lower() for r in rug_risks if isinstance(r, str))
    if bundle_flagged:
        return False, "Sajad bundle gate: RugCheck flagged bundle sniping"
    if top10 is not None and top10 > max_bundle:
        return False, f"Sajad bundle gate: top10 {top10:.1f}% > {max_bundle}% bundle proxy"

    # S1-02: Instant launch pump block (dev sniped own coin)
    block_age  = p.get("block_instant_pump_age", 3.0)
    block_mc   = p.get("block_instant_pump_mc", 50_000)
    age_min    = age_h * 60  # convert hours to minutes
    if age_min < block_age and mc > block_mc:
        return False, f"Sajad instant pump block: {round(age_min,1)}min old, MC ${mc:,.0f}"

    # S1-02: Farmed / dead coin block (flat price, zero volume)
    vol_ratio_threshold = p.get("block_farmed_vol_ratio", 0.15)
    if avg_5m > 0 and vol_m5 < avg_5m * vol_ratio_threshold:
        return False, f"Sajad farmed coin block: vol_m5 ${vol_m5:,.0f} < {int(vol_ratio_threshold*100)}% of avg ${avg_5m:,.0f}"

    # S1-02: SOL balance uniformity — burner wallets (populated by check_holder_sol_balances)
    if not info.get("sol_balances_ok", True):
        _sol_flags = info.get("sol_balance_flags", [])
        reason_txt = _sol_flags[0] if _sol_flags else "Holder SOL balances indicate burner wallets"
        return False, f"Sajad SOL balance gate: {reason_txt}"

    # S1-02: Global fees — near-zero = only dev trading (populated by check_global_fees)
    if not info.get("global_fees_ok", True):
        return False, f"Sajad global fees gate: {info.get('global_fees_sol', 0):.5f} SOL total fees — no real activity"

    # S1-02: Pro traders — zero experienced wallets = smart money avoiding coin
    # Soft block only: don't hard-block, but log clearly (pro_trader_ok can be False)
    # Hard block would fire too often on new tokens where data isn't available yet
    if info.get("pro_trader_count", -1) == 0 and info.get("pro_trader_ok") is False:
        # Only hard-block if we actually checked holders (count > 0 checked)
        if info.get("pro_trader_pct", -1) == 0.0 and info.get("sol_burner_count", 0) > 0:
            return False, "Sajad pro trader gate: zero pro traders + burner wallets detected"

    # S1-03: Green candle trap block (chasing the pump)
    green_candle_pct = p.get("block_green_candle_pct", 25.0)
    if ch_m5 > green_candle_pct:
        # Only block if also near ATH (top-blasting)
        near_ath = ath_mc > 0 and mc >= ath_mc * 0.92
        if near_ath:
            return False, f"Sajad green candle trap: +{round(ch_m5,1)}% 5m candle near ATH"

    # S1-02: Slow rug / robotic staircase detector
    # PDF: "Perfectly flat price over extended period with tiny volume = dev bleeding
    # liquidity slowly. Chart looks stable but dev is exiting. These collapse suddenly."
    # Signal: price barely moved over 1h AND 6h AND volume is very thin.
    # Distinct from farmed coin check (which catches zero-volume dead coins):
    # slow rugs show tiny upward creep with equally tiny volume — controlled pump.
    ch_h1_v = info.get("ch_h1", 0)
    ch_h6_v = info.get("ch_h6", 0)
    _price_flat_h1 = abs(ch_h1_v) < 3.0    # < 3% move over 1 hour
    _price_flat_h6 = abs(ch_h6_v) < 8.0    # < 8% move over 6 hours
    _vol_thin       = avg_5m > 0 and vol_m5 < avg_5m * 0.25  # vol < 25% of avg
    if _price_flat_h1 and _price_flat_h6 and _vol_thin and age_h > 1.0:
        return False, (
            f"Sajad slow rug block: price flat (1h:{round(ch_h1_v,1)}% 6h:{round(ch_h6_v,1)}%) "
            f"+ thin volume (m5 ${vol_m5:,.0f} < 25% avg ${avg_5m:,.0f})"
        )

    return True, "Sajad filters passed"


def _sajad_lower_high_check(h: dict, current_price: float) -> bool:
    """
    Sajad S1-01: Lower High Rule.
    Detect if the most recent bounce high is LOWER than the previous bounce high.
    Uses sr_history first, falls back to price_history on fast positions.
    Requires at least 4 candles (not 6) so it fires on shorter-held positions too.
    Returns True if a lower high is confirmed (exit signal).
    """
    # Use sr_history preferably, fall back to price_history
    history = h.get("sr_history") or h.get("price_history") or []
    if len(history) < 4:
        return False

    prices = [c.get("price", 0) for c in history if c.get("price", 0) > 0]
    if len(prices) < 4:
        return False

    # Find local peaks (price higher than both neighbours)
    peaks = []
    ...
    # Only signal if the drop is meaningful (> 3%) — avoids noise on flat charts
    if recent_peak < previous_peak * 0.97:
        return True
    return False


def _sajad_staircase_break(h: dict, current_price: float) -> bool:
    """
    Sajad S1-01: Staircase Break.
    A healthy uptrend has higher highs AND higher lows.
    If price closes below the previous HIGHER LOW, the staircase is broken.
    Uses sr_history first, falls back to price_history on fast positions.
    Requires at least 5 candles so it fires on shorter-held positions.
    PDF rule: no cx guard — staircase broken = EXIT NOW regardless of profit level.
    Returns True if staircase is broken (exit signal).
    """
    avg = h.get("avg_price", 0)
    ...
    # Find local lows (price lower than both neighbours)
    lows = []
    for i in range(1, len(prices) - 1):
        if prices[i] < prices[i-1] and prices[i] < prices[i+1]:
    ...
    # Only a "higher low staircase" is worth protecting
    if recent_low <= prev_low:
        return False

    # Price broke below the most recent higher low = staircase broken
    if current_price < recent_low * 0.98:
        return True
    return False


def _sajad_fib_zone_check(h: dict, current_price: float) -> bool:
    """
    Sajad S1-01: Fibonacci 62% Golden Zone — trail tighten signal.
    If price has pumped and is now sitting AT the 62% retracement of its ATH,
    it's a logical resistance/exit zone. Hunter tightens trail here.
    Returns True if price is in the Fib zone (trail should tighten).

    Guard: peak must be meaningfully higher than avg (>20% move) otherwise
    the Fib level collapses near entry and produces false signals on flat charts.
    """
    peak = h.get("apex_peak_price", 0)
    avg  = h.get("avg_price", 0)
    if peak <= 0 or avg <= 0 or current_price <= 0:
        return False

    # Position must be profitable at current price
    cx = current_price / avg
    if cx < 1.2:
        return False

    # Peak must be at least 20% above entry — no meaningful Fib level exists otherwise
    peak_x = peak / avg
    if peak_x < 1.2:
        return False

    # The swing range must be large enough to be meaningful (avoids noise on thin moves)
    swing = peak - avg
    if swing / avg < 0.15:   # less than 15% total swing — Fib level is too close to entry
        return False

    # Calculate 62% retrace from peak back toward entry
    # Fib 0.618 retrace: price = peak - swing * 0.618
    fib_level = peak - swing * 0.618
    if fib_level <= avg:     # Fib level below or at entry = degenerate case, skip
        return False

    # Are we currently AT the fib level (within 5%)?
    dist = abs(current_price - fib_level) / fib_level
    return dist <= 0.05


def _sajad_rsi(prices: list, period: int = 14) -> list:
    """
    Calculate RSI from a list of prices.
    Returns a list of RSI values (same length as prices, NaN-padded at start).
...
def _sajad_rsi_divergence(h: dict, current_price: float) -> bool:
    """
    Sajad S1-01: RSI Divergence exit signal.

    Bearish divergence: price makes a HIGHER HIGH but RSI makes a LOWER HIGH.
    This means momentum is fading even as price climbs — the move is running on fumes.

    Conditions:
    - Position must be profitable (cx >= 1.15) — only exit winners on divergence
    - Need at least 20 candles in sr_history for reliable RSI
    - Price recent high > previous high (higher high confirmed)
    - RSI at recent high < RSI at previous high (lower RSI high = divergence)
    - The RSI drop must be meaningful (>= 5 points) to avoid noise

    Returns True if bearish RSI divergence detected (exit signal).
    """
    avg = h.get("avg_price", 0)
    if avg <= 0 or current_price <= 0:
        return False

    # Only fire on profitable positions — divergence on a losing trade
    # is noise, not signal. Let the SL handle losers.
    cx = current_price / avg
    if cx < 1.15:
        return False
    ...
    rsi_vals = _sajad_rsi(prices, period=14)
    if len(rsi_vals) < 6:
        return False

    # Align: rsi_vals[i] corresponds to prices[i + 14]
    # We need the last 10 RSI values and their corresponding prices
    offset     = len(prices) - len(rsi_vals)
    rsi_prices = prices[offset:]   # prices aligned with rsi_vals

    # Find the two most recent local highs in price
    price_peaks = []
    for i in range(1, len(rsi_prices) - 1):
        if rsi_prices[i] > rsi_prices[i-1] and rsi_prices[i] > rsi_prices[i+1]:
            price_peaks.append((i, rsi_prices[i], rsi_vals[i]))

    if len(price_peaks) < 2:
        return False

    prev_peak  = price_peaks[-2]   # (idx, price, rsi)
    recent_peak = price_peaks[-1]

    # Higher high in price
    if recent_peak[1] <= prev_peak[1]:
        return False

    # Lower high in RSI (divergence) — must be meaningful drop
    rsi_drop = prev_peak[2] - recent_peak[2]
    if rsi_drop < 5.0:
        return False
...
def _sajad_macd(prices: list, fast: int = 12, slow: int = 26, signal: int = 9) -> dict:
    """
    ...
    return {
        "macd_line":   macd_line[sig_offset:],
        "signal_line": sig_line,
        "histogram":   histogram,
    }


def _sajad_macd_bearish_cross(h: dict, current_price: float) -> bool:
    """
    Sajad S1-01: MACD bearish crossover exit signal.

    Fires when:
    - MACD line just crossed BELOW the signal line (histogram flipped negative)
    - Position is profitable (cx >= 1.10)
    - At least 40 candles available (26 slow + 9 signal + buffer)

    Returns True if bearish cross detected (exit signal).
    """
    avg = h.get("avg_price", 0)
    if avg <= 0 or current_price <= 0:
        return False
    ...
    result = _sajad_macd(prices)
    if not result or len(result["histogram"]) < 2:
        return False

    hist = result["histogram"]
    # Bearish cross: histogram was positive last candle, now negative
    return hist[-2] >= 0 and hist[-1] < 0


def _sajad_volume_confirmation(h: dict, current_price: float) -> bool:
    """
    Sajad S1-01: Volume Confirmation exit signal.

    Sajad's rule: price cannot break its previous ATH without HIGHER volume
    than the candle that set that ATH. If volume is flat or declining on a
    new price high, the breakout is fake — exit.

    IMPORTANT: apex_sr_record_candle already appended the current candle and
    apex_peak_price is already updated BEFORE this is called. So we must:
      - Use history[:-1] (prior candles) to find the previous peak price+vol
      - Use history[-1] (current candle) for current price+vol
      - Confirm current price > max(prior prices) to detect a genuine new high

    Returns True if fake breakout detected (exit signal).
    """
    avg = h.get("avg_price", 0)
    if avg <= 0 or current_price <= 0:
        return False
    ...
    history = h.get("sr_history") or []
    # Need >=11 so prior_candles has at least 10 entries
    if len(history) < 11:
        return False

    current_candle = history[-1]
    prior_candles  = history[:-1]

    current_vol = current_candle.get("vol", 0)
    if current_vol <= 0:
        return False

    # Find the highest price and its volume among prior candles only
    prior_peak_price = 0.0
    prior_peak_vol   = 0.0
    for candle in prior_candles:
        p = candle.get("price", 0)
        v = candle.get("vol", 0)
        if p > prior_peak_price:
            prior_peak_price = p
            prior_peak_vol   = v

    if prior_peak_vol <= 0 or prior_peak_price <= 0:
        return False

    # Only fire if current price is a genuine new high vs prior history
    # (0.5% noise band avoids micro-fluctuation false positives)
    if current_price <= prior_peak_price * 1.005:
        return False

    # Fake breakout: new price high but volume < 70% of what drove the old peak
    return current_vol < prior_peak_vol * 0.70
...
_apex_entry_queue:  dict = {}  # uid -> {contract -> {info, sc, ai, queued_at, base_amount}}
_apex_paused_until: dict = {}  # uid -> datetime — APEX paused after 3 consecutive losses
_competitions:      dict = {}  # code -> comp dict — persisted via creator ud["_comp_data"]
...
# ── Suppress httpx request logs — they print full URLs including the bot token ─
...
# ── SOL Balance Uniformity Check (Sajad S1-02) ───────────────────────────────
_sol_balance_cache: dict = {}
SOL_BALANCE_CACHE_TTL = 300.0

async def check_holder_sol_balances(top_holders: list, api_key: str) -> dict:
    """
    Sajad S1-02: Check if top holders are burner wallets.
    Real coins: mixed SOL balances (some 1 SOL, some 50 SOL).
    Burner wallets: all near-zero OR all identical.
    Returns: sol_balances_ok, sol_balance_flags, sol_burner_count, sol_uniformity_score
    """
    result = {"sol_balances_ok": True, "sol_balance_flags": [],
              "sol_avg_balance": 0.0, "sol_burner_count": 0, "sol_uniformity_score": 1.0}
    ...
    try:
        ...
        for holder in top_holders[:8]:
            ...
            if not addr or len(addr) < 30:
                continue
            cache_key = addr[:20]
            cached = _sol_balance_cache.get(cache_key)
            if cached and (_sbt.time() - cached["ts"]) < SOL_BALANCE_CACHE_TTL:
                bal = cached["balance"]
            else:
                try:
                    ...
                    if r.status_code != 200:
                        continue
                    bal = float(r.json().get("result", {}).get("value", 0) or 0) / 1e9
                    _sol_balance_cache[cache_key] = {"balance": bal, "ts": _sbt.time()}
                    if len(_sol_balance_cache) > 1000:
                        oldest = sorted(_sol_balance_cache.items(), key=lambda x: x[1]["ts"])[:100]
                        for k, _ in oldest: _sol_balance_cache.pop(k, None)
                except Exception:
                    continue
            balances.append(bal)
            checked += 1
        ...
        if checked < 3:
            return result
        result["sol_burner_count"] = burners
        result["sol_avg_balance"]  = round(sum(balances) / len(balances), 4)
        mean = result["sol_avg_balance"]
        if mean > 0:
            variance = sum((b - mean)**2 for b in balances) / len(balances)
            result["sol_uniformity_score"] = round(min(1.0, (variance**0.5) / mean), 3)
        else:
            result["sol_uniformity_score"] = 0.0
        burner_pct = burners / checked
        if burner_pct >= 0.7:
            result["sol_balances_ok"] = False
            result["sol_balance_flags"].append(f"🚨 {burners}/{checked} holders have <0.05 SOL — burner wallets")
        elif burner_pct >= 0.5:
            result["sol_balance_flags"].append(f"⚠️ {burners}/{checked} holders near-zero SOL")
        if result["sol_uniformity_score"] < 0.15 and mean < 2.0:
            result["sol_balances_ok"] = False
            result["sol_balance_flags"].append(f"🚨 Holder SOL balances nearly identical ({mean:.3f} avg SOL) — coordinated wallets")
        elif result["sol_uniformity_score"] < 0.30:
            result["sol_balance_flags"].append(f"⚠️ Low SOL balance diversity — wallets may be related")
        return result
    except Exception as e:
        logger.debug(f"SOL balance check failed: {e}")
        return result


# ── Global Fees Monitor (Sajad S1-02) ────────────────────────────────────────
_global_fees_cache: dict = {}
GLOBAL_FEES_CACHE_TTL = 120.0

async def check_global_fees(contract: str, api_key: str) -> dict:
    """
    Sajad S1-02: Global fees = real wallet competition for block space.
    Near-zero fees on a coin near migration = only the dev is trading it.
    Returns: global_fees_sol, global_fees_ok, global_fees_flags, tx_count_recent
    """
    result = {"global_fees_sol": 0.0, "global_fees_ok": True,
              "global_fees_flags": [], "tx_count_recent": 0}
    if not api_key:
        return result
    import time as _gft
    cached = _global_fees_cache.get(contract)
    if cached and (_gft.time() - cached["ts"]) < GLOBAL_FEES_CACHE_TTL:
        return cached["data"]
    try:
        client  = await get_http()
        rpc_url = f"https://mainnet.helius-rpc.com/?api-key={api_key}"
        sig_r = await client.post(rpc_url, json={
            "jsonrpc": "2.0", "id": 1, "method": "getSignaturesForAddress",
            "params": [contract, {"limit": 50, "commitment": "confirmed"}]
        }, timeout=6)
        if sig_r.status_code != 200:
            return result
        sigs = sig_r.json().get("result", []) or []
        if not sigs:
            return result
        result["tx_count_recent"] = len(sigs)
        # Use Helius enhanced transactions for fee data
        sig_list = [s["signature"] for s in sigs[:20]]
        txn_r = await client.post(
            f"https://api.helius.xyz/v1/transactions?api-key={api_key}",
        ...
        if txn_r.status_code == 200:
            ...
            for tx in (txns if isinstance(txns, list) else []):
                total_fees_lamports += int(tx.get("prioritizationFee", 0) or 0)
                total_fees_lamports += int(tx.get("fee", 0) or 0)
        else:
            # Fallback: base fee estimate (5000 lamports per tx)
            total_fees_lamports = len(sigs) * 5000
        total_fees_sol = round(total_fees_lamports / 1e9, 6)
        result["global_fees_sol"] = total_fees_sol
        if total_fees_sol < 0.005:
            result["global_fees_ok"] = False
            result["global_fees_flags"].append(
                f"🚨 Near-zero global fees ({total_fees_sol:.5f} SOL) — only dev trading this coin")
        elif total_fees_sol < 0.02:
            result["global_fees_flags"].append(
                f"⚠️ Low global fees ({total_fees_sol:.4f} SOL) — limited real activity")
        _global_fees_cache[contract] = {"data": result, "ts": _gft.time()}
        if len(_global_fees_cache) > 500:
            oldest = sorted(_global_fees_cache.items(), key=lambda x: x[1]["ts"])[:50]
            for k, _ in oldest: _global_fees_cache.pop(k, None)
        return result
    except Exception as e:
        logger.debug(f"Global fees check failed: {e}")
        return result


# ── Pro Trader Count (Sajad S1-02) ───────────────────────────────────────────
_pro_trader_cache: dict = {}
PRO_TRADER_CACHE_TTL = 180.0

async def check_pro_traders(contract: str, top_holders: list, api_key: str) -> dict:
    """
    Sajad S1-02: Pro traders = wallets with real capital and experience.
    If zero pro wallets are in a coin, smart money is staying away.
    Pro criteria:
      - SOL balance >= 2.0 SOL (has real capital), OR
      - >= 20 recent transactions (experienced trader)
    Returns: pro_trader_count, pro_trader_pct, pro_trader_ok, pro_trader_flags
    """
    result = {"pro_trader_count": 0, "pro_trader_pct": 0.0,
              "pro_trader_ok": True, "pro_trader_flags": []}
    if not api_key or not top_holders:
        return result
    import time as _ptt
    cached = _pro_trader_cache.get(contract)
    if cached and (_ptt.time() - cached["ts"]) < PRO_TRADER_CACHE_TTL:
        return cached["data"]
    try:
        client  = await get_http()
        rpc_url = f"https://mainnet.helius-rpc.com/?api-key={api_key}"
        ...
        for holder in top_holders[:8]:
            ...
            try:
                # Check SOL balance
                bal_r = await client.post(rpc_url, json={
                    "jsonrpc": "2.0", "id": 1, "method": "getBalance",
                    "params": [addr, {"commitment": "confirmed"}]
                }, timeout=4)
                if bal_r.status_code == 200:
                    bal_sol = float(bal_r.json().get("result", {}).get("value", 0) or 0) / 1e9
                    if bal_sol >= 2.0:
                        is_pro = True
                if not is_pro:
                    # Check tx history depth
                    sig_r = await client.post(rpc_url, json={
                        "jsonrpc": "2.0", "id": 2, "method": "getSignaturesForAddress",
                        "params": [addr, {"limit": 25, "commitment": "confirmed"}]
                    }, timeout=4)
                    if sig_r.status_code == 200:
                        if len(sig_r.json().get("result", []) or []) >= 20:
                            is_pro = True
            except Exception:
                pass
            if is_pro:
                pro_count += 1
            await _asyncio.sleep(0.05)
        result["pro_trader_count"] = pro_count
        result["pro_trader_pct"]   = round(pro_count / checked * 100, 1) if checked > 0 else 0.0
        if checked > 0:
            if pro_count == 0:
                result["pro_trader_ok"] = False
                result["pro_trader_flags"].append(
                    f"⚠️ Zero pro traders in top {checked} holders — smart money avoiding this coin")
            elif pro_count >= 3:
                result["pro_trader_flags"].append(
                    f"✅ {pro_count} pro traders in top holders — experienced wallets present")
        _pro_trader_cache[contract] = {"data": result, "ts": _ptt.time()}
        if len(_pro_trader_cache) > 500:
            oldest = sorted(_pro_trader_cache.items(), key=lambda x: x[1]["ts"])[:50]
            for k, _ in oldest: _pro_trader_cache.pop(k, None)
        return result
    except Exception as e:
        logger.debug(f"Pro trader check failed: {e}")
        return result
...
def sniper_score(info: dict) -> dict:
    ...
    # Sajad S1-02 enrichment fields
    sol_balances_ok      = info.get("sol_balances_ok", True)
    sol_balance_flags    = info.get("sol_balance_flags", [])
    sol_burner_count     = info.get("sol_burner_count", 0)
    global_fees_ok       = info.get("global_fees_ok", True)
    global_fees_sol      = info.get("global_fees_sol", -1.0)
    pro_trader_count     = info.get("pro_trader_count", -1)
    pro_trader_ok        = info.get("pro_trader_ok", True)
    pro_trader_flags     = info.get("pro_trader_flags", [])
    ...
    # Mint authority: pump.fun tokens always have mint active by design.
    # Non-pf tokens with active mint = hard flag (rug risk).
    # Pump.fun tokens with active mint = score penalty only (expected but still risky).
    if no_mint is False:
        if not pf_curve:
            flags.append("🚨 Mint authority active — supply can inflate")
        else:
            # pf token: mint active is normal, but still penalize
            score = max(0, score - 8)
            warnings.append("Mint authority active (pump.fun — expected pre-graduation)")
    ...
    # Vol/MC wash trade flag: only meaningful on tokens > 30min old.
    # Fresh tokens routinely hit 5-10x vol/MC during a viral launch.
    # Pre-filter already blocks > 10x, so this fires on 5-10x tokens
    # that passed the pre-filter — a false positive on every pump.fun launch.
    if vol_mc_ratio > 15 and age_h > 0.5:
        flags.append(f"🚨 Vol/MC={vol_mc_ratio:.1f}x — likely wash trading")
    elif vol_mc_ratio > 8:
        warnings.append(f"High Vol/MC ({vol_mc_ratio:.1f}x) — check for wash trading")
    # maker_count gate: fresh tokens < 1h old can't have 30+ unique wallets yet.
    if maker_count > 0 and maker_count < 30:
        if age_h > 1.0:
            flags.append(f"🚨 Only {maker_count} unique wallets — too few real buyers")
        else:
            warnings.append(f"Only {maker_count} unique wallets so far (token too new to judge)")
    ...
    # Sajad S1-02: SOL balance uniformity (burner wallets)
    if not sol_balances_ok and sol_balance_flags:
        flags.append(sol_balance_flags[0])
    # Sajad S1-02: Global fees — near-zero = only dev trading
    # Only meaningful on tokens > 1h old. Fresh tokens always have near-zero fees
    # (not enough time to accumulate) — flagging them kills every pump.fun token.
    if global_fees_sol >= 0 and not global_fees_ok:
        if age_h > 1.0:
            flags.append(f"🚨 Near-zero global fees ({global_fees_sol:.5f} SOL) — only dev trading")
        else:
            warnings.append(f"Low global fees ({global_fees_sol:.5f} SOL) — token too new to judge")
    # Sajad S1-02: Pro traders — zero experienced wallets
    if pro_trader_count == 0 and not pro_trader_ok:
        warnings.append("Zero pro traders detected — smart money avoiding this coin")
    ...
    if is_solana:
        if lp_burn is not None:
            ...
            elif lp_burn == 0 and not pf_curve:
                score = max(0, score - 5); warnings.append("LP not burned — rug risk")
            else:               warnings.append(f"LP barely burned ({lp_burn}%)")
        # No lp_burn data: give no bonus, no penalty
        # (pump.fun tokens have no LP to burn until graduation)
    else:
        score += 6
...
def get_user(uid: int, uname: str) -> dict:
    if uid not in users:
        users[uid] = {
            ...
            "sniper_filters": {
                "min_score":        50,      # Raised from 35 — signals must have meaningful quality
...
def sell_core(ud: dict, uid: int, contract: str, usd: float, price: float, reason: str = "manual") -> dict:
    ...
    if h["amount"] < 0.000001:
        # total_ever_invested tracks the true cumulative buy-in across all DCA adds.
        # total_invested is decremented on each partial sell so is unreliable here.
        # Fallback: h["total_invested"](post-deduct=0) + cost(last chunk) gives only
        # the last chunk's cost — use total_ever_invested when available.
        total_ever_invested = h.get("total_ever_invested", 0.0)
        total_invested_full = total_ever_invested if total_ever_invested > 0 else (h["total_invested"] + cost)
        ...
        trade_log.setdefault(uid, []).append({
            ...
            "realized_pnl":  round(total_returned_full - total_invested_full, 4),
            "x":             x_val,
        ...
        for hca, h in list(ud.get("holdings", {}).items()):
            try:
                ...
                if ud.get("milestone_notif", True):
                    ...
                    # Fire every whole-number x from 2x upward
                    _milestone_icons = {
                        2:"🚀", 3:"🚀", 4:"🚀", 5:"🎯", 6:"🎯", 7:"🎯",
                        8:"🎯", 9:"🎯", 10:"🔥", 15:"🔥", 20:"💎", 25:"💎",
                        30:"💎", 40:"💎", 50:"👑"
                    }
                    _cur_x_int = int(current_x)
                    for level in range(2, _cur_x_int + 1):
                        if level not in milestones_hit:
                            milestones_hit.append(level)
                            profit = cur_value - invested
                            icon = _milestone_icons.get(level, "👑" if level >= 50 else "💎" if level >= 20 else "🔥" if level >= 10 else "🎯" if level >= 5 else "🚀")
                            try:
                                await app.bot.send_message(
                                    chat_id=uid, parse_mode="Markdown",
                                    text=(
                                        icon + " *" + str(level) + "× MILESTONE HIT*\n\n"
                ...
                if isinstance(pf_data, list):
                    ...
                    for coin in pf_data:
                        ...
                        out[mint] = {
                            ...
                            "_pf_image":    coin.get("image_uri") or coin.get("image") or "",
                        }
                    return out
        except Exception as e:
...
async def ai_analyze_token(info: dict, sc: dict, ud: dict, uid: int = 0) -> dict:
    ...
    elif score >= 45 and flag_count <= 1 and momentum != "NEGATIVE":
        verdict    = "SNIPE"
        confidence = min(10, max(4, score // 10))
    elif score >= 45 and flag_count <= 1 and momentum == "NEGATIVE":
        # Good score but price is falling — downgrade to WAIT
        verdict    = "WAIT"
        confidence = min(5, max(3, score // 14))
    elif score >= 35:
        verdict    = "WAIT"
...
async def _broadcast_to_channel(bot, channel_id: int, info: dict, sc: dict, ai: dict, contract: str, uid: int = 0) -> bool:
    """Post channel card — image first (if available), then full text card."""
    try:
        ...
        # ── Send token image first (silent) if available ──────────────────
        ...
        if image_url:
            try:
                await bot.send_photo(
                    chat_id=channel_id,
                    photo=image_url,
                    caption=f"<b>${symbol}</b>  ·  {info.get('name', '')}",
                    parse_mode="HTML",
                    disable_notification=True,   # silent — main card will ping
                )
            except Exception as _img_err:
                logger.debug(f"Channel image send failed (non-fatal): {_img_err}")

        # ── Send full text card with buttons ──────────────────────────────
            ...
            try:
                ...
                for buy in new_buys[:3]:   # max 3 alerts per wallet per cycle
                    ...
                    try:
                        ...
                        # ── KOL ATH proximity filter (PDF S1-03) ──────────────
                        # Suppress KOL alert if token is within 5% of its ATH.
                        # PDF: "Wallet trackers are only useful if they alert you
                        # early in a move. If it fires at the top of a pump it is
                        # telling you where smart money just EXITED."
                        _ath_mc   = info.get("ath_mc", 0)
                        _cur_mc   = info.get("mc", 0)
                        _near_ath = _ath_mc > 0 and _cur_mc >= _ath_mc * 0.95
                        if _near_ath:
                            logger.info(
                                f"KOL alert suppressed (near ATH): ${info['symbol']} "
                                f"MC {mc_str(_cur_mc)} vs ATH {mc_str(_ath_mc)}"
                            )
                            continue
...
_MILESTONE_XS = list(range(2, 101))   # every x from 2x to 100x
            ...
            try:
                ...
                for milestone_x in _MILESTONE_XS:
                    ...
                    if current_x >= milestone_x:
                        ...
                        milestone_icons = {2:"🚀", 3:"🚀", 4:"🚀", 5:"🎯", 6:"🎯", 7:"🎯", 8:"🎯", 9:"🎯", 10:"🔥"}
                        rocket = milestone_icons.get(milestone_x, "👑" if milestone_x >= 50 else "💎" if milestone_x >= 20 else "🔥")
                        ms_text = (
...
async def sniper_job(ctx: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        try:
            _kw_counts: dict = {}
            _STOP_WORDS = {"token","coin","inu","sol","the","and","for","with","on"}
            ...
            if _active_narratives:
                ...
                for _nkw in _active_narratives:
                    # Track internally for score threshold bonus — no user DM
                    ...
                    if _nkw not in _recent_nkws:
                        ...
                        # Note: no user DM — narrative data is used internally for
                        # score threshold adjustment only (eff_min_score -= 5)
        ...
        for item in unique_items:
            ...
            try:
                ...
                info["pf_image"]     = item.get("_pf_image", "")
                ...
                # ── Sajad S1-02: global fees check — run ONCE per token, not per user ──
                # Token-level data identical for all users. Running inside the user loop
                # was multiplying Helius calls by N_users and causing the job to hang.
                # check_holder_sol_balances and check_pro_traders are removed: they need
                # full wallet addresses which Solscan truncates — they were no-ops anyway.
                if helius_key and info.get("chain","").lower() in ("solana","sol"):
                    try:
                        _gfees = await check_global_fees(contract, helius_key)
                        info.update(_gfees)
                    except Exception as _gfe:
                        logger.debug(f"Global fees error (per-token): {_gfe}")
            ...
            for uid, ud in active_users:
                try:
                    ...
                    if skip_reason:
                        ...
                        # ── Accumulate pre-filter SKIPs for batch summary ─────
                        # Sending one message per token causes Telegram flood limits
                        # (75+ msgs/cycle to same channel = all silently dropped).
                        # Instead collect into a list and send ONE summary at cycle end.
                        _dl_ch = ud.get("sniper_log_channel")
                        if _dl_ch and ud.get("sniper_log_channel_on", True):
                            ...
                            _dl_reason = skip_reason[:60] if skip_reason else "?"
                            ud.setdefault("_dl_skip_batch", []).append(
                                "🔴 $" + info.get("symbol","?") + " " + _dl_chain
                    ...
                    if helius_key and info.get("chain","").lower() in ("solana","sol"):
                        ...
                        # Re-score with all new data (global_fees already in info from per-token block)
                        sc = sniper_score(info)
                    ...
                    # ── Dump log channel: send compact line for PASS tokens ─
                    ...
                    if _dl_ch2 and ud.get("sniper_log_channel_on", True):
                        try:
                            ...
                            _v_icon = {"SNIPE":"\U0001f7e2","WAIT":"\U0001f7e1","SKIP":"\U0001f534"}.get(ai.get("verdict","?"),"\u26aa")
                            _dl_text2 = (
                            ...
                            await ctx.bot.send_message(
                                chat_id=int(_dl_ch2), text=_dl_text2,
                                disable_notification=(ai.get("verdict","?") != "SNIPE")
                            )
                        except Exception:
                            pass
                    ...
                    if ud.get("apex_mode") and _apex_verdict_ok:
                        ...
                        _min_score  = max(ud.get("apex_learn_score_min", 35),
                    ...
                    # sniper_adv_notify = False → Channel card only, no DM
                    # Advisory is NEVER blocked by market_regime — it is
                    # informational only, the user decides what to do.
                    # ════════════════════════════════════════════════════
        ...
        # ── Skip reason summary + dump log batch send ──────────────────────────
        for uid2, ud2 in active_users:
            ...
            # ── Send ONE batched pre-filter SKIP summary to dump log channel ──
            # Replaces 75 individual messages (flood) with one readable summary.
            _batch = ud2.pop("_dl_skip_batch", [])
            _dl_ch3 = ud2.get("sniper_log_channel")
            if _batch and _dl_ch3 and ud2.get("sniper_log_channel_on", True):
                try:
                    _total_skipped = len(_batch)
                    _preview = _batch[:8]  # show first 8
                    _more = _total_skipped - len(_preview)
                    _batch_lines = ["🚫 *Pre-filter SKIPs: " + str(_total_skipped) + "*"] + _preview
                    if _more > 0:
                        _batch_lines.append("_...and " + str(_more) + " more_")
                    _batch_text = "\n".join(_batch_lines)
    ...
    async with _get_user_lock(uid):
        for contract, h in list(ud.get("holdings", {}).items()):
            ...
            try:
                ...
                # ── Hunter: Sajad S1-01 exit signals ─────────────────────────────
                # Lower High Rule + Staircase Break + Fibonacci zone trail tighten.
                # Only active in Hunter profile and only after enough candle history.
                if _hp and len(h.get("sr_history", [])) >= 6:

                    # ── Fibonacci zone: tighten trail when near 62% retrace ────
                    if _hp.get("fib_trail_tighten") and _sajad_fib_zone_check(h, price):
                        _fib_tpct = _hp.get("fib_tighten_pct", 0.05)
                        if not h.get("_fib_tightened"):
                            h["_fib_tightened"] = True
                            logger.info(f"Hunter Fib zone: tightening trail to {_fib_tpct*100:.0f}% for {h.get('symbol','?')}")
                            try:
                                await app.bot.send_message(
                                    chat_id=uid, parse_mode="Markdown",
                                    text=(
                                        "📐 *HUNTER — FIBONACCI ZONE*\n\n"
                                        "*$" + _md(h["symbol"]) + "* at Fib 62% level\n"
                                        "Trail tightened to *" + str(int(_fib_tpct*100)) + "%*\n"
                                        "_Logical resistance zone — Sajad S1-01_"
                                    ),
                                    reply_markup=main_menu_kb()
                                )
                            except Exception:
                                pass
                    elif h.get("_fib_tightened") and not _sajad_fib_zone_check(h, price):
                        # Price moved away from fib zone — reset flag
                        h["_fib_tightened"] = False

                    # ── Lower High Rule: exit if bounce high < previous high ────
                    if _hp.get("lower_high_exit") and not h.get("_lower_high_fired"):
                        if _sajad_lower_high_check(h, price):
                            h["_lower_high_fired"] = True
                            cv_lh  = h["amount"] * price
                            if cv_lh >= 0.01:
                                result_lh = sell_core(ud, uid, contract, cv_lh, price, "hunter_lower_high")
                                ud["apex_daily_pnl"] = (ud.get("apex_daily_pnl") or 0.0) + (result_lh.get("realized") or 0.0)
                                if result_lh["realized"] < 0:
                                ...
                                apex_learn_record(uid, _apex_rich_record(h, cx, price, avg, result_lh["realized"], "hunter_lower_high", ud))
                                _apex_post_exit.setdefault(uid, {})[contract] = {
                                    "symbol": h["symbol"], "contract": contract,
                                    "exit_price": price, "entry_price": avg,
                                    "exit_reason": "hunter_lower_high",
                                    "exit_x": round(cx, 3),
                                    "peak_x": round(h.get("apex_peak_price", avg) / avg, 3) if avg > 0 else 0,
                                ...
                                try:
                                    await app.bot.send_message(
                                        chat_id=uid, parse_mode="Markdown",
                                        text=(
                                            "📉 *HUNTER — LOWER HIGH EXIT*\n\n"
                                            "*$" + _md(h["symbol"]) + "*\n"
                                            "Bounce high lower than previous high\n"
                                            "Exit: *" + str(round(cx, 2)) + "x*  |  PnL: *" + pstr(result_lh["realized"]) + "*\n"
                                            "_Sajad S1-01: Lower high = trend weakening_"
                                        ),
                                        reply_markup=main_menu_kb()
                                    )
                                except Exception:
                                    pass
                                continue   # position closed, skip rest of loop

                    # ── Staircase Break: exit if price breaks below higher low ──
                    if _hp.get("staircase_exit") and not h.get("_staircase_fired"):
                        if _sajad_staircase_break(h, price):
                            h["_staircase_fired"] = True
                            cv_sc  = h["amount"] * price
                            if cv_sc >= 0.01:
                                result_sc = sell_core(ud, uid, contract, cv_sc, price, "hunter_staircase_break")
                                ud["apex_daily_pnl"] = (ud.get("apex_daily_pnl") or 0.0) + (result_sc.get("realized") or 0.0)
                                if result_sc["realized"] < 0:
                                    ud["apex_consec_losses"] = ud.get("apex_consec_losses", 0) + 1
                                else:
                                    ud["apex_consec_losses"] = 0
                                    ud["apex_total_wins"] = ud.get("apex_total_wins", 0) + 1
                                ud["apex_total_trades"] = ud.get("apex_total_trades", 0) + 1
                                apex_learn_record(uid, _apex_rich_record(h, cx, price, avg, result_sc["realized"], "hunter_staircase_break", ud))
                                _apex_post_exit.setdefault(uid, {})[contract] = {
                                    "symbol": h["symbol"], "contract": contract,
                                    "exit_price": price, "entry_price": avg,
                                    "exit_reason": "hunter_staircase_break",
                                    "exit_x": round(cx, 3),
                                    "peak_x": round(h.get("apex_peak_price", avg) / avg, 3) if avg > 0 else 0,
                                    "held_h": round((_time.time() - h["bought_at"].timestamp()) / 3600, 2) if h.get("bought_at") else 0,
                                    "entry_mc": h.get("avg_cost_mc", 0),
                                ...
                                try:
                                    await app.bot.send_message(
                                        chat_id=uid, parse_mode="Markdown",
                                        text=(
                                            "🪜 *HUNTER — STAIRCASE BROKEN*\n\n"
                                            "*$" + _md(h["symbol"]) + "*\n"
                                            "Price broke below previous higher low\n"
                                            "Exit: *" + str(round(cx, 2)) + "x*  |  PnL: *" + pstr(result_sc["realized"]) + "*\n"
                                            "_Sajad S1-01: Staircase broken = exit immediately_"
                                        ),
                                        reply_markup=main_menu_kb()
                                    )
                                except Exception:
                    ...
                    # ── RSI Divergence: exit if price HH + RSI LH ────────────
                    # Sajad S1-01: momentum fading even as price climbs = exit.
                    # Only fires on profitable positions (cx >= 1.15).
                    if not h.get("_rsi_div_fired"):
                        if _sajad_rsi_divergence(h, price):
                            h["_rsi_div_fired"] = True
                            cv_rsi = h["amount"] * price
                            if cv_rsi >= 0.01:
                                result_rsi = sell_core(ud, uid, contract, cv_rsi, price, "hunter_rsi_divergence")
                                ud["apex_daily_pnl"] = (ud.get("apex_daily_pnl") or 0.0) + (result_rsi.get("realized") or 0.0)
                                if result_rsi["realized"] < 0:
                                    ud["apex_consec_losses"] = ud.get("apex_consec_losses", 0) + 1
                                else:
                                    ud["apex_consec_losses"] = 0
                                    ud["apex_total_wins"] = ud.get("apex_total_wins", 0) + 1
                                ud["apex_total_trades"] = ud.get("apex_total_trades", 0) + 1
                                apex_learn_record(uid, _apex_rich_record(h, cx, h.get("apex_peak_price", price), avg, result_rsi["realized"], "hunter_rsi_divergence", ud))
                                _apex_post_exit.setdefault(uid, {})[contract] = {
                                    "symbol": h["symbol"], "contract": contract,
                                    "exit_price": price, "entry_price": avg,
                                    "exit_reason": "hunter_rsi_divergence",
                                    "exit_x": round(cx, 3),
                                    "peak_x": round(h.get("apex_peak_price", avg) / avg, 3) if avg > 0 else 0,
                                    "held_h": round((_time.time() - h["bought_at"].timestamp()) / 3600, 2) if h.get("bought_at") else 0,
                                    "entry_mc": h.get("avg_cost_mc", 0),
                                ...
                                try:
                                    await app.bot.send_message(
                                        chat_id=uid, parse_mode="Markdown",
                                        text=(
                                            "📊 *HUNTER — RSI DIVERGENCE EXIT*\n\n"
                                            "*$" + _md(h["symbol"]) + "*\n"
                                            "Price: higher high  |  RSI: lower high\n"
                                            "Momentum fading — exiting before reversal\n"
                                            "Exit: *" + str(round(cx, 2)) + "x*  |  PnL: *" + pstr(result_rsi["realized"]) + "*\n"
                                            "_Sajad S1-01: Price HH + RSI LH = exit_"
                                        ),
                                        reply_markup=main_menu_kb()
                                    )
                                except Exception:
                    ...
                    # ── MACD Bearish Cross: exit when momentum turns ───────────
                    # Sajad S1-01: MACD crosses below signal = trend over, exit.
                    # Only fires on profitable positions (cx >= 1.10).
                    if not h.get("_macd_cross_fired"):
                        if _sajad_macd_bearish_cross(h, price):
                            h["_macd_cross_fired"] = True
                            cv_macd = h["amount"] * price
                            if cv_macd >= 0.01:
                                result_macd = sell_core(ud, uid, contract, cv_macd, price, "hunter_macd_bearish")
                                ud["apex_daily_pnl"] = (ud.get("apex_daily_pnl") or 0.0) + (result_macd.get("realized") or 0.0)
                                if result_macd["realized"] < 0:
                                    ud["apex_consec_losses"] = ud.get("apex_consec_losses", 0) + 1
                                else:
                                    ud["apex_consec_losses"] = 0
                                    ud["apex_total_wins"] = ud.get("apex_total_wins", 0) + 1
                                ud["apex_total_trades"] = ud.get("apex_total_trades", 0) + 1
                                apex_learn_record(uid, _apex_rich_record(h, cx, h.get("apex_peak_price", price), avg, result_macd["realized"], "hunter_macd_bearish", ud))
                                _apex_post_exit.setdefault(uid, {})[contract] = {
                                    "symbol": h["symbol"], "contract": contract,
                                    "exit_price": price, "entry_price": avg,
                                    "exit_reason": "hunter_macd_bearish",
                                    "exit_x": round(cx, 3),
                                    "peak_x": round(h.get("apex_peak_price", avg) / avg, 3) if avg > 0 else 0,
                                    "held_h": round((_time.time() - h["bought_at"].timestamp()) / 3600, 2) if h.get("bought_at") else 0,
                                    "entry_mc": h.get("avg_cost_mc", 0),
                                ...
                                try:
                                    await app.bot.send_message(
                                        chat_id=uid, parse_mode="Markdown",
                                        text=(
                                            "📉 *HUNTER — MACD BEARISH CROSS*\n\n"
                                            "*$" + _md(h["symbol"]) + "*\n"
                                            "MACD crossed below signal line\n"
                                            "Momentum turned bearish — exiting\n"
                                            "Exit: *" + str(round(cx, 2)) + "x*  |  PnL: *" + pstr(result_macd["realized"]) + "*\n"
                                            "_Sajad S1-01: MACD bearish cross = trend over_"
                                        ),
                                        reply_markup=main_menu_kb()
                                    )
                                except Exception:
                    ...
                    # ── Volume Confirmation: exit on fake breakout ────────────
                    # Sajad S1-01: new price high on declining volume = no fuel.
                    # Only fires on profitable positions (cx >= 1.20).
                    if not h.get("_vol_confirm_fired"):
                        if _sajad_volume_confirmation(h, price):
                            h["_vol_confirm_fired"] = True
                            cv_vol = h["amount"] * price
                            if cv_vol >= 0.01:
                                result_vol = sell_core(ud, uid, contract, cv_vol, price, "hunter_fake_breakout")
                                ud["apex_daily_pnl"] = (ud.get("apex_daily_pnl") or 0.0) + (result_vol.get("realized") or 0.0)
                                if result_vol["realized"] < 0:
                                    ud["apex_consec_losses"] = ud.get("apex_consec_losses", 0) + 1
                                else:
                                    ud["apex_consec_losses"] = 0
                                    ud["apex_total_wins"] = ud.get("apex_total_wins", 0) + 1
                                ud["apex_total_trades"] = ud.get("apex_total_trades", 0) + 1
                                apex_learn_record(uid, _apex_rich_record(h, cx, h.get("apex_peak_price", price), avg, result_vol["realized"], "hunter_fake_breakout", ud))
                                _apex_post_exit.setdefault(uid, {})[contract] = {
                                    "symbol": h["symbol"], "contract": contract,
                                    "exit_price": price, "entry_price": avg,
                                    "exit_reason": "hunter_fake_breakout",
                                    "exit_x": round(cx, 3),
                                    "peak_x": round(h.get("apex_peak_price", avg) / avg, 3) if avg > 0 else 0,
                                    "held_h": round((_time.time() - h["bought_at"].timestamp()) / 3600, 2) if h.get("bought_at") else 0,
                                    "entry_mc": h.get("avg_cost_mc", 0),
                                ...
                                try:
                                    await app.bot.send_message(
                                        chat_id=uid, parse_mode="Markdown",
                                        text=(
                                            "📊 *HUNTER — FAKE BREAKOUT*\n\n"
                                            "*$" + _md(h["symbol"]) + "*\n"
                                            "New price high on declining volume\n"
                                            "No fuel behind the move — exiting\n"
                                            "Exit: *" + str(round(cx, 2)) + "x*  |  PnL: *" + pstr(result_vol["realized"]) + "*\n"
                                            "_Sajad S1-01: Price HH needs vol HH to be real_"
                                        ),
                                        reply_markup=main_menu_kb()
                                    )
                                except Exception:
                ...
                if _is_red:
                    ...
                    # If Helius key available, check the last 20 transactions to
                    # identify WHO is selling. Different sellers = different actions:
                    #   Dev/bundle wallet selling → exit immediately (rug confirmed)
                    #   Single whale taking profit → hold, tighten trail 5%
                        ...
                        if _time.time() - _pi_last_ts > _pi_cooldown:
                            ...
                            try:
                                ...
                                if _pi_data:
                                    ...
                                    # Case A: Highly concentrated sell volume
                                    # One or few wallets dominate → likely dev/bundle dump
                                    if _pi_top3 >= 60:
                                        # Concentrated selling — confirm exit, tag reason
                                        h["_pullback_reason"] = f"concentrated_sell:{_pi_top3}%"
                                        logger.info(f"Pullback [{h['symbol']}]: top3={_pi_top3}% — dev/bundle dump confirmed, exiting")
                                    # Case B: Many small sellers — organic thin-book
                                    # buy% already low but distributed selling → hold
                                    elif _pi_maker >= 45 and _pi_top3 < 35:
                                        _pi_skip_exit = True
                                        h["_pullback_reason"] = f"organic_pressure:{_pi_maker}%makers"
                                        logger.info(f"Pullback [{h['symbol']}]: distributed selling ({_pi_maker}% makers), holding 1 cycle")
                                    # Case C: Single large wallet, others still buying
                                    elif _pi_maker >= 55 and _pi_top3 < 50:
                                        _pi_skip_exit = True
                                        # Tighten trail by 5% to capture remaining upside
                                        h["apex_trail_pct"] = max(0.04, h.get("apex_trail_pct", 0.22) - 0.05)
                                        h["_pullback_reason"] = f"whale_profit_take:tightened_trail"
                    ...
                    # ── Dev blacklist: block this contract from future sniper entries ──
                    # PDF S1-02: "Blacklist every rug dev wallet immediately."
                    # We can't track dev wallets directly, but we can block the contract
                    # so APEX never re-enters the same rugged token. sniper_seen TTL is
                    # 2h normally — use a far-future timestamp to make it permanent.
                    ud.setdefault("sniper_seen", {})[contract] = _time.time() + 86400 * 30  # 30-day block
                    try:
                ...
                if not _hp:
                    ...
                    # ── 1.3x ladder sell (25%) ────────────────────────────────────
                    if cx >= 1.3 and not h.get("apex_ladder_sold_1_3x") and _ladder_cv >= 0.02:
                        _sell_val = _ladder_cv * 0.25
                        if _sell_val >= 0.01:
                            _lad_result = sell_core(ud, uid, contract, _sell_val, price, "apex_ladder_1_3x")
                            if isinstance(_lad_result, dict):
                                h["apex_ladder_sold_1_3x"] = True
                                ...
                                try:
                                    await app.bot.send_message(
                                        chat_id=uid, parse_mode="Markdown",
                                        text=(
                                            "💰 *APEX — LADDER SELL (1.3x)*\n\n"
                                            "*$" + _md(h["symbol"]) + "*  hit *1.3x*\n"
                                            "Sold *25%* → locked *" + pstr(_lad_pnl) + "*\n"
                                            "🛡 Stop now at break-even\n"
                                            "Remaining *75%* continues trailing…"
                                        ),
                                        reply_markup=main_menu_kb()
                                    )
                                except Exception:
                    ...
                    # ── 2x ladder sell (25%) ─────────────────────────────────────
                    elif cx >= 2.0 and h.get("apex_ladder_sold_1_3x") and not h.get("apex_ladder_sold_2x"):
                        _ladder_amt2 = h.get("amount", 0)
                        _ladder_cv2  = _ladder_amt2 * price
                        if _ladder_cv2 >= 0.02:
                            _sell_val2 = _ladder_cv2 * 0.25
                            if _sell_val2 >= 0.01:
                                    ...
                                    try:
                                        await app.bot.send_message(
                                            ...
                                            text=(
                                                "💰 *APEX — LADDER SELL (2x)*\n\n"
                                                "*$" + _md(h["symbol"]) + "*  hit *2x*\n"
                                                "Sold *25%* more → locked *" + pstr(_lad_pnl2) + "*\n"
                                                "Remaining *50%* trailing with wide stop…"
                                            ),
                                            reply_markup=main_menu_kb()
                                        )
                                    except Exception:
                                ...
                                try:
                                    await app.bot.send_message(
                                        ...
                                        text=(
                                            ...
                                            "Sold *15%* to lock profit at zone\n"
                ...
                if cx >= _trail_activate:
                    if _hp:
                        ...
                        # ── Sajad Fib zone: tighten trail to 5% near 62% retrace ──
                        if _hp.get("fib_trail_tighten") and h.get("_fib_tightened"):
                            tpct = min(tpct, _hp.get("fib_tighten_pct", 0.05))
                    else:
                                    ...
                                    try:
                                        await app.bot.send_message(
                                            ...
                                            text=(
                                                ...
                                                "_Bag flushed \u2014 DCA opportunity if support holds._"
                                            ),
                                            reply_markup=main_menu_kb()
                                        )
                                    except Exception:
...
async def apex_process_entry_queue(app, uid: int, ud: dict) -> None:
    ...
    async with _get_user_lock(uid):
      for contract, entry in list(queue.items()):
        ...
        # ── Hunter: Sajad entry filters (S1-01, S1-02, S1-03) ────────────────
        # Run against LIVE data (not original scan data) so filters reflect
        # current market state at the moment of actual entry execution.
        _hp_check = get_apex_profile(ud)
        if _hp_check:   # Hunter mode only
            _sajad_ok, _sajad_reason = _sajad_entry_checks(info_live, _hp_check)
            if not _sajad_ok:
                logger.info(f"APEX Hunter queue: {contract} rejected by Sajad filter — {_sajad_reason}")
                continue
        ...
        # APEX always trades from vault — stop if vault insufficient
        ...
        try:
            ...
            h["apex_ladder_sold_1_3x"] = False   # 25% sold at 1.3x
            h["apex_ladder_sold_2x"]   = False   # 25% sold at 2x
...
async def do_buy_core(ud: dict, uid: int, contract: str, usd_amount: float, planned: bool = True, mood: str = "", vault_buy: bool = False) -> str | tuple:
    ...
    if contract in ud["holdings"]:
        ...
        h["amount"]        = na
        h["total_invested"] = nt
        h["total_ever_invested"] = round(h.get("total_ever_invested", 0.0) + usd_amount, 4)
        ...
        ud["holdings"][contract] = {
            ...
            "total_ever_invested": usd_amount,
            "total_sold":     0.0,
        ...
        if action == "set_balance":
            ...
            try:
                ...
                await message.reply_text(
                    "✅ Target: *" + money(target) + "*\n\n"
                    "How many days for this challenge?\nExample: `30`",
                    parse_mode="Markdown", reply_markup=cancel_kb()
                )
            except (ValueError, AssertionError):
                await message.reply_text("❌ Enter a valid number. Example: `5000`", parse_mode="Markdown")
            ...
            try:
                ...
                await message.reply_text(
                    "🎯 *Challenge Started!*\n\n"
                    "Starting: *" + money(start_eq) + "*\n"
                    "Goal: *" + money(target) + "*\n"
                    "Duration: *" + str(days) + " days*\n\n"
                    "Trade and check progress anytime from the Challenge menu.",
                    parse_mode="Markdown", reply_markup=main_menu_kb()
                )
            except (ValueError, AssertionError):
                await message.reply_text("❌ Enter a number of days between 1 and 365. Example: `30`", parse_mode="Markdown")
            ...
            try:
                ...
                ud["_persisted_competitions"] = {k: v for k, v in _competitions.items()}
                save_user(u.id, ud)
                del pending[u.id]
...
async def btn(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if cb == "mm":
        ...
        # Shown in buy submenu when no preset is set — redirect to settings
        pending[u.id] = {"action": "cfg_buy", "_prompt_msg_id": q.message.message_id}
        await q.edit_message_text(
            "⚙️ *SET DEFAULT BUY AMOUNT*\n\nEnter the amount in USD you want as your default buy preset:\nExample: `100`",
            parse_mode="Markdown",
            reply_markup=cancel_kb()
        )
    ...
    # ── CHALLENGE MODE ─────────────────────────────────────────────────────
        ...
        if ch:
            ...
            remaining  = max(0, ch["days"] - elapsed)
            # ── Manual equity only — exclude APEX vault + auto positions ──────
            _APEX_MOODS = ("APEX", "AI-Sniper", "APEX-DCA", "DCA", "Sniper")
            _manual_pos_val = sum(
                h["total_invested"] for h in ud["holdings"].values()
                if h.get("mood", "") not in _APEX_MOODS
            )
            current_eq = ud["balance"] + _manual_pos_val
            start_eq   = ch["start_eq"]
            target_eq  = ch["target_eq"]
            progress   = round((current_eq - start_eq) / (target_eq - start_eq) * 100, 1) if target_eq > start_eq else 0
            progress   = max(0, min(progress, 100))
            roi        = round((current_eq - start_eq) / start_eq * 100, 1) if start_eq > 0 else 0

            # ── Filter trades closed since challenge started ───────────────────
            # Only count MANUAL trades — exclude APEX, AI-Sniper, DCA auto-trades.
            # The challenge is a personal discipline tracker, not an APEX report.
            logs      = trade_log.get(u.id, [])
            ch_trades = [
                t for t in logs
                if _safe_dt(t.get("closed_at")) >= started_dt
                and t.get("mood", "") not in _APEX_MOODS
            ]
            wins      = [t for t in ch_trades if t.get("realized_pnl", 0) > 0]
            losses    = [t for t in ch_trades if t.get("realized_pnl", 0) <= 0]
            total_pnl = sum(t.get("realized_pnl", 0) for t in ch_trades)
            wr        = round(len(wins) / len(ch_trades) * 100) if ch_trades else 0
            best      = max(ch_trades, key=lambda t: t.get("realized_pnl", 0)) if ch_trades else None
            worst     = min(ch_trades, key=lambda t: t.get("realized_pnl", 0)) if ch_trades else None

            # ── Progress bar ──────────────────────────────────────────────────
            filled   = round(progress / 10)
            prog_bar = "█" * filled + "░" * (10 - filled)
            status   = "✅ COMPLETE!" if current_eq >= target_eq else ("📈" if roi >= 0 else "📉")

            txt = (
                "🎯 *CHALLENGE — DISCIPLINE TRACKER*\n"
                "━━━━━━━━━━━━━━━━━━\n\n"
                + status + "  *Goal:* " + money(start_eq) + " → " + money(target_eq) + "\n"
                "⏰ *Day " + str(elapsed) + "* of " + str(ch["days"]) + "  ·  *" + str(remaining) + " days left*\n\n"
                "📊 Progress: `" + prog_bar + "` *" + str(progress) + "%*\n"
                "💰 Current equity: *" + money(current_eq) + "*\n"
                "📈 ROI since start: *" + ("+" if roi >= 0 else "") + str(roi) + "%*\n\n"
                "━━━━━━━━━━━━━━━━━━\n"
                "📋 *Manual Trades: " + str(len(ch_trades)) + "*\n"
            )
            if ch_trades:
                txt += (
                    "Win Rate: *" + str(wr) + "%*"
                    + "  (" + str(len(wins)) + "W / " + str(len(losses)) + "L)\n"
                    "Total PnL: *" + pstr(total_pnl) + "*\n"
                    "ROI: *" + ("+" if roi >= 0 else "") + str(roi) + "%*\n"
                )
                if best:
                    txt += "\n🏆 Best: *$" + _md(best["symbol"]) + "*  " + pstr(best["realized_pnl"]) + "  (" + str(round(best.get("x", 0), 2)) + "x)\n"
                if worst:
                    txt += "💀 Worst: *$" + _md(worst["symbol"]) + "*  " + pstr(worst["realized_pnl"]) + "  (" + str(round(worst.get("x", 0), 2)) + "x)\n"
            else:
                txt += "_No trades closed yet. Start trading to track your performance._\n"

            await q.edit_message_text(
                txt, parse_mode="Markdown",
                reply_markup=InlineKeyboardMarkup([
                    [InlineKeyboardButton("📁 Export CSV", callback_data="ch_export")],
                    [InlineKeyboardButton("❌ Abandon",    callback_data="ch_abandon")],
                    [InlineKeyboardButton("◀ Back",        callback_data="v_more")],
                ]))
        else:
            ...
            await q.edit_message_text(
                "🎯 *CHALLENGE MODE*\n\n"
                "Set a goal, trade manually, and get a full performance report.\n\n"
                "Your current equity: *" + money(current_eq) + "*\n\n"
                "Choose a challenge:",
                parse_mode="Markdown",
                reply_markup=InlineKeyboardMarkup([
                    [InlineKeyboardButton("🚀 2x Balance",          callback_data="ch_2x")],
                    [InlineKeyboardButton("✏️ Custom target",       callback_data="ch_custom")],
                    [InlineKeyboardButton("◀ Back to More",         callback_data="v_more")],
                    [InlineKeyboardButton("🏠 Main Menu",           callback_data="mm")],
                ]))
        ...
        current_eq = ud["balance"] + sum(h["total_invested"] for h in ud["holdings"].values())
        target_eq  = round(current_eq * 2, 2)
        ud["challenge"] = {
            "start_eq":  current_eq,
            "target_eq": target_eq,
            "days":      30,
        ...
        save_user(u.id, ud)
        await q.edit_message_text(
            "🚀 *2x CHALLENGE STARTED!*\n\n"
            "Starting: *" + money(current_eq) + "*\n"
            "Goal: *" + money(target_eq) + "* (2x)\n"
            "Duration: *30 days*\n\n"
            "Trade manually and check your progress anytime from the Challenge menu.",
            parse_mode="Markdown", reply_markup=main_menu_kb())

    elif cb == "ch_export":
        # CSV export of all trades since challenge started
        ch = ud.get("challenge")
        if not ch:
            await q.answer("No active challenge.", show_alert=True)
            return
        import csv, io as _io
        started_dt = datetime.fromisoformat(ch["started"])
        logs       = trade_log.get(u.id, [])
        ...
        if not ch_trades:
            await q.answer("No trades to export yet.", show_alert=True)
            return
        buf = _io.StringIO()
        writer = csv.writer(buf)
        writer.writerow(["Date", "Symbol", "Invested", "Returned", "PnL", "X", "Hold(h)", "Reason"])
        total_pnl = 0.0
        wins = 0
        for t in sorted(ch_trades, key=lambda x: _safe_dt(x.get("closed_at"))):
            pnl = t.get("realized_pnl", 0)
            total_pnl += pnl
            if pnl > 0: wins += 1
            writer.writerow([
                _safe_dt(t.get("closed_at")).strftime("%Y-%m-%d %H:%M") if _safe_dt(t.get("closed_at")) != datetime.min else "",
                t.get("symbol", ""),
                round(t.get("invested", 0), 4),
                round(t.get("returned", 0), 4),
                round(pnl, 4),
                round(t.get("x", 0), 4),
                t.get("hold_h", ""),
                t.get("reason", ""),
        ...
        # Summary row
        wr = round(wins / len(ch_trades) * 100, 1) if ch_trades else 0
        writer.writerow([])
        writer.writerow(["SUMMARY", "", "", "", round(total_pnl, 4), "", "", f"WR:{wr}% | {wins}W/{len(ch_trades)-wins}L"])
        buf.seek(0)
        filename = "challenge_trades_" + datetime.now().strftime("%Y%m%d") + ".csv"
        await ctx.bot.send_document(
            chat_id=u.id,
            document=_io.BytesIO(buf.getvalue().encode("utf-8")),
            filename=filename,
            caption="📁 *Challenge Trade Log*\n" + str(len(ch_trades)) + " trades  |  WR: " + str(wr) + "%  |  PnL: " + pstr(total_pnl),
            parse_mode="Markdown",
        )
    ...
    # ── APEX VAULT MENU ───────────────────────────────────────────────────────
        ...
        _trail_str = "1.5x (ratchet)" if profile == "hunter" else "1.5x"
        _halt_str  = "20%" if profile == "hunter" else "20%"
        await q.edit_message_text(
            "⚙️ *APEX SETTINGS*\n\n"
            "Auto-tuned by self-learning engine every 10 trades.\n\n"
            "Min confidence: *" + str(conf_thr) + "/10*\n"
            "Min score: *" + str(score_min) + "/100*\n"
            "Daily loss halt: *" + _halt_str + "*\n"
            "Trail activates at: *" + _trail_str + "*\n"
            "Vault profit split: *" + str(int(split_pct*100)) + "%* → main balance\n\n"
            "🎛 *Risk Profile:* " + _hp_label + "\n"
            "_Use the Vault menu to adjust profit split %._",
            parse_mode="Markdown",
            reply_markup=InlineKeyboardMarkup([
        ...
        await q.edit_message_text(
            ...
            "🎯 *Hunter*\n"
            "Advanced entry filters and structured exit rules.\n\n"
            "*Entry filters:*\n"
            "• MC > $30K, Liq > $15K, Top10 < 20%, Age < 40h\n"
            "• Blocks green candle chasing (+25% 5m pump at ATH)\n"
            "• Blocks farmed/dead coins (flat price + zero volume)\n"
            "• Blocks instant launch pump (dev sniped own coin)\n\n"
            "*Exit rules:*\n"
            "• Ratchet: -18% SL → -4% at 1.25x → break-even at 1.5x\n"
            "• Lower High Rule: exits when bounce peak < previous peak\n"
            "• Staircase Break: exits when price breaks below higher low\n"
            "• Fib 62%: trail tightens to 5% near Fibonacci zone\n"
            "• Wide trail (22–28%) in profit zone — lets winners run\n\n"
        ...
        if old_profile == new_profile:
            ...
            _msg = (
                ...
                + ("🎯 *Hunter active.*\n"
                   "Entry: -18% SL on LOW risk, -14% MEDIUM, -10% HIGH.\n"
                   "At 1.25x: floor moves to -4% from entry.\n"
                   "At 1.5x: floor locks to break-even — can no longer lose.\n"
                   "Lower High, Staircase Break and Fib 62% exits active.\n"
                   "Wide trail (22–28%) once in profit zone."
...
def main():
    ...
    for _uid, _ud in users.items():
        ...
        # ── Restore competitions from creator's ud ────────────────────────────
        _cp = _ud.get("_persisted_competitions")
        if _cp:
            _competitions.update(_cp)
