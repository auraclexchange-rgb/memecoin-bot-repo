...
def token_card(info: dict, contract: str, ud: dict, sc: dict = None) -> str:
    ...
    # Build 3 separate X search links — sanitize text so Markdown doesn't break
    import urllib.parse, re
    name   = str(info.get("name",""))
    symbol = str(info.get("symbol",""))
    # Strip markdown-breaking chars from link display text
    safe_name   = re.sub(r'[_*\[\]()~`>#+\-=|{}.!]', '', name)[:15].strip() or "Name"
    safe_symbol = re.sub(r'[_*\[\]()~`>#+\-=|{}.!]', '', symbol).strip() or "Symbol"
    q_ca     = urllib.parse.quote(contract)
    q_name   = urllib.parse.quote(name)
    q_symbol = urllib.parse.quote("$" + symbol)
    base     = "https://x.com/search?q={}&src=typed_query&f=live"
    x_ca     = f"[📌 CA]({base.format(q_ca)})"
    x_name   = f"[📛 {safe_name}]({base.format(q_name)})"
    x_symbol = f"[🔤 ${safe_symbol}]({base.format(q_symbol)})"
    x_line   = f"\n━━━━━━━━━━━━━━━━\n🔍 *Search on 𝕏*\n{x_ca}  •  {x_name}  •  {x_symbol}"
