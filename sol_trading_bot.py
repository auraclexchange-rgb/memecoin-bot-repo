            ...
            try:
                ...
                if rc_data:
                    ...
                    if holders:
                        ...
                        # ── Fallback: outlier pool detection ──────────────────────
                        # RugCheck sometimes doesn't populate owner/tags/markets for
                        # the bonding curve account, letting it slip through as a
                        # "real" holder with 15–95% of supply while all others hold
                        # <1%. Pattern: top holder pct is >5x the second holder AND
                        # >8% total — this is statistically impossible for organic
                        # retail distribution, only happens with pool/curve accounts.
                        # Only applied on pump.fun tokens (pf_curve key present on
                        # the outer info dict, inferred from chain context here).
                        if len(real_holders) >= 2:
                            _top_pct  = float(real_holders[0].get("pct", 0))
                            _sec_pct  = float(real_holders[1].get("pct", 0))
                            _is_outlier = (
                                _top_pct > 8.0                              # >8% is unusual
                                and (_sec_pct == 0 or _top_pct / _sec_pct > 5.0)  # 5x gap
                                and _top_pct > sum(float(h.get("pct",0)) for h in real_holders[1:6])  # top > next 5 combined
                            )
                            if _is_outlier:
                                logger.debug(
                                    f"Outlier pool filter: removed top holder "
                                    f"{real_holders[0].get('address','?')[:12]} "
                                    f"({_top_pct}% vs #{2} {_sec_pct}%) for {contract[:12]}"
                                )
