...
def generate_trade_card(symbol: str, chain: str, pnl_str: str, x_val: str, held_h: str, bought_str: str, position_str: str, username: str, pnl_pct: str, pnl_positive: bool) -> "io.BytesIO | None":
    ...
    try:
        ...
        try:
            ...
            font_pill  = ImageFont.truetype("/usr/share/fonts/truetype/dejavu/DejaVuSans-Bold.ttf", 62)
        ...
        if os.path.exists(char_path):
            ...
            char_x = W - char_w + 20
            char_y = int((H - char_h) // 2)
            img.paste(char, (char_x, char_y), char)
            overlay = Image.new("RGBA", (W, H), (0, 0, 0, 0))
            ov_draw = ImageDraw.Draw(overlay)
            fade_start = max(0, char_x)
            fade_end = fade_start + 280
            for x in range(fade_start, min(fade_end, W)):
            ...
            draw = ImageDraw.Draw(img)
        chain_colors = {
            "solana": (153, 69, 255), "sol": (153, 69, 255),
            "ethereum": (98, 126, 234), "eth": (98, 126, 234),
            "base": (0, 82, 255), "bsc": (243, 186, 47),
            "bnb": (243, 186, 47), "arbitrum": (40, 160, 240),
            "arb": (40, 160, 240), "polygon": (130, 71, 229), "matic": (130, 71, 229),
        }
        chain_key = chain.lower()
        badge_col = chain_colors.get(chain_key, (80, 100, 160))
        chain_label = chain.upper()
        draw.rounded_rectangle([38, 22, 38 + len(chain_label)*16 + 30, 58], radius=12, fill=badge_col)
        draw.text((53, 40), chain_label, font=font_badge, fill=(255, 255, 255), anchor="lm")
        ...
        draw.text((68, 246), chain_label + "  " + prefix + pnl_str, font=font_pill, fill=txt_col, anchor="lm")
        pnl_col = (0, 220, 120) if pnl_positive else (220, 75, 75)
        stats = [
            ("PNL",      prefix + pnl_pct,                  pnl_col),
            ("Bought",   chain_label + " - " + bought_str,  (195, 210, 235)),
            ("Position", chain_label + " - " + position_str,(195, 210, 235)),
