...
def main_menu_kb() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("👁 Watchlist",       callback_data="v_watchlist"),
         InlineKeyboardButton("🐋 Whale Alerts",   callback_data="v_whale")],
        [InlineKeyboardButton("📊 Portfolio Chart", callback_data="v_chart"),
         InlineKeyboardButton("🎯 Challenge",       callback_data="v_challenge")],
        [InlineKeyboardButton("👥 Multi Account",   callback_data="v_accounts"),
         InlineKeyboardButton("🔗 Referrals",       callback_data="v_referrals")],
        [InlineKeyboardButton("📡 Channel Setup",   callback_data="v_channel"),
    ...
    if p:
        ...
        if action == "set_balance":
            ...
            if not ud.get("accounts"):
                ud["accounts"] = {}
            ud["accounts"][name] = {"balance": 5000, "holdings": {}, "savings": 0}
            del pending[u.id]
            await message.reply_text(f"✅ Account *{name}* created with $5,000!", parse_mode="Markdown", reply_markup=main_menu_kb())

        elif action == "ch_connect":
            # They need to forward a message from their channel
            if message.forward_from_chat:
                ch_id = message.forward_from_chat.id
                ud["channel_id"] = ch_id
                del pending[u.id]
                await message.reply_text(f"✅ Channel connected!\nID: `{ch_id}`\n\nTrade cards will now auto-post there.", parse_mode="Markdown", reply_markup=main_menu_kb())
            else:
                await message.reply_text("❌ Please forward a message FROM your channel, not just type the ID.")
            ...
            if not hasattr(btn, '_competitions') or code not in btn._competitions:
                await message.reply_text("❌ Competition not found.")
                return
