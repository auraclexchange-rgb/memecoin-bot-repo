...
async def _get_rugcheck_semaphore() -> "_asyncio.Semaphore":
    ...
    if _rugcheck_semaphore is None:
        ...
        _rugcheck_semaphore = _asyncio.Semaphore(1)
        ...
        if chain_id_raw in ("solana", "sol"):
            try:
                ...
                if _rc_fresh:
                    ...
                    async with sem:
                        await _asyncio.sleep(1.0)   # 1 req/sec max — RugCheck free tier
                        rc = await get_http()
                        rc_r = await rc.get(
...
def token_card(info: dict, contract: str, ud: dict, sc: dict = None) -> str:
    ...
    try:
        ...
        # ── Liq multiplier vs MC ───────────────────────────────────────────────
        liq_mult = ""
        if mc > 0 and liq > 0:
            _lm = round(liq / mc, 2)
            liq_mult = " [x" + str(_lm) + "]"

        # ── Buy pressure — text only, no bar ─────────────────────────────────
        if buy_pct >= 60:
            pressure_icon = "🟢"
        elif sell_pct >= 60:
            pressure_icon = "🔴"
        ...
        # ── Low liq warning (compact) ─────────────────────────────────────────
        liq_warn_inline = " ⚠️ LOW LIQ" if liq < 50_000 else ""
        ...
        _rc_rate_limited_flag = info.get("rc_rate_limited", False)
        _is_solana_token      = chain_raw in ("solana", "sol")
        if _rc_rate_limited_flag and _is_solana_token and not any(
            x is not None for x in [no_mint, no_freeze, lp_burn, top10, insider]
        ):
            sec_block = (
                "─────────────────────────\n"
        ...
        if any(x is not None for x in [no_mint, no_freeze, lp_burn, top10, insider]):
            ...
            if _dev_sold:
                _holder_parts.append("⚠️ Dev Sold")
        ...
        if _fs:
            ...
            # x change since first scan
            _fs_x = round(mc / _fs_mc, 2) if _fs_mc > 0 and mc > 0 else 0
            _fs_x_str = ("+" if _fs_x >= 1 else "") + str(_fs_x) + "x"
            _fs_x_icon = "🚀" if _fs_x >= 2 else ("📈" if _fs_x >= 1.1 else ("📉" if _fs_x < 1 else "➡️"))
            # elapsed time
            import time as _fst
            _fs_elapsed = _fst.time() - _fs_ts
            if _fs_elapsed < 3600:
                _fs_age = str(int(_fs_elapsed / 60)) + "m"
            elif _fs_elapsed < 86400:
                _fs_age = str(round(_fs_elapsed / 3600, 1)) + "h"
            else:
                _fs_age = str(round(_fs_elapsed / 86400, 1)) + "d"
            first_scan_line = (
                "─────────────────────────\n"
                "👤 *@" + _md(_fs_uname) + "* @ " + mc_str(_fs_mc)
                + "  " + _fs_x_icon + " *" + _fs_x_str + "*  (" + _fs_age + " ago)\n"
            )
        ...
        # ── Truncated CA for display ──────────────────────────────────────────
        _ca_display = (contract[:10] + "..." + contract[-6:]) if len(contract) > 20 else contract
        ...
        card = (
            ...
            + "📋 `" + contract + "`\n"
            + "─────────────────────────\n"
            # ── Stats block (compact, merged lines) ───────────────────────────
            + "├─ 💲 *" + format_token_price(price) + "*\n"
            + "├─ 📊 MC *" + mc_str(mc) + "*  Vol *" + _compact(vol_h1) + "* (1h)\n"
            + "├─ 💧 Liq *" + _compact(liq) + "*" + liq_mult + "  (" + str(liq_pct) + "%)" + liq_warn_inline + "\n"
            + "├─ ⏰ Age *" + age_display + "*  🏆 ATH *" + ath_str + "*\n"
            + "└─ " + pressure_icon + " *" + str(buy_pct) + "%* buys  ·  " + str(buys) + "/" + str(sells) + "\n"
            + "─────────────────────────\n"
            # ── Price changes ─────────────────────────────────────────────────
...
async def send_token_card(
    ...
    if is_query:
        ...
        if target.message.photo or target.message.document:
            # Photo message — edit the caption in place (keeps the image)
            try:
                ...
                pass
            # Caption edit failed (e.g. too long) — delete and resend as text
            try:
        ...
        # Send banner image with card text as caption — one single message.
        # Falls back to text-only if no banner available or image fails.
        ...
        if header_url:
            try:
                ...
                pass   # banner failed — fall through to text-only
        if not _sent:
            await ctx.bot.send_message(
                chat_id=chat_id,
                text=card_txt,
...
def group_token_card(info: dict, contract: str) -> str:
    ...
    No position block, no user balance, no admin data.
    Only public on-chain data everyone can see.
    """
    def fc(v):
        v = float(v or 0)
        e = "🚀" if v >= 100 else "📈" if v >= 20 else "🟢" if v >= 0 else "🔴" if v >= -20 else "💀"
        return e + " " + ("+" if v >= 0 else "") + str(round(v, 1)) + "%"
    ...
    dex      = info.get("dex", "").upper().replace("_V2","").replace("_V3","").replace("_","")
    price    = info.get("price", 0)
    mc       = info.get("mc", 0)
    liq      = info.get("liq", 0)
    liq_pct  = info.get("liq_pct", 0)
    ...
    chain_sym = _CHAIN_ICONS.get(chain, "⛓")
    if age_h < 1:   age_display, age_e = str(round(age_h*60))+"m", "🆕"
    elif age_h < 24: age_display, age_e = str(round(age_h,1))+"h", "⏰"
    else:           age_display, age_e = str(round(age_h/24,1))+"d", "📅"

    bar = "█" * round(buy_pct/10) + "░" * (10 - round(buy_pct/10))
    pressure = "BUYING 🟢" if buy_pct >= 60 else ("SELLING 🔴" if (100-buy_pct) >= 60 else "NEUTRAL ⚖️")

    # ── Security block (public data only) ─────────────────────────────────────
    sec_block = ""
    rc_rate_limited = info.get("rc_rate_limited", False)
    if rc_rate_limited:
        sec_block = "─────────────────────────\n🔒 ⚠️ Security audit loading…\n"
    elif any(x is not None for x in [no_mint, no_freeze, lp_burn, top10]):
        audit_parts = []
        if no_mint   is not None: audit_parts.append("✅ No Mint"   if no_mint   else "🚨 Mint")
        if no_freeze is not None: audit_parts.append("✅ No Freeze" if no_freeze else "🚨 Freeze")
        if lp_burn   is not None: audit_parts.append("🔥 LP " + str(lp_burn) + "%")
        holder_parts = []
        if top10    is not None: holder_parts.append("Top10: " + str(top10) + "%")
        if insider  is not None:
            _ic = "🐭" if insider == 0 else ("🟡" if insider < 15 else "🔴")
            holder_parts.append(_ic + " Insiders: " + str(insider) + "%")
        risk_flags = [r for r in rug_risks if isinstance(r, str)][:2]
        sec_block = (
            "─────────────────────────\n"
            "🔒 " + "  ".join(audit_parts) + "\n"
    ...
    # ── OG/COPY badge ─────────────────────────────────────────────────────────
    _is_copycat = any("copycat" in r.lower() for r in (rug_risks or []))
    _badge = "🔴 COPY" if _is_copycat else "🟢 OG"
    ...
    card = (
        ...
        + "├─ *" + pressure + "*\n"
        + "├─ `" + bar + "`\n"
        + "└─ 🛒 " + str(buys) + " buys (" + str(buy_pct) + "%)  🏃 " + str(sells) + " sells\n"
        + sec_block
        + "─────────────────────────\n"
        + "_Tap ⚡ Buy to trade in your own account_ 👇"
    )
    if len(card) > 4096:
        card = card[:4092] + "…"
    return card
...
def main():
    ...
    async def _on_startup(application):
        global _rugcheck_semaphore
        _rugcheck_semaphore = _asyncio.Semaphore(1)
        logger.info("RugCheck semaphore initialized (max 1 concurrent — prevents 429 bursts).")
