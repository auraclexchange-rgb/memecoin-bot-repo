...
def generate_trade_card(symbol: str, chain: str, pnl_str: str, x_val: str, held_h: str, bought_str: str, position_str: str, username: str, pnl_pct: str, pnl_positive: bool, closed_at: datetime | None = None) -> "io.BytesIO | None":
    ...
    try:
        ...
        # ── Fonts: Noto CJK (full Unicode) → DejaVu → FreeSans ──────────────
        _dir = os.path.dirname(os.path.abspath(__file__))
        _bold, _regular = None, None
        for _p in [
            ...
            if os.path.exists(_p): _bold = _p; break
        for _p in [
            ...
            if os.path.exists(_p): _regular = _p; break
...
async def get_token(contract: str, force: bool = False) -> dict | None:
    ...
    try:
        ...
        import time
        _token_cache[contract] = {"data": result, "ts": time.time()}
        return result
    except Exception as e:
...
def sniper_score(info: dict) -> dict:
    """
    Dedicated sniper scoring — built for early memecoin launches.
    Rewards: fresh age, organic spread volume, complete socials,
             healthy liq ratio, accelerating momentum, safe MC entry.
    Penalizes: rugs, wash trading, whale concentration, no socials.
    Max 100 pts across 8 categories.
    """
    score     = 0
    strengths = []
    warnings  = []
    flags     = []   # hard disqualifiers shown in red
    ...
    ch_h6    = info.get("ch_h6", 0)
    twitter  = info.get("twitter", "")
    telegram = info.get("telegram", "")
    website  = info.get("website", "")
    total_tx = buys + sells

    # ── 1. LAUNCH WINDOW (0–20 pts) ─────────────────────────────────────────
    # Sweet spot: 30min–4h. Too early = coordinated dump. Too late = already pumped.
    if age_h < 0.33:           # under 20 min
        score += 4
        warnings.append("Under 20min old — coordinated dump risk")
    elif age_h < 1:            # 20min–1h  ← best snipe window
        score += 20
        strengths.append("🔥 Perfect launch window (20–60min)")
    elif age_h < 3:            # 1–3h      ← still early
        score += 16
        strengths.append("Early entry (1–3h)")
    elif age_h < 6:            # 3–6h      ← getting late
        score += 10
        warnings.append("Getting late (3–6h in)")
    else:                       # over 6h   ← sniper shouldn't see this
        score += 2
        warnings.append("Too old for snipe entry")

    # ── 2. SOCIAL COMPLETENESS (0–20 pts) ───────────────────────────────────
    # Real projects set up socials before/at launch. No socials = throwaway.
    social_pts = 0
    if twitter:
        social_pts += 8
        strengths.append("Twitter present")
    else:
        warnings.append("No Twitter")
    if telegram:
        social_pts += 8
        strengths.append("Telegram present")
    else:
        warnings.append("No Telegram")
    if website:
        social_pts += 4
        strengths.append("Website present")
    if social_pts == 0:
        flags.append("🚨 Zero socials — likely throwaway token")
    score += social_pts

    # ── 3. LIQUIDITY HEALTH (0–15 pts) ──────────────────────────────────────
    # Liq% of MC is more important than raw liq for new tokens.
    # Under 10% liq/MC = easy rug. Over 50% = whale trap or fake pool.
    if liq_pct >= 25:
        score += 15
        strengths.append(f"Healthy liq ratio ({liq_pct}% of MC)")
    elif liq_pct >= 15:
        score += 11
        strengths.append(f"Good liq ratio ({liq_pct}%)")
    elif liq_pct >= 8:
        score += 6
        warnings.append(f"Low liq ratio ({liq_pct}%)")
    else:
        score += 0
        flags.append(f"🚨 Dangerous liq ratio ({liq_pct}%) — easy rug")

    # Raw liq floor
    if liq < 5_000:
        flags.append("🚨 Liq under $5K — near-certain rug")
    elif liq < 15_000:
        warnings.append(f"Low raw liquidity (${liq:,.0f})")

    # ── 4. ENTRY MC (0–15 pts) ──────────────────────────────────────────────
    # Best snipe entries: $20K–$300K MC. Higher = less upside. Lower = too risky.
    if 20_000 <= mc <= 100_000:
        score += 15
        strengths.append(f"🎯 Perfect entry MC ({mc/1000:.0f}K)")
    elif 100_000 < mc <= 300_000:
        score += 12
        strengths.append(f"Good entry MC ({mc/1000:.0f}K)")
    elif 300_000 < mc <= 700_000:
        score += 8
        warnings.append(f"Mid MC entry ({mc/1000:.0f}K — less upside)")
    elif mc < 20_000:
        score += 4
        warnings.append(f"Very low MC (${mc:,.0f}) — ultra risky")
    else:
        score += 3
        warnings.append(f"High MC entry ({mc/1000:.0f}K — limited upside)")

    # ── 5. VOLUME SPREAD / ORGANIC CHECK (0–15 pts) ─────────────────────────
    # Many small buys = organic retail interest.
    # Few buys with huge volume = whales / wash trading.
    vol_mc_ratio = (vol_h1 / mc) if mc > 0 else 0
    avg_buy_size = (vol_h1 / buys) if buys > 0 else 0

    if vol_mc_ratio > 10:
        flags.append(f"🚨 Vol/MC={vol_mc_ratio:.1f}x — probable wash trading")
    elif vol_mc_ratio > 5:
        warnings.append(f"High vol/MC ratio ({vol_mc_ratio:.1f}x) — check wallets")

    if buys >= 100:
        score += 15
        strengths.append(f"Strong buyer spread ({buys} buys/24h)")
    elif buys >= 50:
        score += 11
        strengths.append(f"Good buyer count ({buys} buys)")
    elif buys >= 25:
        score += 6
        warnings.append(f"Low buyer count ({buys} buys)")
    else:
        score += 2
        warnings.append(f"Very few buyers ({buys}) — thin interest")

    # Flag if avg buy size is suspiciously large (whale concentration)
    if avg_buy_size > 5_000:
    ...
    # ── 6. BUY PRESSURE (0–10 pts) ──────────────────────────────────────────
    if buy_pct >= 70:
        score += 10
        strengths.append(f"Dominant buy pressure ({buy_pct}%)")
    elif buy_pct >= 60:
        score += 7
        strengths.append(f"Strong buy pressure ({buy_pct}%)")
    elif buy_pct >= 52:
        score += 4
    elif buy_pct < 45:
        warnings.append(f"Sell pressure ({100-buy_pct}% sells)")
        score += 0

    # ── 7. MOMENTUM / PRICE ACTION (0–10 pts) ───────────────────────────────
    # Look for: 5m positive AND 1h positive = momentum building
    # Not: straight up parabolic (dump incoming) or straight down
    if ch_m5 > 0 and ch_h1 > 0 and ch_h1 < 200:
        score += 10
        strengths.append(f"Building momentum (5m:{ch_m5:+.1f}% 1h:{ch_h1:+.1f}%)")
    elif ch_m5 > 0 and ch_h1 >= 200:
        score += 4
        warnings.append(f"Parabolic 1h move (+{ch_h1:.0f}%) — late entry risk")
    elif ch_m5 > 0 or ch_h1 > 0:
        score += 5
    elif ch_m5 < -10 and ch_h1 < -10:
        warnings.append(f"Dumping hard (5m:{ch_m5:.1f}% 1h:{ch_h1:.1f}%)")

    # 5m volume acceleration vs 1h average
    vol_h1_per_5m = (vol_h1 / 12) if vol_h1 > 0 else 0
    if vol_m5 > vol_h1_per_5m * 2 and vol_m5 > 500:
        score += 3
        strengths.append("Volume accelerating 🚀")
    elif vol_m5 < vol_h1_per_5m * 0.3 and vol_h1_per_5m > 100:
        warnings.append("Volume drying up")

    # ── 8. HARD PENALTIES ───────────────────────────────────────────────────
    if len(flags) >= 2:
        score = max(0, score - 25)   # multiple red flags = tank the score
    elif len(flags) == 1:
        score = max(0, score - 12)

    if buy_pct < 40:
        score = max(0, score - 10)
    if liq_pct < 5 and liq < 10_000:
...
def get_user(uid: int, uname: str) -> dict:
    if uid not in users:
        users[uid] = {
            ...
            "sniper_filters": {
                "min_score":        45,      # sniper_score minimum (not AURACLE)
                "min_liq":          15_000,  # $15K min liquidity (catches earlier)
                "min_mc":           30_000,  # $30K min MC
                "max_mc":           5_000_000, # $5M max MC
                "max_age_h":        6,       # max 6 hours old — fresh launches only
                "buy_amount":       100,
                "min_buys_h1":      30,      # min 30 buys last hour (spread volume)
                "min_buy_pct":      52,      # buy pressure must dominate
                "max_vol_mc_ratio": 8.0,     # vol/MC cap to filter wash trading
            },
            "sniper_bought":    [],
            ...
            # Favourites
            "favourites":        [],            # Feature: starred/pinned watchlist tokens
...
def token_card(info: dict, contract: str, ud: dict, sc: dict = None) -> str:
    ...
    try:
        ...
        chain_sym = chain_icons.get(chain, "⛓")
        ...
        combined = urllib.parse.quote(f"({name} OR ${symbol} OR {contract})")
        x_line = (
            "🔍 Search 𝕏:  "
            + "[All]("       + base_x.format(combined)                           + ")  "
            + "[CA]("        + base_x.format(urllib.parse.quote(contract))       + ")  "
            + "[" + safe_nm  + "](" + base_x.format(urllib.parse.quote(name))   + ")  "
            + "[$" + safe_sym + "](" + base_x.format(urllib.parse.quote("$"+symbol)) + ")\n"
        )

        # ── footer links: GT · DT · DS · DV · BE · PF ────────────────────────
        dex_chain  = {"solana":"solana","sol":"solana","ethereum":"ethereum","eth":"ethereum",
                      "bsc":"bsc","bnb":"bsc","base":"base","arbitrum":"arbitrum"}.get(chain_raw,"solana")
...
async def send_token_card(
    ...
    try:
        ...
        if pair_addr:
            ...
            ohlcv = await _aio.wait_for(fetch_ohlcv(pair_addr, chain_id), timeout=6)
...
def buy_kb(contract: str, ud: dict) -> InlineKeyboardMarkup:
    ...
    # Favourite star
    favs    = ud.get("favourites", [])
    fav_lbl = "⭐ Unfav" if contract in favs else "☆ Favourite"
    fav_cb  = "wl_unfav_" + contract if contract in favs else "wl_fav_" + contract
    ...
    rows = [
        ...
        [InlineKeyboardButton("📉 DCA",            callback_data="dca_" + contract),
         InlineKeyboardButton(fav_lbl,             callback_data=fav_cb)],
...
async def run_checker(app: Application):
    ...
    if all_contracts:
        await _aio.gather(*[get_token(c, force=True) for c in all_contracts])
    # ─────────────────────────────────────────────────────────────────────────
...
async def sniper_scan() -> list:
    """
    Multi-feed scan for new launches with complete profiles.
    Strategy:
      1. Pull token-profiles/latest  — tokens that just got a full profile set up
      2. Pull token-boosts/latest    — tokens team actively promoted (real project signal)
      3. Merge, deduplicate by tokenAddress
      4. Pre-filter: must have Twitter + Telegram (complete profile check)
    """
        ...
        if True:
            for url in endpoints:
                try:
                    ...
                    logger.warning(f"Sniper feed error {url}: {_e}")
    except Exception as e:
        logger.warning(f"Sniper scan error: {e}")

    # Pre-filter: must have at least Twitter OR Telegram in the profile
    filtered = []
    for item in results.values():
        links = item.get("links", [])
        has_twitter  = any(l.get("type","").lower() in ("twitter","x")    for l in links)
        has_telegram = any(l.get("type","").lower() == "telegram"          for l in links)
        has_website  = any(l.get("type","").lower() in ("website","web")   for l in links)
        # Must have at least 2 of 3 social signals
        social_count = sum([has_twitter, has_telegram, has_website])
        if social_count >= 2:
            filtered.append(item)

    logger.info(f"Sniper scan: {len(results)} raw → {len(filtered)} with complete profiles")
    return filtered
...
ANALYZE — focus on these 5 things:
1. ORGANIC VOLUME: Is this real retail interest or coordinated? Look at avg buy size vs buyer count.
2. RUG SAFETY: Liq% of MC, age, sell pressure — will devs pull?
3. ENTRY TIMING: Is this still early enough for 2–5x? Or already pumped?
4. SOCIAL PROOF: Twitter/TG active signals real community.
5. MOMENTUM QUALITY: Steady climb vs parabolic spike (spike = dump incoming).

Be DECISIVE. If score ≥ 60 and no hard flags → lean SNIPE.
If hard flags present → SKIP regardless of score.
...
def _ai_report_text(info: dict, sc: dict, ai: dict, contract: str = "") -> str:
    ...
    # Confidence bar (filled / empty blocks)
    filled    = "█" * confidence
    empty     = "░" * (10 - confidence)
    ...
    return (
        ...
        "🛒 Buys: *" + str(buys) + "* (" + str(buy_pct) + "% buy pressure)\n"
        "━━━━━━━━━━━━━━━━━━\n"
...
async def _broadcast_to_channel(bot, channel_id: int, info: dict, sc: dict, ai: dict, contract: str) -> bool:
    """Post full AI signal card to user's broadcast channel/group. Returns True on success."""
    try:
        report = _ai_report_text(info, sc, ai, contract=contract)
        ...
        gt_url = "https://www.geckoterminal.com/" + dex_chain + "/pools/" + contract
        # Deep link — opens bot and auto-loads the token card
        bot_url = ("https://t.me/" + _bot_username + "?start=" + contract) if _bot_username else None
        kb_rows = []
        if bot_url:
            kb_rows.append([InlineKeyboardButton("⚡ Buy on Auracle", url=bot_url)])
        kb_rows.append([InlineKeyboardButton("🔍 View Token Live ↗", url=gt_url)])
        await bot.send_message(
            chat_id=channel_id,
            text="📡 *AURACLE AI SIGNAL*\n\n" + report,
            parse_mode="Markdown",
...
async def sniper_job(ctx: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        # Fetch bot username once for deep links
        ...
        # Pre-fetch all scanned token data in parallel for speed
        unique_contracts = []
        ...
        if unique_contracts:
            ...
            await _aio2.gather(*[get_token(c, force=True) for c in unique_contracts])
        seen = set()  # reset for main loop
        ...
        for item in raw_tokens:
            ...
            try:
                info = await get_token(contract)  # hits cache now
                if not info:
                    continue

                token_chain = _sniper_chain_id(info.get("chain", ""))
                sc          = sniper_score(info)   # dedicated sniper scoring
            except Exception as e:
                logger.warning(f"sniper_job: token fetch/score failed for {contract}: {e}")
                continue
        ...
        for uid, ud in active_users:
            try:
                ...
                # ── Deduplicate — skip if already analyzed OR bought ───────
                sniper_bought  = ud.setdefault("sniper_bought", [])
                already_logged = {e["contract"] for e in ud.get("sniper_log", [])}
                if contract in sniper_bought or contract in already_logged:
                    continue

                # ── Basic pre-filters with skip reason tracking ────────────
                skip_reason = None

                if sc.get("flags"):
                    # Hard disqualifiers — skip immediately regardless of score
                    skip_reason = "Hard flag: " + sc["flags"][0]

                elif sc["score"] < int(sf.get("min_score", 45)):
                    skip_reason = f"Sniper score too low ({sc['score']}/100, need {sf.get('min_score',45)})"

                elif info["liq"] < float(sf.get("min_liq", 30_000)):
                    skip_reason = f"Liquidity too low (${info['liq']:,.0f}, need ${float(sf.get('min_liq',30_000)):,.0f})"

                elif not (float(sf.get("min_mc", 50_000)) <= info["mc"] <= float(sf.get("max_mc", 10_000_000))):
                    skip_reason = f"MC out of range (${info['mc']:,.0f})"

                elif info.get("age_h") and info["age_h"] > float(sf.get("max_age_h", 6)):
                    skip_reason = f"Too old ({round(info['age_h'],1)}h, max {sf.get('max_age_h',6)}h)"

                else:
                    # ── Volume spread / organic check ──────────────────────
                    buys     = info.get("buys", 0)
                    buy_pct  = info.get("buy_pct", 50)
                    vol_h1   = info.get("vol_h1", 0)
                    mc       = info.get("mc", 1)
                    ...
                    min_buys = int(sf.get("min_buys_h1", 30))   # at least 30 unique buys last hour
                    if buys < min_buys:
                        skip_reason = f"Too few buyers ({buys} buys, need {min_buys})"

                    elif buy_pct < int(sf.get("min_buy_pct", 52)):
                        skip_reason = f"Sell pressure dominant ({buy_pct}% buys)"

                    elif vol_mc_ratio > float(sf.get("max_vol_mc_ratio", 8.0)):
                        skip_reason = f"Suspicious volume (vol/MC={round(vol_mc_ratio,1)}x — possible wash)"

                    # ── Socials check ──────────────────────────────────────
                    elif not info.get("twitter") and not info.get("telegram"):
                        skip_reason = "No socials (no Twitter or Telegram)"

                # Log and skip if pre-filter failed
                if skip_reason:
                ...
                # Log every decision with full AI + sniper score details
                ud.setdefault("sniper_log", []).append({
                    "contract":    contract,
                    "symbol":      info["symbol"],
                    "chain":       info.get("chain", "?"),
                    "mc":          info["mc"],
                    "liq":         info.get("liq", 0),
                    "age_h":       round(info.get("age_h", 0), 2),
                    "score":       sc["score"],
                    "verdict":     ai["verdict"],
                ...
                if ud.get("sniper_advisory"):
                    # Cache analysis so "View Analysis" button can retrieve it later
                    _sniper_analysis_cache.setdefault(uid, {})[contract] = {
                        "info": info, "sc": sc, "ai": ai
                    }
                    # Trim cache to last 20 tokens per user
                    cache = _sniper_analysis_cache[uid]
                    if len(cache) > 20:
                        oldest = list(cache.keys())[:-20]
                        for k in oldest:
                            del cache[k]

                    # ── Broadcast full card to channel/group if configured ──
                    ch_id = ud.get("sniper_broadcast_channel")
                    if ch_id:
                        broadcast_ok = await _broadcast_to_channel(ctx.bot, int(ch_id), info, sc, ai, contract)
                        if broadcast_ok:
                            ch_name = ud.get("sniper_broadcast_name", str(ch_id))
                            try:
                                await ctx.bot.send_message(
                                    chat_id=uid,
                                    text="📡 *$" + info["symbol"] + "* signal sent to *" + ch_name + "*",
                                    parse_mode="Markdown"
                                )
                            except Exception:
                                pass

                    # ── Send compact pill to DM if notifications ON ────────
                    if ud.get("sniper_adv_notify", True):
...
async def btn(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if cb == "mm":
        ...
        _pos_infos = await _aio.gather(*[get_token(c) for c in _pos_contracts])
    ...
    # ── SNIPER MODE v2 ─────────────────────────────────────────────────────────
        ...
        await q.edit_message_text(
            ...
            "🔧 *Filters:*\n"
            "  Min Score: *"    + str(sf.get("min_score",50))          + "/100*\n"
        ...
        buttons.append([InlineKeyboardButton("🗑 Clear Log", callback_data="sniper_log_clear")])
        buttons.append([InlineKeyboardButton("◀ Back",       callback_data="v_sniper")])
    ...
    # FAVOURITE TOKENS
    # ══════════════════════════════════════════════════════════════════════
    elif cb.startswith("wl_fav_"):
        contract = cb[7:]
        favs = ud.setdefault("favourites", [])
        if contract not in favs:
            favs.append(contract)
        # Refresh token card
        info2 = await get_token(contract)
        sym = info2["symbol"] if info2 else contract[:8]
        await q.answer("⭐ " + sym + " added to favourites")
        await q.edit_message_reply_markup(reply_markup=buy_kb(contract, ud))

    elif cb.startswith("wl_unfav_"):
        contract = cb[9:]
        favs = ud.get("favourites", [])
        if contract in favs:
            favs.remove(contract)
        info2 = await get_token(contract)
        sym = info2["symbol"] if info2 else contract[:8]
        await q.answer("☆ " + sym + " removed from favourites")
        await q.edit_message_reply_markup(reply_markup=buy_kb(contract, ud))
        ...
        await q.edit_message_text(
            ...
            "⭐ *Favourites* — star tokens to pin them to the top\n"
            "Tap ☆ Favourite on any token card to star it.\n\n"
