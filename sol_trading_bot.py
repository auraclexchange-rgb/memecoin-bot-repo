...
def _compact_pill_text(info: dict, sc: dict, ai: dict) -> str:
    """
    Compact DM pill for Advisory Mode.
    Sent to user DM when sniper_adv_notify=True.
    Short, scannable — 6-8 lines max.
    """
    symbol   = info.get("symbol", "???")
    ...
    thesis_line = ("\n_" + thesis[:120] + ("…" if len(thesis) > 120 else "") + "_") if thesis else ""

    return (
        f"🤖 *AI SNIPER ALERT* — *${symbol}* `{chain}`\n"
        f"─────────────────────\n"
        f"{verdict_icon} *{verdict}*  ·  Conf: `{conf_bar}` {confidence}/10\n"
        f"{s_icon} Score: *{score}/100*  ·  Rug: {rug_icon} {rug_risk}  ·  Mom: {momentum}\n"
        f"MC: *{mc_str(mc)}*  ·  Liq: *${liq:,.0f}*  ·  Age: *{age_str}*  ·  Buy%: *{buy_pct}%*"
        f"{thesis_line}\n"
        f"─────────────────────\n"
        f"_Tap to review ↓_"
    )
...
def _ai_report_text(info: dict, sc: dict, ai: dict, contract: str = "", expanded: bool = False) -> str:
    """
    Full AI analysis report — used for:
      • Channel broadcast  (_broadcast_to_channel, parse_mode=Markdown)
      • snp_view_ callback (full analysis tap, parse_mode=Markdown)
    ...
    verdict    = ai.get("verdict", "WAIT")
    confidence = ai.get("confidence", 0)
    thesis     = ai.get("thesis", "No thesis available.")
    rug_risk   = ai.get("rug_risk", "MEDIUM")
    momentum   = ai.get("momentum", "FLAT")
    social     = ai.get("social_score", "NONE")
    ...
    socials_parts = []
    if twitter:  socials_parts.append("🐦 TW")
    if telegram: socials_parts.append("✈️ TG")
    if website:  socials_parts.append("🌐 Web")
    social_str = "  ·  ".join(socials_parts) if socials_parts else "None"

    # Green / red flag lines
    gf_lines = "\n".join(["  ✅ " + f for f in green_flags]) if green_flags else "  _None detected_"
    rf_lines = "\n".join(["  ⚠️ " + f for f in red_flags])   if red_flags   else "  _None detected_"
    ...
    entry_line = (
        f"\n💰 *Suggested Entry:* `${suggested:,.2f}`"
        if verdict in ("SNIPE", "WAIT") and suggested > 0 else ""
    )
    ...
    return (
        ...
        f"─────────────────────\n"
        f"*${symbol}*  ·  {name}  ·  {chain}\n\n"
        f"{verdict_icon} *VERDICT: {verdict}*  ·  Conf: `{conf_bar}` {confidence}/10\n"
        f"{rug_icon} Rug Risk: *{rug_risk}*  ·  Momentum: *{momentum}*  ·  Social: *{social}*\n\n"
        f"_{thesis}_\n\n"
        f"─────────────────────\n"
        f"📊 *SCORE:* `{score_bar}` *{score}/100* {s_icon}\n\n"
        f"*MC:* {mc_str(mc)}  ·  *Liq:* ${liq:,.0f} ({round(liq_pct, 1)}%)\n"
        f"*Age:* {age_str}  ·  *Buy%:* {buy_pct}%  ·  *Vol/h:* ${vol_h1:,.0f}\n"
        f"*Mint:* {mint_str}  ·  *LP:* {lp_str}\n"
        f"*Socials:* {social_str}\n\n"
        f"✅ *Green Flags:*\n{gf_lines}\n\n"
        f"⚠️ *Red Flags:*\n{rf_lines}\n"
        + (f"\n🚨 *Hard Flags:* {flags_line}\n" if flags_line else "")
        + entry_line
    )
