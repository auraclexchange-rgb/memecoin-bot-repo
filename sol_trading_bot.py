...
async def send_token_card(
    ...
    """
    Send chart as a plain photo (no buttons), then token card as a
    separate text message with all buttons attached.
    This keeps all button callbacks working since they edit a text message.
    """
    ...
    # Try to generate and send chart as standalone photo (no buttons)
    try:
        ohlcv     = []
    ...
    # Send token card as plain text message with all buttons
    if is_query:
        # Delete the old button message so it doesn't leave dead buttons behind
        try:
