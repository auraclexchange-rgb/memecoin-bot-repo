...
def _card_stats_block(info: dict, contract: str, first_scan_line: str = "") -> str:
    ...
    if info.get("rc_rate_limited") and not any(x is not None for x in [no_mint, no_freeze, lp_burn, top10, insider]):
        ...
        # Top10 with deep link if available
        _top_holders_data = info.get("top_holders_data", [])
        top10_str = ""
        if top10 is not None:
            if _top_holders_data and _bot_username:
                _hurl = "https://t.me/" + _bot_username + "?start=holders_" + contract
                top10_str = "[Top10: " + str(top10) + "%](" + _hurl + ")"
            else:
                top10_str = "Top10: " + str(top10) + "%"
        ...
        _risk_flags = [r for r in rug_risks if isinstance(r, str)][:2]
    ...
    safe_sym = _safe(str(info.get("symbol",""))).strip() or "SYM"
    safe_nm  = _safe(str(info.get("name",""))).strip()[:10] or "Token"
    name_raw = info.get("name", "")
    sym_raw  = info.get("symbol", "")
    combined = _urlparse.quote(f"({name_raw} OR ${sym_raw} OR {contract})")
    x_line = (
        "🔍 Search 𝕏:  "
        + "[All]("     + base_x.format(combined)                              + ")  "
        + "[CA]("      + base_x.format(_urlparse.quote(contract))             + ")  "
        + "[" + safe_nm + "](" + base_x.format(_urlparse.quote(name_raw))    + ")  "
        + "[$" + safe_sym + "](" + base_x.format(_urlparse.quote("$"+sym_raw)) + ")\n"
    )
    ...
    return (
        ...
        "├─ ⏰ Age *" + age_display + "*  B: *" + str(buys) + "* (" + str(buy_pct) + "%)\n"
        "└─ 🏆 ATH *" + ath_str + "*  S: *" + str(sells) + "* (" + str(sell_pct) + "%)\n"
        "─────────────────────────\n"
...
async def send_token_card(
    ...
    if is_query:
        ...
        _sent = False
        if header_url:
            # Truncate caption to 1024 chars if needed
            _caption = card_txt if len(card_txt) <= 1024 else card_txt[:1020] + "…"
            try:
                ...
                logger.warning(f"Banner send failed ({contract[:12]}): {_photo_err}")
        if not _sent:
            await ctx.bot.send_message(
                chat_id=chat_id,
                text=card_txt,
