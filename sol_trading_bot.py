...
MIN_BALANCE = 1.0
SNIPER_SEEN_EXPIRY_H = 24.0  # don't re-evaluate same token within 24h
                ...
                try:
                    ...
                    if ud.get("sniper_advisory"):
                        ...
                        if dm_notify:
                            ...
                            # Guard: only pill if we haven't already sent this contract to the user
                            _dm_cache = _sniper_analysis_cache.get(uid, {})
                            _already_dm = contract in _dm_cache
                            if not _already_dm and ai["verdict"] in ("SNIPE", "WAIT"):
                            ...
                            if ch_id:
                                # Guard: never broadcast the same contract twice to the same channel
                                _already_ch = _channel_calls.get(uid, {})
                                if contract in _already_ch:
                                    pass  # already called — skip silently
                                else:
