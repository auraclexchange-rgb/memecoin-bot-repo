...
# at 20 messages per user every 5 seconds (~200 tokens delivered in ~50 seconds).
# Structure: {uid: deque([{chat_id, text, disable_notification}, ...])}
import collections as _collections
_dl_queue: dict = {}          # uid -> deque of pending message dicts
DL_BATCH_SIZE  = 20           # messages to send per user per drain cycle
DL_DRAIN_EVERY = 5            # seconds between drain cycles
...
async def btn(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # ── APEX VAULT MENU ───────────────────────────────────────────────────────
        ...
        if not apex_on:
            ...
            # Auto-enable advisory so user gets token pings while APEX is running.
            # Advisory is independent of APEX entries — it shows every token that
            # passes pre-filters so the user can see what APEX is evaluating.
            if not ud.get("sniper_advisory"):
                ud["sniper_advisory"] = True
                ud["sniper_adv_notify"] = True
            save_user(u.id, ud)
            vault = ud.get("apex_vault", 0.0)
            split_pct = ud.get("apex_vault_profit_split", 0.50)
            msg = (
...
async def dump_log_drain_job(ctx: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    Prevents Telegram 429 rate-limit errors caused by blasting 200 channel messages at once.
    Flow: sniper_job pushes to _dl_queue → this job sends 20 at a time → done in ~50s.
    On repeated send failure, disables the channel and notifies the user.
    """
    if not _dl_queue:
        return
        ...
        while queue and sent < DL_BATCH_SIZE:
            ...
            try:
                await ctx.bot.send_message(
                    chat_id=msg["chat_id"],
                    text=msg["text"],
                    disable_notification=msg.get("disable_notification", True),
                )
                queue.popleft()   # success — remove from queue
                sent += 1
                consecutive_fails = 0
                if ud:
                ...
                consecutive_fails += 1
                logger.warning(f"Dump log drain failed (uid={uid}, ch={msg.get('chat_id')}): {_drain_err}")
                if consecutive_fails >= 3:
                    # Channel is broken — clear its queue and disable
                    queue.clear()
                    _dl_queue.pop(uid, None)
                    if ud:
                        ...
                        try:
                            await ctx.bot.send_message(
                                ...
                                text=(
                                    ...
                                    "The bot failed to post to your dump log channel 3 times in a row.\n\n"
                    ...
                    # Transient error — stop draining this user this cycle, retry next cycle
                    break
        if queue:
...
def main():
    ...
    for _uid, _ud in users.items():
        ...
        if _sf.get("max_mc", 0) > 100_000 and _sf.get("max_mc", 0) >= 500_000:
            _sf["max_mc"] = 100_000
            _changed = True
        # Enable advisory for existing APEX users who never had it on —
        # without it they get zero pings even when APEX is running.
        if _ud.get("apex_mode") and not _ud.get("sniper_advisory"):
            _ud["sniper_advisory"]  = True
            _ud["sniper_adv_notify"] = True
            _changed = True
        if _changed:
            save_user(_uid, _ud)
            _migrated_users += 1
    ...
    if _migrated_users:
        logger.info(f"Startup migration: upgraded {_migrated_users} users to min confidence=6, score=45, max_mc=100K + advisory enabled")
