...
_token_cache: dict = {}
CACHE_TTL = 12.0
_ch_card_cache: dict = {}  # {contract[:32]: card_data}
_ch_card_cache: dict = {}   # {contract: {ch_id, msg_id, info, sc, ai, expanded}}
    ...
    try:
        ...
        for acct in accounts:
            ...
            if mint == contract:
                ...
                # SOL price approximation — use stored SOL price or $140 default
                sol_price = 140.0
...
def get_user(uid: int, uname: str) -> dict:
    if uid not in users:
        users[uid] = {
            ...
            "apex_mode":              False,
            "apex_vault":             0.0,
            "vault_log":              [],   # [{ts, symbol, milestone, amount, contract}]
            ...
            "apex_total_trades":      0,
            "apex_total_wins":        0,
            "apex_learn_threshold":   5,
            "apex_learn_score_min":   45,
            ...
            "sniper_filters": {
                ...
                "max_mc":           200_000, # GemTools never called above $182K
                "max_age_h":        72,      # GemTools calls up to 2D 9H old tokens
                "buy_amount":       100,
                "min_buys_h1":      10,      # micro-caps have lower buy counts
                "min_buy_pct":      50,      # standard buy pressure
...
def sell_core(ud: dict, uid: int, contract: str, usd: float, price: float, reason: str = "manual") -> dict:
    ...
    # APEX vault cleanup: if position fully closed, clear its locked vault entry
    if closed:
        # Remove the vault_locked sub-amounts that belong to this position
        # (The vault balance itself remains — it's real profit already locked)
        # Just ensure the holding's apex_vault_locked doesn't ghost
        pass  # vault balance stays — it's already realised profit in the vault
    save_user(uid, ud)
    if closed:
...
def token_card(info: dict, contract: str, ud: dict, sc: dict = None) -> str:
    ...
    try:
        ...
        if contract in ud.get("holdings", {}):
            ...
            # ── History fields ──────────────────────────────────────────
            _peak    = h.get("peak_price", price)
            _low     = h.get("low_price",  price)
            _avg     = h.get("avg_price", price)
            _peak_x  = round(_peak / _avg, 2) if _avg > 0 else 0
            _low_x   = round(_low  / _avg, 2) if _avg > 0 else 0
            _bought  = h.get("bought_at")
            _held_m  = int((datetime.now() - _bought).total_seconds() / 60) if _bought else 0
            _held_s  = (str(_held_m) + "m") if _held_m < 60 else (str(round(_held_m/60, 1)) + "h")
            # Sparkline
            _ph      = h.get("price_history", [])
            _spark   = ""
            if len(_ph) >= 4:
                _step   = max(1, len(_ph) // 10)
                _samp   = [_ph[i]["price"] for i in range(0, len(_ph), _step)][-10:]
                _mn, _mx = min(_samp), max(_samp)
                _blks   = " ▁▂▃▄▅▆▇█"
                if _mx > _mn:
                    _spark = "`" + "".join(_blks[max(1, int((p-_mn)/(_mx-_mn)*8))] for p in _samp) + "`"
            # Post-buy snaps
            _snaps   = h.get("post_buy_snaps", {})
            _snap_ln = ""
            for _lbl in ("5m", "15m", "1h"):
                _s = _snaps.get(_lbl)
                if _s:
                    _xi = "🚀" if _s["x"]>=2 else "📈" if _s["x"]>=1.1 else "📉" if _s["x"]<0.9 else "➡️"
                    _snap_ln += " · " + _lbl + ":" + str(_s["x"]) + "x" + _xi
            # Threat badge
            _threat  = h.get("apex_threat", "")
            _tbadge  = {"RED":"🔴 RED","ORANGE":"🟠 ORANGE","YELLOW":"🟡 YELLOW","CLEAR":"🟢 CLEAR"}.get(_threat,"")
            pos_block = (
                "─────────────────────────\n"
                "> 💰 *YOUR POSITION*\n"
                ">\n"
                ...
                + "> ⏱ Held       " + _held_s + "\n"
                + "> 📈 Peak       *" + str(_peak_x) + "x*   📉 Low: *" + str(_low_x) + "x*\n"
                + ("> " + _spark + (_snap_ln if _snap_ln else "") + "\n" if _spark else "")
                + ("> 🎯 Threat     " + _tbadge + "\n" if _tbadge else "")
                + ">\n"
            )
...
def buy_kb(contract: str, ud: dict) -> InlineKeyboardMarkup:
    ...
    rows = [
        ...
        *([[InlineKeyboardButton("📜 Pos History",  callback_data="posh_" + contract),
            InlineKeyboardButton("🎯 Entry Signals",callback_data="apes_" + contract)]]
           if held else []),
...
async def run_checker(app: Application):
    ...
    for uid, ud in list(users.items()):
        ...
        for contract, h in list(ud["holdings"].items()):
            ...
            # ── Skip APEX-managed positions (trail/threat manages exits) ──────────
            if h.get("mood") in ("APEX", "AI-Sniper") and h.get("apex_trail_stop") is not None:
                continue  # APEX position manager runs in its own loop above
        ...
        for hca, h in list(ud.get("holdings", {}).items()):
            try:
                ...
                # Update peak + low price
                if cur_price > h.get("peak_price", 0):
                    h["peak_price"] = cur_price
                if cur_price < h.get("low_price", cur_price):
                    h["low_price"] = cur_price
                # Rolling price history snapshot
                _ph = h.setdefault("price_history", [])
                _now_ts = datetime.now().timestamp()
                _ph.append({"price": cur_price, "mc": hi.get("mc",0), "ts": _now_ts})
                if len(_ph) > 200:
                    h["price_history"] = _ph[-200:]
                # Post-buy snaps (5m/15m/1h)
                _snaps = h.setdefault("post_buy_snaps", {})
                _bt = h.get("bought_at")
                if _bt:
                    _ets = _bt.timestamp() if hasattr(_bt,"timestamp") else float(_bt)
                    _ep  = h.get("avg_price", cur_price)
                    _ela = _now_ts - _ets
                    for _lbl,_secs in (("5m",300),("15m",900),("1h",3600)):
                        if _lbl not in _snaps and _ela >= _secs:
                            _snaps[_lbl] = {"price":cur_price,"mc":hi.get("mc",0),
                                "x":round(cur_price/_ep,3) if _ep>0 else 0,"ts":_now_ts}
...
async def ai_analyze_token(info: dict, sc: dict, ud: dict, uid: int = 0) -> dict:
    ...
    # ── Verdict + Confidence ──────────────────────────────────────────────────
    if flag_count >= 2 or score < 45:
        verdict    = "SKIP"
        confidence = max(1, min(3, 10 - score // 10))
    elif score >= 68 and flag_count == 0:
        verdict    = "SNIPE"
        confidence = min(10, max(6, score // 10))
    elif score >= 58:
        verdict    = "WAIT"
        confidence = min(7, max(4, score // 12))
    else:
        verdict    = "SKIP"
...
def _compact_pill_text(info: dict, sc: dict, ai: dict) -> str:
    ...
    rug     = ai.get("rug_risk", "?")
    thesis  = (ai.get("thesis") or "")[:120]
    mc      = mc_str(info.get("mc", 0))
    liq     = money(info.get("liq", 0))
    age_h   = round(info.get("age_h", 0), 1)
    buy_pct = info.get("buy_pct_m5") or info.get("buy_pct", 50)
    v_e     = {"SNIPE":"🟢","WAIT":"🟡","SKIP":"🔴"}.get(verdict,"⚪")
    r_e     = {"LOW":"✅","MEDIUM":"⚠️","HIGH":"🚨"}.get(rug,"❓")
    flags   = sc.get("flags", [])
    flag_ln = ("\n🚩 " + "  ".join(f[:35] for f in flags[:2])) if flags else ""
    parts   = [
        v_e + " *$" + symbol + "* — " + verdict + "  |  " + chain,
        "🧠 Score: *" + str(score) + "/100*  Conf: *" + str(conf) + "/10*  " + r_e + " " + rug,
        "MC: *" + mc + "*  Liq: *" + liq + "*  Age: *" + str(age_h) + "h*  Buy%: *" + str(buy_pct) + "%*",
    ]
    if thesis:
        parts.append("📝 _" + thesis + "_")
    return "\n".join(parts) + flag_ln


def _ai_report_text(info: dict, sc: dict, ai: dict,
                    contract: str = "", expanded: bool = True) -> str:
    symbol   = info.get("symbol", "TOKEN")
    chain    = info.get("chain", "SOL").upper()
    mc       = mc_str(info.get("mc", 0))
    liq      = money(info.get("liq", 0))
    age_h    = round(info.get("age_h", 0), 1)
    score    = sc.get("score", 0)
    verdict  = ai.get("verdict", "SKIP")
    conf     = ai.get("confidence", 0)
    rug      = ai.get("rug_risk", "UNKNOWN")
    momentum = ai.get("momentum", "UNKNOWN")
    thesis   = (ai.get("thesis") or "No analysis.")[:300]
    buy_pct  = info.get("buy_pct_m5") or info.get("buy_pct", 50)
    v_e = {"SNIPE":"🟢","WAIT":"🟡","SKIP":"🔴"}.get(verdict,"⚪")
    r_e = {"LOW":"✅","MEDIUM":"⚠️","HIGH":"🚨"}.get(rug,"❓")
    m_e = {"STRONG":"🚀","MODERATE":"📈","WEAK":"📉","NEGATIVE":"💀"}.get(momentum,"➡️")
    flags = sc.get("flags",[])
    flag_txt = ("\n" + "\n".join("🚩 " + f[:60] for f in flags[:4])) if flags else ""
    strengths = sc.get("strengths",[])
    str_txt = ""
    if expanded and strengths:
        str_txt = "\n✅ " + "\n✅ ".join(s[:60] for s in strengths[:3])
    pf_curve = info.get("pf_curve")
    curve_txt = ("\n📊 Bonding curve: <b>" + str(round(pf_curve)) + "%</b>") if pf_curve is not None else ""
    header = (
        "<b>$" + symbol + "</b>  " + chain + "  |  " + v_e + " <b>" + verdict + "</b>\n"
        "🧠 Score: <b>" + str(score) + "/100</b>  Conf: <b>" + str(conf) + "/10</b>  " + r_e + " " + rug + "\n"
        "MC: <b>" + mc + "</b>  Liq: <b>" + liq + "</b>  Age: <b>" + str(age_h) + "h</b>\n"
        "Buy%: <b>" + str(buy_pct) + "%</b>  " + m_e + " " + momentum + curve_txt
    )
    body = ("\n\n📝 <i>" + thesis + "</i>" + flag_txt + str_txt) if expanded else ""
    return header + body
                    ...
                    if current_x >= milestone_x:
                        ...
                        try:
                            await ctx.bot.send_message(
                                ...
                                reply_markup=InlineKeyboardMarkup([
                                    [InlineKeyboardButton("⚡ Buy on Auracle", url=buy_url)],
                                ]),
                                disable_web_page_preview=True
                            )
        ...
        for item in unique_items:
            ...
            for uid, ud in active_users:
                try:
                    ...
                    # ── Enforce floor values — saved settings can't go below these ──
                    eff_min_score   = max(int(sf.get("min_score",   60)),  60)
                    eff_min_liq     = max(float(sf.get("min_liq",   15_000)), 15_000)
                    eff_min_mc      = max(float(sf.get("min_mc",    20_000)), 20_000)
                    eff_max_mc      = min(float(sf.get("max_mc", 3_000_000)), 3_000_000)
                    eff_max_age     = min(float(sf.get("max_age_h",    5.0)),  5.0)
                    eff_min_buys    = max(int(sf.get("min_buys_h1",    30)),   30)
                    eff_min_buy_pct = max(int(sf.get("min_buy_pct",    55)),   55)
                    eff_max_vol_mc  = min(float(sf.get("max_vol_mc_ratio", 5.0)), 5.0)
                    ...
                    # 9. Must have at least Twitter OR Telegram
                    elif not info.get("twitter") and not info.get("telegram"):
                        skip_reason = "No socials"

                    # 10. Too few unique holders
                    elif maker_count > 0 and maker_count < 25:
                    ...
                    _apex_eligible = (
                        ai["verdict"] == "SNIPE" or
                        (ai["verdict"] == "WAIT" and ai.get("confidence",0) >= 8
                         and sc.get("score",0) >= 68 and len(sc.get("flags",[])) == 0)
                    )
                    if ud.get("apex_mode") and _apex_eligible:
                        apex_reset_daily(ud)
                        _ok = True
                        ...
                        elif apex_count_positions(ud) >= APEX_MAX_POSITIONS:     _ok = False
                        elif ai.get("confidence",0) < ud.get("apex_learn_threshold", APEX_MIN_CONFIDENCE): _ok = False
                        elif sc.get("score",0) < ud.get("apex_learn_score_min", 60):                       _ok = False
                        if _ok:
                    ...
                    # ════════════════════════════════════════════════════
                    _ch_id = ud.get("sniper_broadcast_channel")
                    _qualifying = ai.get("verdict") in ("SNIPE","WAIT") and ai.get("confidence",0) >= 5
                    if _ch_id and _qualifying:
                        try:
                            await _broadcast_to_channel(ctx.bot, int(_ch_id), info, sc, ai, contract, uid=uid)
                        except Exception as _bce:
                            logger.error(f"Channel broadcast: {_bce}")
...
def apex_capital_heat(ud: dict) -> float:
    ...
    deployed = sum(h.get("total_invested", 0) for h in ud.get("holdings", {}).values())
...
def apex_position_size(ud: dict, ai_confidence: int, base_amount: float) -> float:
    ...
    return max(1.0, round(base_amount * conf_mult * heat_mult * dd_mult, 2))
...
def apex_check_threat(info: dict, h: dict) -> str:
    """
    Rug/dump threat detector — runs every 20s.
    Priority: price drop + liq drain (real-time).
    Buy% used only as secondary signal — too slow for sub-5min rugs.
    """
    price      = info.get("price", 0)
    avg        = h.get("avg_price", price)
    liq        = info.get("liq", 0)
    ...
    buy_pct_h1 = info.get("buy_pct_h1", info.get("buy_pct", 50))
    pct_drop   = (avg - price) / avg if avg > 0 else 0
    liq_drain  = (liq_at_buy - liq) / liq_at_buy if liq_at_buy > 0 else 0
    ...
    # Price alone — no buy% needed, rug is already happening
    if drop_pct <= -15:                        return "RED"  # -15% from entry = out
    # Liq drain alone — fastest rug signal
    if liq_drop >= 25:                         return "RED"  # 25% liq gone = rug
    # Price + any sell pressure
    if drop_pct <= -10 and buy_pct_m5 < 40:   return "RED"
    # Liq drain + sell pressure
    if liq_drop >= 15 and buy_pct_m5 < 40:    return "RED"
    # Sell panic spike — massive volume, overwhelmingly sells
    if avg_5m_vol > 0 and vol_m5 >= avg_5m_vol * 4 and buy_pct_m5 < 30: return "RED"
    # Near-total sell domination
    if buy_pct_m5 < 20:                        return "RED"

    # ══ ORANGE — TIGHTEN TRAIL STOP ══════════════════════════════════════════
    sigs = 0
    if drop_pct <= -8:                         sigs += 1
...
def apex_self_calibrate(ud: dict, uid: int) -> dict:
    ...
    score_low = [t.get("pnl", 0) for t in recent if t.get("score", 50) < 55]
    if len(score_low) >= 3 and sum(score_low) < 0 and cur_score < 55:
        ud["apex_learn_score_min"] = 55
        changes["apex_learn_score_min"] = {"old": cur_score, "new": 55,
            "reason": f"Low-score trades ({len(score_low)}) consistently losing"}
...
async def apex_run_position_manager(app, uid: int, ud: dict, positions_due: list = None) -> None:
    ...
    async with _get_user_lock(uid):
     for contract, h in list(ud.get("holdings", {}).items()):
        if h.get("mood") not in ("APEX", "AI-Sniper"):
            continue
        ...
        try:
            ...
            # Record threat transitions in history
            _prev_threat = h.get("apex_threat", "CLEAR")
            if threat != _prev_threat:
                _th = h.setdefault("threat_history", [])
                _th.append({"threat": threat, "prev": _prev_threat,
                            "price": price, "cx": round(price/max(h.get("avg_price",price),1e-12),3),
                            "ts": datetime.now().isoformat()})
                if len(_th) > 50: h["threat_history"] = _th[-50:]
            ...
            if cx >= 5.0 and "5x" not in vault_locks:
                ...
                if lock_amt > 0:
                    ud["apex_vault"] = ud.get("apex_vault", 0.0) + lock_amt
                    vault_locks["5x"] = lock_amt
                    ud.setdefault("vault_log", []).append({
                        "ts": datetime.now().isoformat(), "symbol": h.get("symbol","?"),
                        "contract": contract, "milestone": "5x", "amount": lock_amt,
                    })
                    try:
                        await app.bot.send_message(chat_id=uid, parse_mode="Markdown",
                            text=("\U0001f3e6 *APEX \u2014 PROFIT LOCKED (5x)*\n\n"
                                  "*$" + h["symbol"] + "* hit *5x*!\n"
                                  "Locked: *" + money(lock_amt) + "* \u2192 Vault\n"
                                  "\U0001f512 Vault total: *" + money(ud["apex_vault"]) + "*\n"
                                  "Trail continues\u2026"),
                            reply_markup=main_menu_kb())
                    except Exception:
                        pass
                ...
                if lock_amt > 0:
                    ...
                    ud.setdefault("vault_log", []).append({
                        "ts": datetime.now().isoformat(), "symbol": h.get("symbol","?"),
                        "contract": contract, "milestone": "2x", "amount": lock_amt,
                    })
                    try:
                        await app.bot.send_message(chat_id=uid, parse_mode="Markdown",
                            text=("\U0001f3e6 *APEX \u2014 PROFIT LOCKED (2x)*\n\n"
                                  "*$" + h["symbol"] + "* hit *2x*!\n"
                                  "Locked: *" + money(lock_amt) + "* \u2192 Vault\n"
                                  "\U0001f512 Vault total: *" + money(ud["apex_vault"]) + "*\n"
...
async def apex_process_entry_queue(app, uid: int, ud: dict) -> None:
    ...
    async with _get_user_lock(uid):
      for contract, entry in list(queue.items()):
        ...
        if buy_amt < 1.0:
            continue
        if apex_count_positions(ud) >= APEX_MAX_POSITIONS:
            continue
        ...
        if not isinstance(result, tuple):
            ...
            continue
        info_post, _ = result
        # Mark as seen so sniper_job doesn't re-scan this contract
        ud.setdefault("sniper_seen", {})[contract] = __import__("time").time()
        h = ud["holdings"].get(contract, {})
        if not h:
            continue
        ...
        h["apex_entry_signals"] = {
            "thesis":      ai.get("thesis", ""),
            "strengths":   sc.get("strengths", [])[:3],
            "warnings":    sc.get("warnings",  [])[:2],
            "flags":       sc.get("flags",     []),
            "rug_risk":    ai.get("rug_risk",  "UNKNOWN"),
            "momentum":    ai.get("momentum",  "UNKNOWN"),
            "dev_risk":    info.get("dev_risk", "UNKNOWN"),
            "vol_organic": info.get("vol_organic_score", -1),
            "pf_curve":    info_post.get("pf_curve"),
            "entry_mc":    info_post.get("mc", 0),
            "entry_liq":   info_post.get("liq", 0),
        }
        h["threat_history"] = []   # [{threat, ts, price, cx}]
        # Tighter stop loss — rugs move fast
        _rug = ai.get("rug_risk", "LOW")
        h["stop_loss_pct"] = 12.0 if _rug == "HIGH" else (18.0 if _rug == "MEDIUM" else 22.0)
        ...
        try:
            await app.bot.send_message(chat_id=uid, parse_mode="Markdown",
                text=("\U0001f3af *APEX ENTRY CONFIRMED*\n\n"
                      ...
                      "Heat: *" + str(heat) + "%*  |  Positions: *" + str(apex_count_positions(ud)) + "/" + str(APEX_MAX_POSITIONS) + "*\n"
...
APEX_DCA_MIN_DIP        = 0.15
APEX_DCA_MIN_CX         = 0.75
APEX_DCA_MAX_CX         = 1.30
APEX_DCA_BUY_PCT_FLOOR  = 45.0
APEX_DCA_BUY_PCT_RECOV  = 52.0
APEX_DCA_MAX_HEAT       = 0.65
APEX_DCA_SIZE_MULT      = 0.50
APEX_DCA_MAX_PER_POS    = 1
...
async def do_buy_core(ud: dict, uid: int, contract: str, usd_amount: float, planned: bool = True, mood: str = "") -> str | tuple:
    ...
    if contract in ud["holdings"]:
        ...
        ud["holdings"][contract] = {
            ...
            "price_history":  [{               # rolling price snapshots for sparkline
                "price": info["price"],
                "mc":    info.get("mc", 0),
                "ts":    datetime.now().timestamp(),
            }],
            "low_price":      info["price"],   # lowest price seen since entry
        }
    if planned:
        ud["planned"] += 1
    else:
...
async def btn(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if cb == "mm":
        pending.pop(u.id, None)
        # Delete any lingering chart when returning to main menu
        _prev_chart = chart_msg_ids.pop(u.id, None)
        if _prev_chart:
            try:
                await ctx.bot.delete_message(chat_id=u.id, message_id=_prev_chart)
            except Exception:
                pass
        if ud.get("balance") is None:
        ...
        # Clean up chart on navigation
        _cid = chart_msg_ids.pop(u.id, None)
        if _cid:
            try: await ctx.bot.delete_message(chat_id=u.id, message_id=_cid)
        ...
        for contract, info in zip(_pos_contracts, _pos_infos):
            ...
            if info:
                ...
                as_txt = "  🎯" + str(len(targets)) + " targets" if targets else ""
                # Peak / low since entry
                peak   = h.get("peak_price", price)
                low    = h.get("low_price",  price)
                peak_x = round(peak / avg, 2) if avg > 0 else 0
                low_x  = round(low  / avg, 2) if avg > 0 else 0
                # Sparkline from price_history
                ph     = h.get("price_history", [])
                spark  = ""
                if len(ph) >= 4:
                    # Sample up to 8 evenly spaced points
                    step   = max(1, len(ph) // 8)
                    sample = [ph[i]["price"] for i in range(0, len(ph), step)][-8:]
                    mn, mx = min(sample), max(sample)
                    blocks = " ▁▂▃▄▅▆▇█"
                    if mx > mn:
                        spark = "`" + "".join(
                            blocks[max(1, int((p - mn) / (mx - mn) * 8))]
                            for p in sample
                        ) + "`"
                    else:
                        spark = "`▄▄▄▄▄▄▄▄`"
                ...
                held_txt  = ""
                if bought_at:
                    held_m = int((datetime.now() - bought_at).total_seconds() / 60)
                    held_txt = str(held_m) + "m" if held_m < 60 else str(round(held_m/60,1)) + "h"
                # Threat badge for APEX positions
                threat = h.get("apex_threat", "")
                threat_badge = {"RED":"🔴","ORANGE":"🟠","YELLOW":"🟡","CLEAR":"🟢"}.get(threat, "")
                mood_badge = "⚡" if h.get("mood") in ("APEX","AI-Sniper") else ""
                lines.append(
                    mood_badge + threat_badge + " *$" + h["symbol"] + "*  " + str(round(cx, 2)) + "x"
                    + ("  " + held_txt if held_txt else "") + "\n"
                    + "  " + money(cv) + "  " + pstr(ppnl) + sl_txt + as_txt + "\n"
                    + "  Peak: *" + str(peak_x) + "x*  Low: *" + str(low_x) + "x*\n"
                    + ("  " + spark + "\n" if spark else "")
                )
        buttons = []
    ...
    # ── COMPETITION BETS UPDATE ────────────────────────────────────────────
    ...
    elif cb.startswith("posh_"):
        # Position history screen
        contract = cb[5:]
        ...
        low     = h.get("low_price",  avg)
        peak_x  = round(peak / avg, 2) if avg > 0 else 0
        low_x   = round(low  / avg, 2) if avg > 0 else 0
        bought  = h.get("bought_at")
        held_m  = int((datetime.now() - bought).total_seconds() / 60) if bought else 0
        held_txt= str(held_m) + "m" if held_m < 60 else str(round(held_m/60,1)) + "h"
        # Sparkline
        ph     = h.get("price_history", [])
        spark  = "No price data yet — check back after 1 cycle"
        if len(ph) >= 4:
            step   = max(1, len(ph) // 12)
            sample = [ph[i]["price"] for i in range(0, len(ph), step)][-12:]
            mn, mx = min(sample), max(sample)
            blocks = " ▁▂▃▄▅▆▇█"
            if mx > mn:
                spark = "`" + "".join(
                    blocks[max(1, int((p - mn) / (mx - mn) * 8))] for p in sample
                ) + "`"
            else:
                spark = "`▄▄▄▄▄▄▄▄▄▄▄▄` (flat)"
        # Post-buy time snapshots (5m / 15m / 1h)
        snaps    = h.get("post_buy_snaps", {})
        snap_txt = ""
        if snaps:
            snap_txt = "\n⏱ *Price After Entry*\n"
            for _lbl in ("5m", "15m", "1h"):
                ...
                if _s:
                    _xi = "🚀" if _s["x"]>=2 else "📈" if _s["x"]>=1.1 else "📉" if _s["x"]<0.9 else "➡️"
                    snap_txt += "  " + _lbl + ": *" + str(_s["x"]) + "x* " + _xi + "\n"
                else:
                    snap_txt += "  " + _lbl + ": pending...\n"
        # DCA history
        dca_lines = ""
        for d in h.get("apex_dca_history", []):
            dca_lines += "  DCA at *" + str(round(d.get("cx",0),2)) + "x* — " + money(d.get("amount",0)) + "\n"
        # Threat history
        threat_lines = ""
        for t in h.get("threat_history", [])[-5:]:
            ts_str = t.get("ts","")[:16].replace("T"," ")
            threat_lines += "  " + t.get("prev","?") + " → *" + t.get("threat","?") + "* at " + str(t.get("cx",0)) + "x  _" + ts_str + "_\n"
        txt = (
            "📜 *POSITION HISTORY — $" + h["symbol"] + "*\n\n"
            "⏱ Held: *" + held_txt + "*\n"
            "📈 Peak: *" + str(peak_x) + "x*\n"
            "📉 Low:  *" + str(low_x)  + "x*\n"
            "💸 Invested: *" + money(h.get("total_invested",0)) + "*\n\n"
            ...
            + "\n"
            + ("💉 *DCA History*\n" + dca_lines + "\n" if dca_lines else "")
            + ("⚠️ *Threat Transitions*\n" + threat_lines if threat_lines else "✅ No threat signals recorded")
        )
        await q.edit_message_text(txt, parse_mode="Markdown",
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("◀ Back", callback_data="btt_" + contract)]]))

    elif cb.startswith("apes_"):
        # APEX entry signals screen
        contract = cb[5:]
        h = ud.get("holdings", {}).get(contract)
        if not h:
            await q.edit_message_text("Position not found.", reply_markup=back_main())
            return
        sig = h.get("apex_entry_signals", {})
        if not sig:
            await q.edit_message_text(
                "No APEX entry signals recorded.\n(Only available for APEX/AI-Sniper entries)",
                reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("◀ Back", callback_data="btt_" + contract)]]))
            return
        strengths  = "\n".join("  ✅ " + s for s in sig.get("strengths", [])) or "  —"
        warnings   = "\n".join("  ⚠️ " + w for w in sig.get("warnings",  [])) or "  —"
        flags      = "\n".join("  🚨 " + f for f in sig.get("flags",     [])) or "  None"
        txt = (
            "🎯 *APEX ENTRY SIGNALS — $" + h["symbol"] + "*\n\n"
            "📝 *Thesis*\n  _" + sig.get("thesis","—") + "_\n\n"
            "🟢 *Strengths*\n" + strengths + "\n\n"
            "🟡 *Warnings*\n"  + warnings  + "\n\n"
            "🔴 *Flags*\n"     + flags     + "\n\n"
            "📊 *At Entry*\n"
            "  Score: *" + str(h.get("apex_entry_score",0)) + "/100*\n"
            "  Confidence: *" + str(h.get("apex_entry_conf",0)) + "/10*\n"
            "  Rug Risk: *" + sig.get("rug_risk","?") + "*\n"
            "  Dev Risk: *" + sig.get("dev_risk","?") + "*\n"
            "  Vol Organic: *" + (str(sig.get("vol_organic",-1)) + "/10" if sig.get("vol_organic",-1) >= 0 else "N/A") + "*\n"
            "  Momentum: *"  + sig.get("momentum","?") + "*\n"
            "  MC at entry: *" + mc_str(sig.get("entry_mc",0)) + "*\n"
            "  Liq at entry: *" + money(sig.get("entry_liq",0)) + "*\n"
            "  Stop Loss: *" + str(h.get("stop_loss_pct","?")) + "%*"
        )
        await q.edit_message_text(txt, parse_mode="Markdown",
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("◀ Back", callback_data="btt_" + contract)]]))
    ...
    # ── SNIPER MODE v2 ─────────────────────────────────────────────────────────
        ...
        parts    = []
        if apex_on:  parts.append("⚡ APEX")
        if auto_on and not apex_on: parts.append("🟢 AUTO")
        if adv_on:   parts.append("🧠 ADVISORY")
        status   = " + ".join(parts) if parts else "🔴 OFF"
        budget   = ud.get("sniper_daily_budget", 500.0)
        spent    = ud.get("sniper_daily_spent", 0.0)
        sf       = ud.get("sniper_filters", {})
        chains   = ud.get("sniper_chains", {})
        ...
        await q.edit_message_text(
            ...
            "🔧 *Filters:*\n"
            "  Min Score: *"    + str(sf.get("min_score",45))          + "/100*\n"
            "  Min Liq: *"      + money(sf.get("min_liq",15_000))       + "*\n"
            "  MC Range: *"     + mc_str(sf.get("min_mc",30_000))       + "* → *" + mc_str(sf.get("max_mc",5_000_000)) + "*\n"
            "  Max Age: *"      + str(sf.get("max_age_h",6))            + "h* (fresh launches only)\n"
            "  Min Buys/1h: *"  + str(sf.get("min_buys_h1",30))        + "* (spread volume)\n"
            "  Min Buy%: *"     + str(sf.get("min_buy_pct",52))         + "%* (buy pressure)\n"
            "  Vol/MC Cap: *"   + str(sf.get("max_vol_mc_ratio",8.0))   + "x* (wash trading filter)\n"
            "  Buy Amount: *"   + money(sf.get("buy_amount",100))       + "*",
            parse_mode="Markdown",
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("⚡ APEX Mode",         callback_data="apex_menu")],
        ...
        await q.edit_message_text(
            ...
            "Min Liq: *"      + money(sf.get("min_liq",       15_000))   + "*\n"
            "MC Range: *"     + mc_str(sf.get("min_mc",        30_000))   + "* → *" + mc_str(sf.get("max_mc", 5_000_000)) + "*\n"
            "Max Age: *"      + str(sf.get("max_age_h",  6))              + "h* (fresh only)\n"
            "Min Buys/1h: *"  + str(sf.get("min_buys_h1", 30))           + "*\n"
            "Min Buy%: *"     + str(sf.get("min_buy_pct",   52))          + "%*\n"
            "Vol/MC Cap: *"   + str(sf.get("max_vol_mc_ratio", 8.0))      + "x*\n"
            "Buy Amount: *"   + money(sf.get("buy_amount",100))           + "*",
            parse_mode="Markdown",
            reply_markup=InlineKeyboardMarkup([
    ...
    # ── APEX VAULT MENU ───────────────────────────────────────────────────────
        ...
        await q.edit_message_text(
            "🏦 *APEX PROFIT VAULT*\n\n"
            "Your locked profits — protected from trading.\n\n"
            "💰 Vault Balance: *" + money(vault) + "*\n"
            "   ↳ " + money(total_locked) + " locked from open positions\n"
            "💵 Trading Balance: *" + money(balance) + "*\n"
            "📊 Total Portfolio: *" + money(vault + balance) + "*\n\n"
            "Vault holds profits locked at 2x/5x.\n"
            "Open position locks are already included in vault balance.",
            parse_mode="Markdown",
        ...
        vault_log = ud.get("vault_log", [])
        # Combine log entries with open-position status
        open_locks = {}
        for h in ud.get("holdings", {}).values():
            for ms, amt in h.get("apex_vault_locked", {}).items():
                open_locks[h.get("symbol","?")+":"+ms] = amt
        if not vault_log and not open_locks:
            await q.edit_message_text(
                "🏦 *VAULT HISTORY*\n\nNo vault locks yet.\n\n"
                "Profits lock at 2x and 5x milestones in APEX mode.",
                parse_mode="Markdown",
                reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("⬅️ Back", callback_data="apex_vault_menu")]])
            )
            return
        total_locked_all = sum(e.get("amount",0) for e in vault_log)
        lines_out = [
            "🏦 *VAULT HISTORY*",
            "💰 Current vault: *" + money(ud.get("apex_vault",0)) + "*",
            "📊 Total ever locked: *" + money(total_locked_all) + "*\n",
        ]
        if open_locks:
            lines_out.append("*Currently locked (open positions):*")
            for key, amt in open_locks.items():
                sym, ms = key.rsplit(":",1)
                lines_out.append("  🔒 $" + sym + " " + ms + " → " + money(amt))
            lines_out.append("")
        if vault_log:
            lines_out.append("*Lock history (last 10):*")
            for e in vault_log[-10:][::-1]:
                ts_str = e.get("ts","")[:16].replace("T"," ")
                lines_out.append(
                    "  🔒 $" + e.get("symbol","?") + " " + e.get("milestone","?") + "x"
                    " → *" + money(e.get("amount",0)) + "*  _" + ts_str + "_"
                )
        await q.edit_message_text(
            "\n".join(lines_out),
            parse_mode="Markdown",
    ...
    # ── APEX MODE MENU ────────────────────────────────────────────────────────
        ...
        text = (
            ...
            "  Open positions: *" + str(positions) + "/" + str(APEX_MAX_POSITIONS) + "*\n"
        ...
        exit_counts: dict = {}
        for t in apex_logs:
            r = t.get("apex_exit_reason", "manual")
        ...
        text = (
            ...
            "Min confidence: *" + str(conf_thr) + "/10*\n"
            "Min score: *" + str(score_min) + "/100*\n"
            "Max open positions: *" + str(APEX_MAX_POSITIONS) + "*\n"
            "Daily loss halt: *20%*\n"
...
async def hourly_balance_job(ctx: ContextTypes.DEFAULT_TYPE) -> None:
    import time as _t
    now_ts = _t.time()
    for uid, ud in list(users.items()):
        if ud.get("balance") is None:
            continue
        bal = ud.get("balance", 0)
        dep = sum(h.get("total_invested", 0) for h in ud.get("holdings", {}).values())
        bh  = ud.setdefault("balance_history", [])
        bh.append({"ts": now_ts, "balance": round(bal,2), "deployed": round(dep,2),
                   "total": round(bal+dep,2), "pnl": round(ud.get("apex_daily_pnl",0),2)})
        if len(bh) > 720:
            ud["balance_history"] = bh[-720:]


async def sniper_followup_job(ctx: ContextTypes.DEFAULT_TYPE) -> None:
    import time as _t
    now = _t.time()
    for uid, entries in list(_sniper_followup.items()):
        ud = users.get(uid)
        if not ud:
            continue
        remaining = []
        for entry in entries:
            elapsed = now - entry.get("queued_at", now)
            if 3600 <= elapsed <= 4200:
                try:
                    info = await get_token(entry["contract"])
                    log  = ud.get("sniper_log", [])
                    li   = entry.get("log_idx", -1)
                    if info and 0 <= li < len(log):
                        ep = entry.get("entry_price", 0)
                        cp = info.get("price", 0)
                        log[li]["followup_1h"] = {
                            "price": cp, "mc": info.get("mc",0), "liq": info.get("liq",0),
                            "x": round(cp/ep,3) if ep > 0 else 0,
                            "ts": datetime.now().isoformat(),
                        }
                except Exception as _fe:
                    logger.debug(f"Followup: {_fe}")
            elif elapsed < 3600:
                remaining.append(entry)
        if remaining:
            _sniper_followup[uid] = remaining
        else:
            _sniper_followup.pop(uid, None)


async def autosave_job(ctx: ContextTypes.DEFAULT_TYPE) -> None:
    for uid, ud in list(users.items()):
        try:
            save_user(uid, ud)
        except Exception as _se:
            logger.error(f"Autosave {uid}: {_se}")
    ...
    try:
        for uid, ud in list(users.items()):
            ...
            apex_holdings = {
                ...
                if h.get("mood") in ("APEX", "AI-Sniper") and h.get("amount", 0) > 0
            }
...
def main():
    ...
    app.job_queue.run_repeating(hourly_balance_job,  interval=3600, first=60)
    app.job_queue.run_repeating(sniper_followup_job, interval=300,  first=120)
