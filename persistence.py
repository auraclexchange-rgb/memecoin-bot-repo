...
# Persistent sync HTTP client — reuses TCP connections across saves instead of
# opening a new connection on every _sb_upsert call.  Timeout mirrors the per-
# call value already in use.  Created lazily so module import never blocks.
_sync_client: "httpx.Client | None" = None

def _get_sync_client() -> "httpx.Client":
    global _sync_client
    if _sync_client is None or _sync_client.is_closed:
        _sync_client = httpx.Client(timeout=10, limits=httpx.Limits(max_connections=10))
    return _sync_client
...
async def autosave_job(context):
    ...
    for uid, ud in list(_bot.users.items()):
        try:
            ...
            # Persist pause timer so consecutive-loss pause survives bot restarts
            if uid in _bot._apex_paused_until:
                ud["_persisted_paused_until"] = _bot._apex_paused_until[uid]
            elif "_persisted_paused_until" in ud:
                del ud["_persisted_paused_until"]
            # Persist competitions — store on first user encountered (creator)
            # Each competition is saved on its creator's ud to survive restarts
            my_comp_codes = list(ud.get("competitions", {}).keys())
            for _cc2 in my_comp_codes:
                if _cc2 in _bot._competitions:
                    ud.setdefault("_persisted_competitions", {})[_cc2] = _bot._competitions[_cc2]
            save_user(uid, ud)
