...
def generate_trade_card(symbol: str, chain: str, pnl_str: str, x_val: str, held_h: str, bought_str: str, position_str: str, username: str, pnl_pct: str, pnl_positive: bool) -> "io.BytesIO | None":
    ...
    try:
        ...
        try:
            font_token = ImageFont.truetype("/usr/share/fonts/truetype/dejavu/DejaVuSans-Bold.ttf", 86)
            font_pill  = ImageFont.truetype("/usr/share/fonts/truetype/dejavu/DejaVuSans-Bold.ttf", 66)
            ...
            font_user  = ImageFont.truetype("/usr/share/fonts/truetype/dejavu/DejaVuSans-Bold.ttf", 28)
            font_tiny  = ImageFont.truetype("/usr/share/fonts/truetype/dejavu/DejaVuSans.ttf", 19)
        except Exception:
            font_token = font_pill = font_label = font_value = font_brand = font_user = font_tiny = ImageFont.load_default()
        for y in range(H):
            t = y / H
            draw.line([(0, y), (W, y)], fill=(int(8+t*4), int(10+t*6), int(18+t*14)))
        glow_col = (0, 55, 25) if pnl_positive else (55, 8, 8)
        for i in range(100, 0, -1):
            draw.rectangle([0, 0, i*2, H], fill=glow_col)
        char_path = "/app/win_char.jpg" if pnl_positive else "/app/loss_char.jpg"
        if not os.path.exists(char_path):
            char_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "win_char.jpg" if pnl_positive else "loss_char.jpg")
        if os.path.exists(char_path):
            ...
            char_x = W - char_w + 60
            img.paste(char, (char_x, -20), char)
            overlay = Image.new("RGBA", (W, H), (0, 0, 0, 0))
            ov_draw = ImageDraw.Draw(overlay)
            fade_start = W - char_w + 60
            fade_end = fade_start + 220
            ...
            draw = ImageDraw.Draw(img)
        draw.polygon([(38, 76), (78, 16), (118, 76)], outline=(210, 220, 240), width=3)
        draw.text((W - 40, 26), "AURACLE_XBOT", font=font_brand, fill=(180, 195, 225), anchor="ra")
        draw.text((38, 96), "$" + symbol, font=font_token, fill=(240, 245, 255))
        pill_col = (0, 195, 105) if pnl_positive else (200, 40, 40)
        txt_col  = (5, 15, 10)   if pnl_positive else (255, 235, 235)
        prefix   = "+"           if pnl_positive else "-"
        draw.rounded_rectangle([38, 205, 580, 305], radius=18, fill=pill_col)
        draw.text((65, 255), "◎ " + prefix + pnl_str, font=font_pill, fill=txt_col, anchor="lm")
        pnl_col = (0, 220, 120) if pnl_positive else (220, 75, 75)
        stats = [("PNL", prefix + pnl_pct, pnl_col), ("Bought", "◎ " + bought_str, (195, 210, 235)), ("Position", "◎ " + position_str, (195, 210, 235)), ("Held", held_h, (195, 210, 235))]
        ...
        draw.line([(38, H - 68), (620, H - 68)], fill=(35, 45, 68), width=1)
        ax, ay = 55, H - 34
        ...
        draw.text((620, H-34), "AURACLE_XBOT  |  Paper Trading", font=font_tiny, fill=(60, 75, 100), anchor="ra")
        buf = io.BytesIO()
...
def main_menu_kb() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("🏅 Rewards",        callback_data="v_rewards"),
         InlineKeyboardButton("🏁 Compete",        callback_data="v_compete")],
        [InlineKeyboardButton("🔁 Copy Trading",   callback_data="v_copy"),
         InlineKeyboardButton("🌐 Public Profile", callback_data="v_public")],
...
async def text_handler(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if p:
        ...
        if action == "set_balance":
            ...
            if not hasattr(btn, "_competitions") or code not in btn._competitions:
                pending.pop(u.id, None)
                await update.message.reply_text("Competition not found. Check the code and try again.", reply_markup=back_main())
                return
            comp = btn._competitions[code]
            if datetime.now() > comp["end_time"]:
                pending.pop(u.id, None)
                await update.message.reply_text("This competition has ended.", reply_markup=back_main())
                return
            comp["members"][u.id] = {"username": ud["username"], "joined_balance": ud["balance"]}
            ud["competitions"][code] = comp
            pending.pop(u.id, None)
            await update.message.reply_text(
                "🏁 Joined competition *" + code + "*!\n"
                "Creator: " + comp["creator"] + "\n"
                "Ends: " + comp["end_time"].strftime("%b %d %Y") + "\n"
                "Members: " + str(len(comp["members"])),
                parse_mode="Markdown", reply_markup=back_main()
            )
            return
