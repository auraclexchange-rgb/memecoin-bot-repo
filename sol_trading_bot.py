...
IMPORTANT CONTEXT: These are memecoins. Do NOT penalise for lacking utility, whitepaper, or roadmap — that is irrelevant. Focus on: momentum, community speed, narrative strength, and rug risk signals. For metadata analysis, look for: copy-paste rug templates, identity mismatches, pressure scam language, suspicious Twitter accounts created same day as token, and extreme wallet concentration. A token with a blank description but viral narrative and healthy distribution is far better than one with a long description full of template phrases.
...
META INTELLIGENCE (Description + Twitter + Holder Distribution):
{_build_meta_prompt_block(info)}
...
def _mtv_block(info: dict) -> str:
    """Compact M·T·V block — single line per timeframe, matches v2 card design."""
    def _dot(pct):
        return "🟢" if pct >= 60 else ("🟡" if pct >= 52 else "🔴")
    ...
    lines = ["📊 <b>M · T · V Intelligence</b>"]
    for label, txns, pct, vol in rows:
        dot = _dot(pct)
        lines.append(f"<code>{label:<3}</code>  T: <b>{txns}</b> {dot}{pct:.0f}%   V: <b>{_vol(vol)}</b> {dot}{pct:.0f}%")
    return "\n".join(lines) + "\n"


def _ai_report_text(info: dict, sc: dict, ai: dict, contract: str = "", expanded: bool = False) -> str:
    """Build channel card v2.  expanded=False → AI section hidden (button reveals it)."""
    import re as _re2
    ...
    social_score   = ai.get("social_score", "UNKNOWN")

    verdict_icon = {"SNIPE": "🟢", "SKIP": "🔴", "WAIT": "🟡"}.get(verdict, "⚪")
    rug_icon     = {"LOW": "✅", "MEDIUM": "⚠️", "HIGH": "🚨"}.get(rug_risk, "❓")
    mom_icon     = {"STRONG": "🚀", "MODERATE": "📈", "WEAK": "📉", "NEGATIVE": "💀"}.get(momentum, "❓")
    soc_icon     = {"GOOD": "✅", "PARTIAL": "⚠️", "NONE": "🚨"}.get(social_score, "❓")

    symbol  = info.get("symbol", "???")
    chain   = info.get("chain", "?").upper()[:3]
    mc      = info.get("mc", 0)
    liq     = info.get("liq", 0)
    liq_pct = info.get("liq_pct", 0)
    ...
    score_tag = "🔴 WEAK" if score < 40 else ("🟡 MOD" if score < 65 else "🟢 STRONG")

    # Safety
    lp_burn   = info.get("lp_burn")
    no_mint   = info.get("no_mint")
    no_freeze = info.get("no_freeze")
    lp_str    = ("LP: <b>" + str(lp_burn) + "%</b> 🔥") if lp_burn is not None else "LP: —"
    mint_str  = ("✅ No Mint" if no_mint else "🚨 Mint!") if no_mint is not None else "Mint: —"
    frz_str   = ("✅ No Freeze" if no_freeze else "⚠️ Freeze") if no_freeze is not None else ""
    ...
    if pf_curve is not None:
        grad_str = " 🎓 <b>GRADUATED</b>" if pf_graduated else f" Curve: <b>{pf_curve}%</b>"
        rep_str  = f"  💬 {pf_replies}" if pf_replies > 0 else ""
    ...
    insider = info.get("insider_pct")
    dev_pct = info.get("pf_dev_pct") or info.get("dev_pct_rc")
    maker_n = info.get("maker_count")

    def _pf(v, warn=20, danger=35):
        if v is None: return "—"
        v = float(v)
        c = "🔴" if v >= danger else ("🟡" if v >= warn else "🟢")
        return f"{c} <b>{v}%</b>"

    holder_parts = []
    if maker_n is not None: holder_parts.append(f"👨‍👩‍👧 Holders: <b>{maker_n}</b>")
    if top10   is not None: holder_parts.append(f"Top10: {_pf(top10, 25, 40)}")
    if top20   is not None: holder_parts.append(f"Top20: {_pf(top20, 35, 55)}")

    dev_parts = []
    if insider is not None: dev_parts.append(f"🐀 Insiders: {_pf(insider, 5, 15)}")
    if dev_pct is not None: dev_parts.append(f"👤 DEV: <b>{dev_pct}%</b>")
    dex_paid_icon = "✅" if boost >= 1 else "❌"
    dev_parts.append(f"⚡ DEX PAID: {dex_paid_icon}")

    holder_block = ""
    if holder_parts or dev_parts:
        holder_block = "📊 <b>Holder Intel</b>\n"
        if holder_parts: holder_block += "  ".join(holder_parts) + "\n"
        if dev_parts:    holder_block += "  ·  ".join(dev_parts) + "\n"

    # MTV
    mtv = _mtv_block(info)

    # ── AI section (only when expanded) ──────────────────────────────────────
    ai_section = ""
    if expanded:
        ...
        thesis      = ai.get("thesis", "No analysis available.")

        str_lines  = "\n".join("  ✅ " + s for s in strengths[:3])
        warn_lines = "\n".join("  ⚠️ " + w for w in warnings[:3])
        hard_lines = "\n".join("  🚫 " + f for f in hard_flags[:3])
        red_lines  = "\n".join("  🚨 " + f for f in red_flags[:4])  or "  None"
        grn_lines  = "\n".join("  ✅ " + f for f in green_flags[:4]) or "  None"

        ai_section = (
            "\n━━━━━━━━━━━━━━━━━━\n"
            "🧠 <b>AI ANALYSIS</b>\n\n"
            + f"{verdict_icon} <b>VERDICT: {verdict}</b>   Confidence: <b>{confidence}/10</b>\n"
            ...
            + f"🚩 <b>Red Flags:</b>\n{red_lines}\n\n"
            + f"💚 <b>Green Flags:</b>\n{grn_lines}\n\n"
            + f"{rug_icon} Rug Risk: <b>{rug_risk}</b>   "
            + f"{mom_icon} Momentum: <b>{momentum}</b>   "
            + f"{soc_icon} Socials: <b>{social_score}</b>\n\n"
            + f"💸 <b>Suggested Entry: {money(ai.get('suggested_amount', 0))}</b>"
        )

    return (
        "🤖 <b>AI SNIPER ADVISORY</b>\n"
        "━━━━━━━━━━━━━━━━━━\n"
        "🪙 <b>$" + symbol + "</b>  ·  " + chain + "\n"
        + (f"<code>{contract}</code>\n" if contract else "")
        + "\n"
        + f"MC: <b>{mc_str(mc)}</b>   Age: <b>{age_h}h</b>\n"
        + f"Liq: <b>{money(liq)}</b> ({liq_pct}%)   "
        + f"1h: <b>{('+' if ch_h1 >= 0 else '') + str(round(ch_h1, 1))}%</b>   "
        + f"24h: <b>{('+' if ch_h24 >= 0 else '') + str(round(ch_h24, 1))}%</b>\n"
        + lp_str + "   " + mint_str + ("   " + frz_str if frz_str else "") + "\n"
        + pf_line
        + "\n"
        + holder_block
        + "\n"
        + "🧠 <b>Score: " + str(score) + "/100</b>  <code>" + score_bar + "</code>  " + score_tag + "\n"
        + "\n"
        + mtv
        + ai_section
    )
...
async def _broadcast_to_channel(bot, channel_id: int, info: dict, sc: dict, ai: dict, contract: str, uid: int = 0) -> bool:
    """Post channel card v2. AI section hidden by default — toggle button reveals/hides it."""
    try:
        text    = _ai_report_text(info, sc, ai, contract=contract, expanded=False)
        ...
        def _build_kb(expanded: bool) -> InlineKeyboardMarkup:
            ...
            # Row 2 — X search pills (always shown)
            rows.append([
                InlineKeyboardButton("𝕏 $" + symbol,     url="https://twitter.com/search?q=%24" + symbol),
                InlineKeyboardButton("𝕏 CA",              url="https://twitter.com/search?q=" + contract),
                InlineKeyboardButton("𝕏 Name",            url="https://twitter.com/search?q=" + info.get("name", symbol).replace(" ", "%20")),
                InlineKeyboardButton("𝕏 Symbol",          url="https://twitter.com/search?q=" + symbol),
            ])

            # Row 3 — Social links (only if present)
            social_row = []
            if info.get("twitter"):  social_row.append(InlineKeyboardButton("🐦 Twitter",  url=info["twitter"]))
            if info.get("telegram"): social_row.append(InlineKeyboardButton("✈️ Telegram", url=info["telegram"]))
            if info.get("website"):  social_row.append(InlineKeyboardButton("🌐 Website",  url=info["website"]))
            if social_row:
                rows.append(social_row)

            # Row 4 — AI Analysis toggle
            ai_label = "🧠 Hide AI Analysis ▲" if expanded else "🧠 Show AI Analysis ▼"
            rows.append([InlineKeyboardButton(ai_label, callback_data="ch_ai_" + contract[:32])])
        ...
        msg = await bot.send_message(
            ...
            text=text,
            parse_mode="HTML",
            reply_markup=_build_kb(False)
        )

        # Cache card data so toggle button can regenerate
        _ch_card_cache[contract[:32]] = {
            ...
            "dex_url": dex_url,
            "gt_url": gt_url,
            "build_kb": None,   # set after def — see ch_ai_ handler
        }
            ...
            try:
                ...
                for milestone_x in _MILESTONE_XS:
                    ...
                    if current_x >= milestone_x:
                        ...
                        # Build milestone message (GemTools style)
                        rocket = "🚀" * min(milestone_x, 5)
                        msg = (
                            f"{rocket} *${symbol}* — *{milestone_x}x*\n"
                            f"━━━━━━━━━━━━━━━━━━\n"
                            f"📊 MC: *{mc_str(entry_mc)}* → *{mc_str(cur_mc)}*\n"
                            f"💰 Price: *{money(entry_price)}* → *{money(cur_price)}*\n"
                            f"⏱ Time to {milestone_x}x: *{elapsed_str}*\n"
                            "🎯 Called by *AURACLE\\_XBOT*"
                        )

                        # Add chart + trade buttons
                        chain = info.get("chain", "solana").lower()
                        dex_chain = {"solana":"solana","ethereum":"ethereum","base":"base","bsc":"bsc","arbitrum":"arbitrum"}.get(chain, chain)
                        ds_url = f"https://dexscreener.com/{dex_chain}/{contract}"
                        try:
                            await ctx.bot.send_message(
                                chat_id=ch_id,
                                text=msg,
                                parse_mode="Markdown",
                                reply_markup=InlineKeyboardMarkup([
                                    [InlineKeyboardButton(f"📈 Chart — {milestone_x}x", url=ds_url)],
                                ]),
                                disable_web_page_preview=True
                            )
        ...
        for item in unique_items:
            ...
            for uid, ud in active_users:
                try:
                    ...
                    # ════════════════════════════════════════════════════
                    if ud.get("apex_mode") and ai["verdict"] == "SNIPE":
                        apex_reset_daily(ud)
                        _ok = True
                        ...
                        elif sc.get("score",0) < ud.get("apex_learn_score_min", 45):                       _ok = False
                        if _ok:
                            ...
                            if not _already_queued:
                                ...
                                sniper_seen[contract] = _time.time()
...
def apex_check_threat(info: dict, h: dict) -> str:
    ...
    avg_5m     = vol_h1 / 12 if vol_h1 > 0 else 0
    if drop_pct <= -25 and buy_pct < 30:          return "RED"
    if avg_5m > 0 and vol_m5 >= avg_5m * 5 and buy_pct < 35: return "RED"
    if liq_at_buy > 0 and liq < liq_at_buy * 0.60: return "RED"
    red_sig = 0
    if drop_pct <= -15 and buy_pct < 40:           red_sig += 1
    if avg_5m > 0 and vol_m5 >= avg_5m * 3 and buy_pct < 45: red_sig += 1
    if liq_at_buy > 0 and liq < liq_at_buy * 0.75: red_sig += 1
    if red_sig >= 2: return "ORANGE"
    yellow = 0
    if drop_pct <= -10 and buy_pct < 48: yellow += 1
    if buy_pct < 45:                     yellow += 1
    if liq_at_buy > 0 and liq < liq_at_buy * 0.85: yellow += 1
    if yellow >= 2 or red_sig >= 1: return "YELLOW"
    return "CLEAR"
...
async def apex_run_position_manager(app, uid: int, ud: dict) -> None:
...
async def apex_process_entry_queue(app, uid: int, ud: dict) -> None:
    ...
    async with _get_user_lock(uid):
     for contract, entry in list(queue.items()):
        ...
        h["stop_loss_pct"]     = 20.0 if ai.get("rug_risk") == "HIGH" else 35.0
...
async def btn(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # ── APEX VAULT MENU ───────────────────────────────────────────────────────
        ...
        await q.edit_message_text(
            ...
            "Withdrawing moves them to your trading balance.",
            parse_mode="Markdown",
    ...
    # ── CHANNEL CARD AI TOGGLE ────────────────────────────────────────────────
        ...
        ca_key = cb[6:]   # contract[:32]
        card   = _ch_card_cache.get(ca_key)
        if not card:
            try: await q.answer("Card data expired — resend from Advisory Mode.", show_alert=True)
        ...
        dex_url   = card.get("dex_url")
        gt_url    = card.get("gt_url")
        symbol    = info.get("symbol", "TOKEN")
        chain     = info.get("chain", "solana").lower()
        dex_ch    = {"solana":"solana","ethereum":"ethereum","base":"base","bsc":"bsc","arbitrum":"arbitrum"}.get(chain, chain)
        ...
        # Rebuild keyboard with updated toggle label
        rows = []
        if bot_url:
            rows.append([InlineKeyboardButton("⚡ Buy on Auracle", url=bot_url)])
        ...
        ai_label = "🧠 Hide AI Analysis ▲" if expanded else "🧠 Show AI Analysis ▼"
        rows.append([InlineKeyboardButton(ai_label, callback_data="ch_ai_" + ca_key)])
        rows.append([
            InlineKeyboardButton("📊 DexScreener ↗", url=dex_url or "https://dexscreener.com"),
            InlineKeyboardButton("🔍 GeckoTerminal ↗", url=gt_url or "https://www.geckoterminal.com"),
        ])
        ...
        try:
            ...
            logger.warning(f"ch_ai toggle edit failed: {edit_err}")
            try: await q.answer("Couldn't update — try again.", show_alert=False)
            except Exception: pass
