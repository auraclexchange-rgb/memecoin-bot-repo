...
async def get_helius_maker_pct(contract: str, api_key: str) -> dict:
    ...
    try:
        ...
        total_wallets = len(set(list(buyer_vols.keys()) + list(seller_vols.keys())))
        if total_wallets == 0:
            return {}
...
async def sniper_scan() -> list:
    ...
    Each feed catches a different stage of token lifecycle.
    Returns merged, deduplicated list with extra metadata attached.
    """
    results: dict = {}   # tokenAddress → item

    # ── FEED 1: Pump.fun new launches (Solana only, earliest signal) ──────────
    try:
        client = await get_http()
        ...
        if pf_resp.status_code == 200:
            ...
            if isinstance(pf_data, list):
                for coin in pf_data:
                    ...
                    if not mint:
                        continue
                    # Calculate bonding curve % from reserves
                    v_sol = float(coin.get("virtual_sol_reserves", 0) or 0)
                    # Pump.fun bonding curve %:
                    # virtual_sol_reserves starts at 30 SOL (initial reserve)
                    # graduation threshold is ~85 SOL. Formula: (current-initial)/(grad-initial)*100
                    _pf_initial = 30.0
                    _pf_grad    = 85.0
                    ...
                    results[mint] = {
                        ...
                        "source":       "pumpfun",
                        "links":        [],
                        # Attach pump.fun specific data
                        "_pf_curve":    curve_pct,
                        "_pf_replies":  int(coin.get("reply_count", 0) or 0),
                        "_pf_graduated":graduated,
                        "_pf_dev_pct":  float(coin.get("creator_token_holdings_pct", 0) or 0),
        ...
        logger.info(f"Pump.fun feed: {len([v for v in results.values() if v.get('source')=='pumpfun'])} tokens")
    except Exception as e:
        logger.warning(f"Pump.fun feed error: {e}")

    # ── FEED 2 & 3: DexScreener boosts + profiles ─────────────────────────────
    ds_endpoints = [
        "https://api.dexscreener.com/token-boosts/latest/v1",
        "https://api.dexscreener.com/token-profiles/latest/v1",
    ]
    for url in ds_endpoints:
        try:
            client = await get_http()
                ...
                if isinstance(data, list):
                    for item in data:
                        ...
                        if addr not in results:
                            ...
                            # Enrich existing entry with boost amount if available
    ...
    for item in results.values():
        ...
        # Also check pump.fun native social fields
        pf_twitter  = bool(item.get("_pf_twitter"))
        pf_telegram = bool(item.get("_pf_telegram"))
        pf_website  = bool(item.get("_pf_website"))
        has_any_social = has_twitter or has_telegram or has_website or pf_twitter or pf_telegram or pf_website
        if has_any_social:
...
async def get_kol_recent_buys(wallet: str, helius_key: str, last_sig: str | None = None) -> list:
    ...
    Returns list of new buy events since last_sig.
    Each item: {mint, symbol, amount_sol, signature, timestamp}
    """
    if not helius_key:
        return []
    try:
        ...
        if last_sig:
            params["before"] = last_sig   # paginate backwards past last seen
            ...
            try:
                ...
                for buy in new_buys[:3]:   # max 3 alerts per wallet per cycle
                    ...
                    try:
                        ...
                        sol = buy["sol_spent"]
                        usd_est = sol * 150   # rough SOL/USD proxy; real price not fetched here
...
async def sniper_job(ctx: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await _aio2.gather(*[get_token(c["tokenAddress"], force=True) for c in unique_items], return_exceptions=True)

        # ── Helius API key (optional) ─────────────────────────────────────────
        import os as _os
        helius_key = _os.environ.get("HELIUS_API_KEY", "")

        # ── Expire stale sniper_seen entries (older than SNIPER_SEEN_EXPIRY_H) ─
        ...
        for item in unique_items:
            ...
            for uid, ud in active_users:
                try:
                    ...
                    elif info["liq"] < float(sf.get("min_liq", 15_000)):
...
async def btn(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # ── KOL / SMART WALLET TRACKER UI ─────────────────────────────────────────
        ...
        helius_line = "✅ Helius connected" if helius_set else "⚠️ *HELIUS\\_API\\_KEY not set* — add it in Railway Variables"
