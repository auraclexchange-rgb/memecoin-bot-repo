...
def _position_history_line(h: dict, current_price: float) -> str:
    ...
    Format (examples):
        ▁▂▄▇▅▃  Peak 3.4x  Low 0.82x
        ▁▁▂▁▁▁  Peak 1.1x  (flat token, no significant move)
        (empty string if no history yet)

    Fields used:
        h["peak_price"]    — all-time high since entry (set by checker_job)
        h["avg_price"]     — entry price
        sparkline          — from _position_sparkline()
    """
    ...
    # Low x from history (lowest price seen since entry)
    history = h.get("sr_history") or h.get("price_history") or []
    if history:
        low_p = min(snap["price"] for snap in history)
        low_x = round(low_p / avg, 2) if avg > 0 else 0
        low_txt = f"  Low {low_x}x" if low_x < 0.99 else ""
    else:
        low_txt = ""
    if not spark and peak_x < 1.01:
        return ""   # nothing meaningful to show yet
    parts = []
    if spark:
        parts.append(spark)
    parts.append(f"Peak {peak_x}x")
    if low_txt:
        parts.append(low_txt)
    return "  " + "  ".join(parts)
