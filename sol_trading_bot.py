...
MIN_BALANCE = 1.0
SNIPER_SEEN_EXPIRY_H = 24.0  # don't re-signal same token for 24h once seen
...
APEX_PROFILES = {
    ...
    "hunter": {
        ...
        "sl_low":            20.0,   # LOW rug risk initial stop
        "sl_med":            16.0,   # MEDIUM rug risk
        "sl_high":           12.0,   # HIGH rug risk (same as default — rugs are rugs)
        # ── Ratchet thresholds ───────────────────────────────────────────────
        "ratchet_1x":        1.30,   # at 1.3x: SL moves to -5% from entry
        "ratchet_2x":        1.60,   # at 1.6x: SL locks to entry (break-even)
        "red_2x_start":      1.30,   # after 1.3x: require 2× RED before exit
        # ── Trail (only active once position is profitable) ──────────────────
        "trail_activate_x":  1.60,   # trail starts at 1.6x (we have profit locked)
        "trail_1_6":         0.30,   # 1.6x–2x:  30% trail  — give early gains room
        "trail_2_0":         0.38,   # 2x–5x:    38% trail  — the big move zone
        "trail_5_0":         0.28,   # 5x–10x:   28% trail  — tighten as moon forms
        "trail_10":          0.18,   # 10x+:     18% trail  — lock moonshot gains
        # ── Entry gate (Hunter is selective) ─────────────────────────────────
        "min_confidence":     6,     # only 6+/10 confidence tokens
        "min_score":         58,     # only 58+/100 score tokens
        "daily_loss_limit":   0.25,  # 25% daily loss tolerance (vs 20% Default)
        "heat_stop":          0.80,  # same heat cap
        # ── ORANGE/YELLOW trail tightening ───────────────────────────────────
        "orange_tighten":     0.10,  # less aggressive tighten on orange
        "yellow_tighten":     0.15,  # barely reacts to yellow
    },
}
...
def _compact_pill_text(info: dict, sc: dict, ai: dict) -> str:
    ...
    return (
        f"{v_icon} *${symbol}* — {verdict}  `{conf_bar}` {confidence}/10\n"
...
def _ai_report_text(info: dict, sc: dict, ai: dict, contract: str = "", expanded: bool = True) -> str:
    ...
    gf = "\n".join(["  ✅ " + f for f in green_flags]) if green_flags else "  _None_"
    rf = "\n".join(["  ⚠️ " + f for f in red_flags])   if red_flags   else "  _None_"

    return (
        f"{v_icon} *{verdict}* — *${symbol}* _{name}_\n"
        f"`{contract}`\n\n"
        f"Confidence: `{conf_bar}` *{confidence}/10*\n"
        f"{rug_icon} Rug: *{rug_risk}*  |  Momentum: *{momentum}*  |  Social: *{social_sc}*\n\n"
        f"Score: `{score_bar}` *{score}/100* {s_icon}\n\n"
...
def _channel_card_text(info: dict, sc: dict, ai: dict, contract: str) -> str:
    """
    Build the HTML channel broadcast card.
    Includes: verdict, score, M·T·V Intelligence, MC/Liq, socials (clickable),
    X search link, green/red flags, suggested entry.
    No AI-analysis toggle button — clean card only.
    """
    symbol      = info.get("symbol", "?")
    name        = info.get("name", symbol)
    chain       = info.get("chain", "SOL").upper()
    ...
    score       = sc.get("score", 0)
    # MTV data
    buys_m5     = info.get("buys_m5", 0);  sells_m5  = info.get("sells_m5", 0)
    buy_pct_m5  = info.get("buy_pct_m5", 50)
    buys_h1     = info.get("buys_h1", 0);  sells_h1  = info.get("sells_h1", 0)
    buy_pct_h1  = info.get("buy_pct_h1", 50)
    buys_h24    = info.get("buys", 0);     sells_h24 = info.get("sells", 0)
    # Socials
    twitter  = info.get("twitter", "")
    telegram = info.get("telegram", "")
    website  = info.get("website", "")
    ...
    mtv_block = (
        ...
        f"M5    T: {t_m5}  {_bp_icon(buy_pct_m5)}{buy_pct_m5}%   V: {_mf(vol_m5)}  {_bp_icon(buy_pct_m5)}{buy_pct_m5}%\n"
        f"H1    T: {t_h1}  {_bp_icon(buy_pct_h1)}{buy_pct_h1}%   V: {_mf(vol_h1)}  {_bp_icon(buy_pct_h1)}{buy_pct_h1}%\n"
        f"H24  T: {t_h24}  {_bp_icon(buy_pct)}{buy_pct}%   V: {_mf(vol_h24)}  {_bp_icon(buy_pct)}{buy_pct}%"
    )

    # ── Clickable socials + X search ─────────────────────────────────────────
    soc_parts = []
    if twitter:  soc_parts.append(f'<a href="{twitter}">🐦 TW</a>')
    if telegram: soc_parts.append(f'<a href="{telegram}">💬 TG</a>')
    if website:  soc_parts.append(f'<a href="{website}">🌐 Web</a>')
    soc_line  = "  ·  ".join(soc_parts) if soc_parts else "None"
    x_search  = f'<a href="https://x.com/search?q=%24{symbol}">🔍 Search ${symbol} on X</a>'

    gf = "\n".join(["  ✅ " + f for f in green_flags]) if green_flags else "  <i>None</i>"
    rf = "\n".join(["  ⚠️ " + f for f in red_flags])   if red_flags   else "  <i>None detected</i>"
    ...
    text = (
        ...
        f"{mtv_block}\n\n"
        f"MC: <b>{_mf(mc)}</b>  ·  Liq: <b>{_mf(liq)}</b> ({round(liq_pct, 1)}%)\n"
...
async def _broadcast_to_channel(bot, channel_id: int, info: dict, sc: dict, ai: dict, contract: str, uid: int = 0) -> bool:
    """Post channel card. Clean card — no AI-analysis toggle. DEX shortlinks included."""
    try:
        ...
        dt_ch  = {"solana":"solana","ethereum":"ether","base":"base","bsc":"bnb","arbitrum":"arbitrum"}.get(chain, chain)
        ...
        dt_url  = "https://www.dextools.io/app/en/" + dt_ch + "/pair-explorer/" + contract
        ...
        # DEX shortlinks row — fix 5
        rows.append([
            InlineKeyboardButton("📊 DS",  url=dex_url),
            InlineKeyboardButton("🦎 GT",  url=gt_url),
            InlineKeyboardButton("🔧 DXT", url=dt_url),
        ])
        ...
        _ch_card_cache[contract[:32]] = {
            ...
            "gt_url": gt_url, "dt_url": dt_url,
        }
        # Pass message_id so milestone replies attach to the original call — fix 4
        if uid:
    ...
    async with _get_user_lock(uid):
        for contract, h in list(ud.get("holdings", {}).items()):
            ...
            try:
                ...
                _hp    = get_apex_profile(ud)   # None = default, dict = hunter
...
async def apex_process_entry_queue(app, uid: int, ud: dict) -> None:
    ...
    async with _get_user_lock(uid):
      for contract, entry in list(queue.items()):
        ...
        h["apex_threat"]       = "CLEAR"
        h["apex_vault_locked"] = {}
        h["apex_hunter_floor"] = 0.0   # Hunter ratchet floor (price absolute, not pct)
        ...
        # ── Rich entry metadata for self-learning ────────────────────────────
        h["apex_entry_rug"]       = ai.get("rug_risk", "LOW")
        ...
        # Stop loss at entry — use learned per-tier SL if available, else defaults
        _rug = ai.get("rug_risk", "LOW")
        _hp  = get_apex_profile(ud)
        if _hp:
            _apex_sl = _hp["sl_high"] if _rug == "HIGH" else (_hp["sl_med"] if _rug == "MEDIUM" else _hp["sl_low"])
        else:
            # Use calibration-learned SL if available, else hardcoded defaults
