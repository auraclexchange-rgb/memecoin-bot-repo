...
async def send_token_card(
    ...
    if is_query:
        ...
        _edit_txt   = (_header_url + "\n" + card_txt) if _header_ok else card_txt
        ...
        # Only use header URL if it came directly from DexScreener info field
        # (not our fallback guess) — fallback URLs 404 for tokens without banners
        # and a 404 URL still triggers a link preview box (just broken/empty).
        _use_header = header_url and "header_image" in info and bool(
            info.get("_header_confirmed")  # set below when DexScreener provided it
