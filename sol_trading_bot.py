    ...
    if cb == "mm":
        ...
        try:
            def _sort_key(x):
                ...
                if isinstance(val, str):
                    try:
                        ...
                        return datetime.min
                return datetime.min
            recent = sorted(logs, key=_sort_key, reverse=True)[:10]
            ...
            for t in recent:
                ...
                lines.append(
                    icon + " *$" + t.get("symbol", "?") + "*  " + str(round(t.get("x", 0), 2)) + "x  " + pstr(pnl) + "\n"
                    "  Held: " + str(t.get("hold_h", 0)) + "h  |  " + "_" + reason_short + "_" + j
                )
            _btn_rows = []
            _row = []
            seen_contracts = set()
            for t in recent:
                ...
                pnl_icon = "✅" if (t.get("realized_pnl", 0) or 0) > 0 else "❌"
