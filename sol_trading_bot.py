...
def token_card(info: dict, contract: str, ud: dict, sc: dict = None) -> str:
    ...
    try:
        ...
        if ath_price and ath_price > price and price > 0:
            ...
            ath_line = "🏆 ATH  " + mc_str(ath_mc) + "  (-" + str(ath_down) + "% from ATH)\n"

        # ── pressure bar ──────────────────────────────────────────────────────
        filled = round(buy_pct / 10)
        ...
        website      = info.get("website", "")
        soc_parts = []
        if twitter:      soc_parts.append("🐦 [TW]("  + twitter      + ")")
        if telegram_url: soc_parts.append("💬 [TG]("  + telegram_url + ")")
        if website:      soc_parts.append("🌐 [WEB](" + website      + ")")
        social_line = ("🔗 " + "  ".join(soc_parts) + "\n") if soc_parts else ""
        ...
        x_line = (
            ...
            + "[" + _safe(name)[:10].strip() + "](" + base_x.format(urllib.parse.quote(name)) + ")  "
        ...
        dex_url   = "https://dexscreener.com/"  + dex_chain  + "/" + contract
        gmgn_url  = "https://gmgn.ai/"          + gmgn_chain + "/token/" + contract
        pump_url  = "https://pump.fun/"          + contract
        axiom_url = "https://axiom.trade/t/"     + contract
        ...
        if contract in ud.get("holdings", {}):
            ...
            pos_block = (
                "┌─────────────────────────┐\n"
                "│  📌 YOUR POSITION\n"
                "│  Balance  " + money(cv) + "   " + cx_e + " " + str(round(cx, 2)) + "x\n"
                "│  " + pnl_e + " PnL  " + pstr(ppnl) + "\n"
