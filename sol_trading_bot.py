...
def generate_trade_card(symbol: str, chain: str, pnl_str: str, x_val: str, held_h: str, reason: str, pnl_positive: bool) -> "io.BytesIO | None":
    if not PILLOW_OK:
        return None
    try:
        W, H = 800, 440
        ...
        accent = (0, 200, 100) if pnl_positive else (220, 50, 50)
        draw.rectangle([0, 0, W, 6], fill=accent)
        draw.text((W - 20, 20), "AURACLE_XBOT", font=font_tiny, fill=(100, 100, 120), anchor="ra")
        draw.rounded_rectangle([20, 20, 130, 50], radius=10, fill=(30, 40, 60))
        draw.text((75, 35), chain.upper(), font=font_tiny, fill=(150, 180, 220), anchor="mm")
        draw.text((40, 60), "$" + symbol, font=font_big, fill=(255, 255, 255))
        draw.line([(40, 155), (W - 40, 155)], fill=(40, 50, 70), width=1)
        pnl_color = (0, 220, 100) if pnl_positive else (220, 60, 60)
        pnl_prefix = "+" if pnl_positive else ""
        draw.text((40, 175), "PnL", font=font_small, fill=(120, 130, 150))
        draw.text((40, 210), pnl_prefix + pnl_str, font=font_med, fill=pnl_color)
        draw.text((W // 2, 175), "Multiple", font=font_small, fill=(120, 130, 150), anchor="la")
        draw.text((W // 2, 210), str(x_val) + "x", font=font_med, fill=(255, 200, 50), anchor="la")
        draw.line([(40, 275), (W - 40, 275)], fill=(40, 50, 70), width=1)
        draw.text((40, 295), "Held: " + str(held_h) + "h", font=font_small, fill=(160, 170, 190))
        draw.text((40, 335), "Entry: " + str(reason), font=font_small, fill=(160, 170, 190))
        draw.rectangle([0, H - 50, W, H], fill=(20, 25, 35))
        draw.text((W // 2, H - 25), "Paper Trading  |  AURACLE_XBOT", font=font_tiny, fill=(80, 90, 110), anchor="mm")
        buf = io.BytesIO()
...
async def btn(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if cb == "mm":
        ...
        card = generate_trade_card(
            ...
            held_h     = str(trade["hold_h"]),
            reason     = trade.get("mood", "Manual"),
