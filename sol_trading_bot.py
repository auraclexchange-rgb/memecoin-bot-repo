...
APEX_DAILY_LOSS_HALT_ON = False   # Global: False = halt disabled for all users, True = enabled
                ...
                try:
                    ...
                    if skip_reason:
                        ...
                        if _dl_ch_skip and ud.get("sniper_log_channel_on", True):
                            ...
                            _dl_details = "  ".join(x for x in [_dl_age_s, _dl_ath_s, _dl_top10_s, _dl_bundle_s, _dl_lp_s] if x)
                            _dl_text   = (
...
def apex_is_daily_loss_halted(ud: dict) -> bool:
    apex_reset_daily(ud)
    # Global override — if halt is disabled globally, never halt any user
    if not APEX_DAILY_LOSS_HALT_ON:
        return False
...
async def btn(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    global APEX_DAILY_LOSS_HALT_ON
    ...
    # ── APEX VAULT MENU ───────────────────────────────────────────────────────
        ...
        _halt_display = "*" + _halt_str + "*" if APEX_DAILY_LOSS_HALT_ON else "~~" + _halt_str + "~~ _(disabled globally)_"
        _halt_btn_lbl = "🔴 Disable Loss Halt (all users)" if APEX_DAILY_LOSS_HALT_ON else "🟢 Enable Loss Halt (all users)"
        await q.edit_message_text(
            "⚙️ *APEX SETTINGS*\n\n"
        ...
        APEX_DAILY_LOSS_HALT_ON = not APEX_DAILY_LOSS_HALT_ON
        _new_state = APEX_DAILY_LOSS_HALT_ON
        await q.answer(
            "Loss halt " + ("ENABLED — APEX pauses at loss limit." if _new_state else "DISABLED — APEX trades through losses for all users."),
            show_alert=True
        )
        # Re-render settings menu with updated global state
        conf_thr  = ud.get("apex_learn_threshold", 5)
        score_min = ud.get("apex_learn_score_min", 45)
        split_pct = ud.get("apex_vault_profit_split", 0.50)
        profile   = ud.get("apex_risk_profile", "default")
        _hp_label2  = {"default": "⚖️ Default", "hunter": "🎯 Hunter"}.get(profile, "⚖️ Default")
        _trail_str2 = "2.5x (ratchet)" if profile == "hunter" else "1.5x"
        _halt_str2  = "25%" if profile == "hunter" else "20%"
        _halt_display2 = "*" + _halt_str2 + "*" if _new_state else "~~" + _halt_str2 + "~~ _(disabled globally)_"
        _halt_btn_lbl2 = "🔴 Disable Loss Halt (all users)" if _new_state else "🟢 Enable Loss Halt (all users)"
        await q.edit_message_text(
            "⚙️ *APEX SETTINGS*\n\n"
            "Auto-tuned by self-learning engine every 10 trades.\n\n"
            "Min confidence: *" + str(conf_thr) + "/10*\n"
            "Min score: *" + str(score_min) + "/100*\n"
            "Daily loss halt: " + _halt_display2 + "\n"
            "Trail activates at: *" + _trail_str2 + "*\n"
            "Vault profit split: *" + str(int(split_pct*100)) + "%* → main balance\n\n"
            "🎛 *Risk Profile:* " + _hp_label2 + "\n"
            "_Use the Vault menu to adjust profit split %._",
            parse_mode="Markdown",
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("🎛 Change Risk Profile",  callback_data="apex_profile_menu")],
                [InlineKeyboardButton(_halt_btn_lbl2,            callback_data="apex_halt_toggle")],
                [InlineKeyboardButton("⬅️ Back",                callback_data="apex_menu")],
            ])
        )
