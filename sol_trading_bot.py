    ...
    async with _get_user_lock(uid):
        for contract, h in list(ud.get("holdings", {}).items()):
            ...
            try:
                ...
                # ── Exit ladder: break-even floor at 1.3x, profit takes at 2x / 5x ──
                # At 1.3x: lock floor to entry price (no sell — just protection).
                # First real sell at 2x: 25% locked as cash.
                # The original design was 2x/5x exits. The 1.3x sell was removed
                # because it cuts runners short — a token that hits 1.3x often
                # continues to 3x, 5x, 10x. Lock break-even, let it run.
                if not _hp:
                    # ── 1.3x break-even floor (NO SELL — floor lock only) ──────────
                    if cx >= 1.3 and not h.get("apex_ladder_sold_1_3x"):
                        h["apex_ladder_sold_1_3x"] = True  # flag reused to mean "floor locked"
                        _old_sl = h.get("stop_loss_pct")
                        h["apex_hunter_floor"] = avg * 1.002
                        h["stop_loss_pct"]     = None
                        h.setdefault("stop_loss_history", []).append({
                            "old":    _old_sl,
                            "new":    None,
                            "source": "apex_be_floor_1_3x",
                            "cx":     round(cx, 3),
                            "ts":     _time.time(),
                        })
                        try:
                            await app.bot.send_message(
                                chat_id=uid, parse_mode="Markdown",
                                text=(
                                    "🛡 *APEX — BREAK-EVEN LOCKED*\n\n"
                                    "*$" + _md(h["symbol"]) + "*  hit *1.3x*\n"
                                    "Floor locked at entry — cannot lose on this position\n"
                                    "_Holding for 2x+ target…_"
                                ),
                                reply_markup=main_menu_kb()
                            )
                        except Exception:
                    ...
                    # ── 2x sell: return initial capital ──────────────────────────
                    # At 2x the position is worth 2× the investment.
                    # Selling 50% returns exactly the initial capital back to vault.
                    # The remaining 50% is pure house money — zero risk, ride to 10x.
                    # Trail width after 2x is determined by token intelligence:
                    #   High conviction (score 75+, TECH, first narrative) → wide trail, target 10x
                    #   Medium conviction → standard trail
                    #   MEME + late narrative → tighter trail, target 3-5x
                ...
                # ── Fix: clear SL the moment trail activates ──────────────────────
                # Data shows SL was firing on positions at 1.3x–1.7x because SL and
                # trail were running simultaneously. Once trail is active the position
                # is profitable — SL has no business running anymore.
                if cx >= _trail_activate and h.get("stop_loss_pct") is not None:
                    h["stop_loss_pct"] = None
                    sl_pct = None  # update local var too so SL block above is clean
...
async def btn(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # ── APEX MANUAL CONTROL (from token card button) ──────────────────────────
        ...
        wave_labels = {
            "SILENCE":  "SILENCE - Quiet market. Patient hold.",
            "STIRRING": "STIRRING - Wave forming. Standard exits.",
            "WAVE":     "WAVE - Active. Extract fast.",
            "CRASH":    "CRASH - Defensive mode only.",
            "RESET":    "RESET - Waiting for next cycle.",
        }
        wave_desc = wave_labels.get(wave, wave)
        text = (
            "🤖 APEX CONTROL — $" + sym + "\n\n"
            "Wave: " + wave_desc + "\n\n"
            "Exit params:\n"
            "  Trail activates: " + str(ep["trail_activate_x"]) + "x\n"
            "  Trail: " + str(round(ep["trail_pct_early"]*100)) + "% early / " + str(round(ep["trail_pct_mid"]*100)) + "% mid\n"
            "  Ladder: " + str(round(ep["ladder_1_3x_pct"]*100)) + "% at 1.3x | " + str(round(ep["ladder_2x_pct"]*100)) + "% at 2x\n\n"
            "Buy below and APEX manages the position."
        )
        kb = InlineKeyboardMarkup([
...
def main():
    ...
    app.job_queue.run_repeating(advisory_job,          interval=300,  first=90)   # advisory engine
