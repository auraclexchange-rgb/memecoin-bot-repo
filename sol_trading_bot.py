        ...
        if chain_id_raw in ("solana", "sol"):
            try:
                ...
                if _rc_fresh:
                    ...
                    async with sem:
                        await _asyncio.sleep(0.3)   # ~3 req/sec — RugCheck free tier handles this
                        rc = await get_http()
                        rc_r = await rc.get(
...
def _channel_card_text(info: dict, sc: dict, ai: dict, contract: str) -> str:
    ...
    # ── Banner image as hidden link preview (renders at bottom) ─────────────
    _ch_header = info.get("header_image", "")
    _ch_use_hdr = bool(_ch_header and (
        info.get("_header_confirmed")
        or not _ch_header.startswith("https://dd.dexscreener.com/ds-data/")
    ))
    _hidden_img = f'[\u200b]({_ch_header})\n' if _ch_use_hdr else ""
