...
# ── APEX Risk Profile Presets ─────────────────────────────────────────────────
# Each profile is a dict of overrides applied at runtime via get_apex_profile().
# Keys map directly to the constants / thresholds used throughout the engine.
# "default" = existing behaviour (unchanged).
#
#  trail_activate_x  — multiple at which trailing stop kicks in
#  trail_early/mid/high/moon — trail % at each cx band
#  stop_low/med/high — initial stop-loss % by rug_risk (LOW/MEDIUM/HIGH)
#  peak_exp_1x/2x   — extra stop % earned when peak passes 1.2x / 2.0x
#  lock_2x/5x_pct   — fraction of profit reserved at 2x/5x milestones
#  daily_loss_limit  — fraction of session balance allowed to be lost per day
#  heat_stop         — capital-heat % that blocks new entries
#  min_confidence    — minimum AI confidence for entry gate
#  min_score         — minimum sniper score for entry gate
#  red_liq_drain     — liq drain % that triggers unconditional RED
#  red_buy_pct_hard  — buy% below which is unconditional RED panic
#  orange_tighten    — trail% cap when threat is ORANGE
#  yellow_tighten    — trail% cap when threat is YELLOW
#  threat_red_immediate — True: exit on first RED (default). False: require 2 consecutive REDs
#  pos_size_conf_gate   — minimum confidence to take any position at all
APEX_PROFILES = {
    "default": {
        ...
        "min_score":           45,
        "red_liq_drain":       25.0,
        "red_buy_pct_hard":    10.0,
        "orange_tighten":       0.06,
        "yellow_tighten":       0.10,
        "threat_red_immediate": True,
        "pos_size_conf_gate":   5,
    },
    "hunter": {
        # ── Philosophy: ride the big moves, sit through volatility ──────────
        # Trail activates much later — token must prove itself before APEX
        # starts pulling the leash. Wide trail percentages mean a 10x can
        # retrace 40% and still be held. Daily loss limit is higher because
        # Hunter accepts more variance in exchange for catching the 50x.
        "trail_activate_x":    2.5,    # was 1.5 — don't trail until genuine momentum
        "trail_early":         0.40,   # was 0.18 — breathe through normal volatility
        "trail_mid":           0.32,   # was 0.15
        "trail_high":          0.22,   # was 0.12
        "trail_moon":          0.14,   # was 0.08 — even moonshots get room to consolidate
        "stop_low":            35.0,   # was 22 — healthy tokens get maximum room
        "stop_med":            28.0,   # was 18
        "stop_high":           18.0,   # was 12 — high rug risk still tighter, but not panic-tight
        "peak_exp_1x":         15.0,   # was 8  — proven buyer interest → more room
        "peak_exp_2x":         25.0,   # was 15 — multi-bagger proven → ride the full wave
        "lock_2x_pct":          0.30,  # was 0.50 — lock less at 2x, keep more running
        "lock_5x_pct":          0.60,  # was 0.75 — same at 5x
        "daily_loss_limit":     0.30,  # was 0.20 — wider daily loss tolerance
        "heat_stop":            0.85,  # was 0.80 — slightly more capital deployed at once
        "min_confidence":       6,     # was 3 — Hunter is PICKY. Skips low-conviction plays.
        "min_score":           60,     # was 45 — only high-quality entries
        "red_liq_drain":       22.0,   # same — liq drain is always a hard signal
        "red_buy_pct_hard":     8.0,   # was 10 — Hunter is less hair-trigger on buy%
        "orange_tighten":       0.12,  # was 0.06 — don't over-tighten on orange
        "yellow_tighten":       0.18,  # was 0.10 — yellow barely changes the trail
        "threat_red_immediate": False, # was True — require 2 consecutive REDs before exiting
        "pos_size_conf_gate":   6,     # same as min_confidence
    },
}
...
def _recent_loss_names(uid: int, hours: float = 12.0) -> list:
    ...
    return [
        ...
        and t.get("closed_at", datetime.min) >= cutoff
    ]
            ...
            for uid, ud in active_users:
                try:
                    ...
                    if ud.get("apex_mode") and _apex_verdict_ok:
                        ...
                        _ap = get_apex_profile(ud)   # active risk profile
                        if _apex_phase == "learning":
                            # Learning: heat check + SOL market condition
                            if apex_capital_heat(ud) >= _ap["heat_stop"]:           _ok = False
                            ...
                            elif apex_capital_heat(ud) >= _ap["heat_stop"]:          _ok = False
                            elif apex_count_positions(ud) >= ud.get("apex_max_positions_learned", APEX_MAX_POSITIONS): _ok = False
                            elif ai.get("confidence",0) < max(ud.get("apex_learn_threshold", APEX_MIN_CONFIDENCE), _ap["min_confidence"]): _ok = False
                            elif sc.get("score",0) < max(ud.get("apex_learn_score_min", 45), _ap["min_score"]):        _ok = False
                        if _ok:
...
def apex_position_size(ud: dict, ai_confidence: int, base_amount: float) -> float:
    p    = get_apex_profile(ud)
    heat = apex_capital_heat(ud)
    if heat >= p["heat_stop"]:
        return 0.0
    # Hunter: block low-confidence entries entirely
    if ai_confidence < p["pos_size_conf_gate"]:
        return 0.0
    if ai_confidence >= 9:
        conf_mult = 1.0
...
def apex_trail_pct(current_x: float, ud: dict = None) -> float:
    p = get_apex_profile(ud) if ud else APEX_PROFILES["default"]
    if current_x >= 10.0: return p["trail_moon"]
    if current_x >= 5.0:  return p["trail_high"]
    if current_x >= 2.0:  return p["trail_mid"]
    return p["trail_early"]


def apex_check_threat(info: dict, h: dict, ud: dict = None) -> str:
    """
    Rug/dump threat detector — runs every 8–15s.
    All thresholds read from the active risk profile so Hunter mode
    can sit through volatility without exiting too early.
    """
    p          = get_apex_profile(ud) if ud else APEX_PROFILES["default"]
    price      = info.get("price", 0)
    avg        = h.get("avg_price", price)
    ...
    # ── Compute smart RED threshold (profile-aware) ───────────────────────────
    base_stop = h.get("stop_loss_pct", 15.0)
    peak      = h.get("apex_peak_price", avg)
    cx_peak   = (peak / avg) if avg > 0 else 1.0
    cx_now    = (price / avg) if avg > 0 else 1.0
    if cx_peak >= 2.0:
        red_threshold = -(base_stop + p["peak_exp_2x"])
    elif cx_peak >= 1.2:
        red_threshold = -(base_stop + p["peak_exp_1x"])
    else:
        red_threshold = -base_stop

    # ══ HARD FLOOR RED — rug signals ══════════════════════════════════════════
    if liq_drop >= p["red_liq_drain"]:
        return "RED"

    if buy_pct_m5 < p["red_buy_pct_hard"]:
        return "RED"
    elif buy_pct_m5 < 20:
        if cx_now < 0.95:
            return "RED"
    ...
    # ══ ORANGE ════════════════════════════════════════════════════════════════
    orange_threshold = red_threshold * 0.55
    sigs = 0
...
def get_apex_profile(ud: dict) -> dict:
    """Return the active APEX risk profile for this user.
    Falls back to 'default' if no profile is set or an unknown name is stored.
    Usage:  p = get_apex_profile(ud);  trail_pct = p["trail_early"]
    """
    name = ud.get("apex_risk_profile", "default")
    return APEX_PROFILES.get(name, APEX_PROFILES["default"])
...
def apex_is_daily_loss_halted(ud: dict) -> bool:
    ...
    if start is None or start <= 0:
        return vault <= 0.0
    _limit = get_apex_profile(ud)["daily_loss_limit"]
    ...
    async with _get_user_lock(uid):
        for contract, h in list(ud.get("holdings", {}).items()):
            ...
            try:
                ...
                _ap    = get_apex_profile(ud)          # active risk profile
                threat = apex_check_threat(info, h, ud)
                ...
                # ── RED: emergency exit ───────────────────────────────────────────
                # Hunter: require 2 consecutive RED cycles before exiting
                # (protects against single-candle wick-outs on thin books)
                _is_red = (threat == "RED" or info.get("_helius_rug"))
                if _is_red:
                    if not _ap["threat_red_immediate"]:
                        # Hunter mode: only exit if RED was already set last cycle too
                        if _prev_threat != "RED":
                            # First RED — record but don't exit yet. Trail tightens.
                            h["apex_threat"] = "RED"
                            continue
                        # Second consecutive RED — now exit
                    cv     = h["amount"] * price
                ...
                if cx >= 5.0 and "5x" not in vault_locks:
                    profit   = cv - entry_val
                    lock_amt = max(0.0, profit * _ap["lock_5x_pct"])
                    if lock_amt > 0:
                    ...
                    profit   = cv - entry_val
                    lock_amt = max(0.0, profit * _ap["lock_2x_pct"])
                    if lock_amt > 0:
                ...
                # ── APEX Stop-Loss (pre-trail, before position can recover) ─────────
                # run_checker skips APEX moods — we own the stop-loss here.
                # Only fires when cx < profile's trail_activate_x (trail not yet active).
                sl_pct = h.get("stop_loss_pct")
                if sl_pct and cx < _ap["trail_activate_x"]:
                    drop_pct_sl = (price - avg) / avg * 100
                    if drop_pct_sl <= -sl_pct:
                        cv_sl = h["amount"] * price
                        if cv_sl >= 0.01:
                            result = sell_core(ud, uid, contract, cv_sl, price, "stop_loss")
                            ud["apex_daily_pnl"] = ud.get("apex_daily_pnl", 0) + result["realized"]
                            if result["realized"] < 0:
                                ud["apex_consec_losses"] = ud.get("apex_consec_losses", 0) + 1
                                if ud["apex_consec_losses"] >= 3:
                            ...
                            _apex_post_exit.setdefault(uid, {})[contract] = {
                                ...
                                "exit_reason": "stop_loss", "exit_x": round(cx, 3),
                                "peak_x":      round(peak / avg, 3) if avg > 0 else 0,
                            ...
                            try:
                                await app.bot.send_message(
                                    ...
                                    text=(
                                        "🛑 *APEX — STOP LOSS HIT*\n\n"
                                        "*$" + h["symbol"] + "* dropped *" + str(round(drop_pct_sl, 1)) + "%*\n"
                                        "SL: *-" + str(sl_pct) + "%*  |  Exit: *" + str(round(cx, 2)) + "x*\n"
                                        "PnL: *" + pstr(result["realized"]) + "*\n"
                                        "🏦 Vault: *" + money(ud.get("apex_vault", 0)) + "*  |  Cash: *" + money(ud["balance"]) + "*"
                                    ),
                                    reply_markup=main_menu_kb()
                                )
                            except Exception:
                ...
                # ── Trailing stop ─────────────────────────────────────────────────
                if cx >= _ap["trail_activate_x"]:
                    tpct = apex_trail_pct(cx, ud)
                    if threat == "ORANGE":
                        tpct = min(tpct, _ap["orange_tighten"])
                    elif threat == "YELLOW":
                        tpct = min(tpct, _ap["yellow_tighten"])
...
async def apex_process_entry_queue(app, uid: int, ud: dict) -> None:
    ...
    async with _get_user_lock(uid):
      for contract, entry in list(queue.items()):
        ...
        # Mark watchlist re-entries so they are NEVER re-watchlisted on exit.
        # This breaks chained re-buys (e.g. Monk bought 4× on same contract).
        if entry.get("reentry"):
            h["apex_watchlist_reentry"] = True
        # Tighter stop loss — rugs move fast
        _rug = ai.get("rug_risk", "LOW")
        _ep  = get_apex_profile(ud)
        _apex_sl = _ep["stop_high"] if _rug == "HIGH" else (_ep["stop_med"] if _rug == "MEDIUM" else _ep["stop_low"])
    ...
    for uid, watchlist in list(_apex_watchlist.items()):
        ...
        for contract, rec in list(watchlist.items()):
            try:
                ...
                reentry_note = ""
                if apex_on and not rec.get("re_entry_queued"):
                    _heat_ok  = apex_capital_heat(ud) < APEX_HEAT_STOP
                    _loss_ok  = not apex_is_daily_loss_halted(ud)
                    _pause_ok = not apex_is_paused(uid)
                    _vault_ok = ud.get("apex_vault", 0) >= ud.get("sniper_filters", {}).get("buy_amount", 20.0)
                    if _heat_ok and _loss_ok and _pause_ok and _vault_ok:
                        base_amt = ud.get("sniper_filters", {}).get("buy_amount", 20.0)
                        buy_amt  = apex_position_size(ud, rec.get("entry_conf", 5), base_amt)
                        if buy_amt >= 1.0:
                            _re_ai = {
                                "verdict": "SNIPE", "confidence": rec.get("entry_conf", 5),
                                "rug_risk": "LOW",
                                "thesis": "Watchlist re-entry: reversal confirmed",
                                "suggested_amount": buy_amt,
                            }
                            _re_sc = {"score": rec.get("entry_score", 50), "flags": []}
                            _apex_entry_queue.setdefault(uid, {})[contract] = {
                                "info": info, "sc": _re_sc, "ai": _re_ai,
                                "queued_at": __import__("datetime").datetime.now(),
                                "base_amount": buy_amt, "reentry": True,
                            }
                            rec["re_entry_queued"] = True
                            reentry_note = "\n\n\u26a1 *APEX queued re-entry* (45s confirmation)"
                    else:
                        reentry_note = "\n\n_Re-entry skipped \u2014 vault empty, paused or heat too high_"
...
async def btn(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if cb == "mm":
        ...
        if not logs:
            ...
            return
        recent = sorted(logs, key=lambda x: x.get("closed_at", datetime.min), reverse=True)[:10]
        lines = ["\U0001f4dc *TRADE HISTORY*\n"]
        for t in recent:
            icon = "\U0001f7e2" if t["realized_pnl"] > 0 else "\U0001f534"
            reason_short = (t.get("reason", "manual")
                .replace("apex_trail_exit",     "trail exit")
                .replace("apex_threat_red",     "threat red")
                .replace("apex_momentum_decay", "momentum dec")
                .replace("manual",              "manual"))
            j = "\n  \"" + t["journal"][:40] + "\"" if t.get("journal") else ""
            lines.append(
                icon + " *$" + t["symbol"] + "*  " + str(round(t.get("x", 0), 2)) + "x  " + pstr(t["realized_pnl"]) + "\n"
                "  Held: " + str(t["hold_h"]) + "h  |  " + "_" + reason_short + "_" + j
            )
        # ── Clickable token card buttons — one per trade ──────────────────────
        # Each button opens the live token card (score, AI analysis, buy/sell).
        # Rows of 2 to keep the keyboard compact.
        _btn_rows = []
        _row = []
        seen_contracts = set()
        for t in recent:
            ...
            pnl_icon = "✅" if t["realized_pnl"] > 0 else "❌"
        ...
        if _row:
            _btn_rows.append(_row)
        _btn_rows.append([InlineKeyboardButton("\U0001f3e0 Main Menu", callback_data="mm"),
    ...
    # ── APEX VAULT MENU ───────────────────────────────────────────────────────
        ...
        for t in reversed(logs):
            ...
            rsn  = t.get("reason","?").replace("apex_","").replace("_"," ")[:12]
        ...
        profile_labels = {"default": "⚖️ Default", "hunter": "🎯 Hunter"}
        profile_label  = profile_labels.get(profile, "⚖️ Default")
        await q.edit_message_text(
            "\u2699\ufe0f *APEX SETTINGS*\n\n"
            "Auto-tuned by self-learning engine every 10 trades.\n\n"
            "Min confidence: *" + str(conf_thr) + "/10*\n"
            "Min score: *" + str(score_min) + "/100*\n"
            "Daily loss halt: *" + ("20%" if profile == "default" else "30%") + "*\n"
            "Drawdown pause: *30 min* after 3 losses\n"
            "Trail activates at: *" + ("1.5x" if profile == "default" else "2.5x") + "*\n"
        ...
        profile = ud.get("apex_risk_profile", "default")
        def _ptick(name): return "✅ " if profile == name else "      "
        await q.edit_message_text(
            "🎛 *APEX RISK PROFILE*\n\n"
            "Choose how APEX manages positions and takes profits.\n\n"
            "⚖️ *Default*\n"
            "Balanced exits. Trail at 1.5x, 18% trail, tight SL.\n"
            "Best for consistent small–medium wins.\n\n"
            "🎯 *Hunter*\n"
            "Rides the big moves. Trail at 2.5x, 40% trail.\n"
            "Sits through volatility. High confidence only (6+/10).\n"
            "Needs 2 consecutive RED signals before exiting.\n"
            "Best for 10x–100x potential tokens.\n\n"
            "_Active profile shown with_ ✅",
            parse_mode="Markdown",
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton(_ptick("default") + "⚖️ Default",  callback_data="apex_profile_set_default")],
                [InlineKeyboardButton(_ptick("hunter")  + "🎯 Hunter",   callback_data="apex_profile_set_hunter")],
        ...
        save_user(u.id, ud)
        labels = {"default": "⚖️ Default", "hunter": "🎯 Hunter"}
        await q.edit_message_text(
            "✅ *APEX Profile Updated*\n\n"
            + ("No change — already on *" + labels.get(new_profile, new_profile) + "*." if old_profile == new_profile else
               "Switched from *" + labels.get(old_profile, old_profile) + "* → *" + labels.get(new_profile, new_profile) + "*\n\n"
               + ("🎯 *Hunter is now active.* APEX will ride moves harder.\n"
                  "Trail activates at 2.5x · 40% trail · 2× RED required to exit.\n"
                  "Only confidence 6+/10 tokens will be entered." if new_profile == "hunter" else
                  "⚖️ *Default is now active.* Balanced exits restored.")),
            parse_mode="Markdown",
            reply_markup=InlineKeyboardMarkup([
        ...
        await q.edit_message_text(
            ...
            "Status: " + vault_status + "\n\n"
            "_Fund vault from your main balance to start APEX trading._\n"
