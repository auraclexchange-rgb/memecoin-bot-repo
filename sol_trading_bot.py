        ...
        if chain_id_raw in ("solana", "sol"):
            try:
                ...
                if rc_data:
                    ...
                    if holders:
                        # ── TEMPORARY DEBUG — remove after confirming filter works ─
                        for _di, _dh in enumerate(holders[:3]):
                            logger.info(f"RC holder[{_di}] keys={list(_dh.keys())} data={_dh}")
                        # Log markets structure to find pool/LP addresses
                        _mkts = rc_data.get("markets") or []
                        for _mi, _mk in enumerate(_mkts[:2]):
                            logger.info(f"RC market[{_mi}] keys={list(_mk.keys())} lp={_mk.get('lp',{})} pubkey={_mk.get('pubkey','?')} market={_mk.get('market','?')}")
                        logger.info(f"RC response top-level keys: {list(rc_data.keys())}")
                        # ─────────────────────────────────────────────────────────
                        ...
                        # They inflate top10% dramatically (e.g. +37% from PF alone).
                        #
                        # Verified Pump.fun program IDs (these never change):
                        _PROGRAM_IDS = {
                        ...
                        # Name-based filter: RugCheck labels these accounts in the
                        # holder's name/owner string. Any holder whose name contains
                        # these substrings is a protocol account, not a real holder.
...
def token_card(info: dict, contract: str, ud: dict, sc: dict = None) -> str:
    ...
    try:
        ...
        if any(x is not None for x in [no_mint, no_freeze, lp_burn, top10, insider]):
            ...
            # ── Holder row ────────────────────────────────────────────────────
            _holder_parts = []
            if top10   is not None: _holder_parts.append("Top10: " + str(top10) + "%")
...
async def do_sell_query(q, ud, uid, contract, pct=None, usd=None):
    ...
    if h.get("mood") not in ("APEX", "AI-Sniper", "APEX-DCA") and result.get("closed"):
        try:
            await _check_challenge(q.bot, uid, ud)
        except Exception:
            pass
