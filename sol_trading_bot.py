...
def more_menu_kb() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("🎯 Sniper Mode",    callback_data="v_sniper"),
...
async def sniper_job(ctx: ContextTypes.DEFAULT_TYPE):
    """Scan latest tokens and auto-buy for users with sniper mode ON."""
    sniper_users = [
        (uid, ud) for uid, ud in users.items()
        if ud.get("sniper_mode") and ud.get("balance", 0) > 0
    ]
    if not sniper_users:
        return
    ...
    for item in raw_tokens:
        ...
        for uid, ud in sniper_users:
            sf = ud.get("sniper_filters", {})
            buy_amt = float(sf.get("buy_amount", 100))

            # Apply filters
            if sc["score"] < int(sf.get("min_score", 60)):
                continue
            if info["liq"] < float(sf.get("min_liq", 50_000)):
                continue
            if not (float(sf.get("min_mc", 100_000)) <= info["mc"] <= float(sf.get("max_mc", 5_000_000))):
                continue
            if info.get("age_h") and info["age_h"] > float(sf.get("max_age_h", 48)):
                continue
            if buy_amt > ud.get("balance", 0):
                continue

            # Deduplicate — don't snipe same token twice per user
            sniper_bought = ud.setdefault("sniper_bought", [])
            if contract in sniper_bought:
                continue
            sniper_bought.append(contract)

            result = await do_buy_core(ud, uid, contract, buy_amt, planned=True, mood="Sniper")
            if isinstance(result, tuple):
                info2, _ = result
                try:
                    await ctx.bot.send_message(
                        chat_id=uid, parse_mode="Markdown",
                        text=t(ud, "sniper_fired",
                            symbol=info2["symbol"], score=sc["score"], verdict=sc["verdict"],
                            mc=mc_str(info2["mc"]), liq=money(info2["liq"]),
                            amount=money(buy_amt), price=money(info2["price"]),
                            cash=money(ud["balance"])
                        ),
                    ...
                    logger.error(f"Sniper notify error for {uid}: {_se}")
    ...
    if p:
        ...
        # ── SNIPER CONFIG INPUTS ───────────────────────────────────────────────
            ...
            try:
                ...
                pending.pop(u.id, None)
                await update.message.reply_text(
                    "✅ MC range set: *" + mc_str(min_mc) + "* → *" + mc_str(max_mc) + "*",
                    parse_mode="Markdown", reply_markup=back_main()
                )
            except Exception:
                await update.message.reply_text("❌ Format: min_mc max_mc\nExample: 100000 5000000", reply_markup=cancel_kb())
            return
            ...
            try:
                ...
                pending.pop(u.id, None)
                await update.message.reply_text("✅ Max age set to *" + str(val) + "h*", parse_mode="Markdown", reply_markup=back_main())
            except Exception:
                await update.message.reply_text("❌ Enter a number like 24", reply_markup=cancel_kb())
            return
            ...
            try:
                ...
                pending.pop(u.id, None)
                await update.message.reply_text("✅ Sniper buy amount set to *" + money(val) + "* per token", parse_mode="Markdown", reply_markup=back_main())
            except Exception:
...
async def btn(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # ── SNIPER MODE ────────────────────────────────────────────────────────────
        ...
        sf  = ud.get("sniper_filters", {})
        on  = ud.get("sniper_mode", False)
        stx = "🟢 ON" if on else "🔴 OFF"
        await q.edit_message_text(
            "🎯 *SNIPER MODE*  —  " + stx + "\n\n"
            "Automatically buys new tokens that match your filters.\n\n"
            "Current filters:\n"
            "  Min Score: *" + str(sf.get("min_score", 60)) + "/100*\n"
            "  Min Liquidity: *" + money(sf.get("min_liq", 50_000)) + "*\n"
            "  MC Range: *" + mc_str(sf.get("min_mc", 100_000)) + "* → *" + mc_str(sf.get("max_mc", 5_000_000)) + "*\n"
            "  Max Age: *" + str(sf.get("max_age_h", 48)) + "h*\n"
            "  Buy Amount: *" + money(sf.get("buy_amount", 100)) + "* per token\n\n"
            "Sniper runs every 5 minutes and checks the latest DexScreener listings.",
            parse_mode="Markdown",
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("🔄 Toggle ON/OFF",       callback_data="sniper_toggle")],
                [InlineKeyboardButton("⚙️ Min Score",           callback_data="sniper_cfg_score"),
    ...
    elif cb == "sniper_toggle":
        ud["sniper_mode"] = not ud.get("sniper_mode", False)
        msg = t(ud, "sniper_on") if ud["sniper_mode"] else t(ud, "sniper_off")
        await q.edit_message_text(msg, parse_mode="Markdown", reply_markup=back_main())

    elif cb == "sniper_cfg_score":
        pending[u.id] = {"action": "sniper_score"}
        await q.edit_message_text("🎯 Enter minimum AURACLE score (0–100):\nExample: 65", reply_markup=cancel_kb())

    elif cb == "sniper_cfg_liq":
        pending[u.id] = {"action": "sniper_liq"}
        await q.edit_message_text("💧 Enter minimum liquidity in USD:\nExample: 75000", reply_markup=cancel_kb())

    elif cb == "sniper_cfg_mc":
        pending[u.id] = {"action": "sniper_mc"}
        await q.edit_message_text(
            "📊 Enter MC range:\nFormat: min_mc max_mc\nExample: 100000 5000000\n\nThis means tokens between $100K and $5M MC.",
            reply_markup=cancel_kb()
        )

    elif cb == "sniper_cfg_age":
        pending[u.id] = {"action": "sniper_age"}
        await q.edit_message_text("⏰ Enter max token age in hours:\nExample: 24\n\n(Only snipe tokens newer than this)", reply_markup=cancel_kb())

    elif cb == "sniper_cfg_amt":
        pending[u.id] = {"action": "sniper_amt"}
        await q.edit_message_text("💵 Enter buy amount per snipe in USD:\nExample: 150", reply_markup=cancel_kb())
    ...
    # ── CSV EXPORT ─────────────────────────────────────────────────────────────
        ...
        await q.edit_message_text("📁 Generating your trade history CSV...", reply_markup=back_main())
