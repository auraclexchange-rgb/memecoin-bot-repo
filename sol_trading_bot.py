...
async def _broadcast_to_channel(bot, channel_id: int, info: dict, sc: dict, ai: dict, contract: str) -> bool:
    ...
    try:
        ...
        await bot.send_message(
            ...
            parse_mode="Markdown",
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("🔍 View Token Live", url=gt_url)]
            ])
        )
        return True
    except Exception as e:
...
async def text_handler(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if p:
        ...
        if not action:
            ...
            p = None
        elif len(text) > 30 and action not in ("set_balance", "comp_bet", "comp_join", "acc_new", "sniper_channel_input"):
...
async def btn(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # Advisory confirm / skip
        ...
        report = _ai_report_text(info2, sc2, ai2)
        kb_rows = []
