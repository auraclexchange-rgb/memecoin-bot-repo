            ...
            if not rec.get("pump_alerted") and x_vs_exit >= 1.5:
                ...
                try:
                    await app.bot.send_message(
                        chat_id=uid, parse_mode="Markdown",
                        text=(
                            "\U0001f4ca *POST-EXIT INSIGHT*\n\n"
                            "*$" + rec.get("symbol","?") + "* pumped after APEX sold\n\n"
                            "Exit reason: *" + rec.get("exit_reason","?").replace("apex_","").replace("_"," ") + "*\n"
                            "Exited at: *" + str(rec.get("exit_x","?")) + "x*\n"
                            "Now (" + age_label + " later): *" + str(round(x_vs_entry,2)) + "x vs entry*\n"
...
def apex_watchlist_reversal(info: dict, rec: dict) -> tuple:
    ...
    All 4 conditions must be true simultaneously:
      1. Price bounced 15%+ from the lowest point seen after exit
      2. Buy pressure recovered above 52% on M5
      3. Liquidity still within 15% of what it was at exit (not being drained)
      4. Volume alive (>= 50% of hourly average baseline)
    """
    cur_price  = info.get("price", 0)
    buy_pct_m5 = info.get("buy_pct_m5", info.get("buy_pct", 50))
    liq        = info.get("liq", 0)
