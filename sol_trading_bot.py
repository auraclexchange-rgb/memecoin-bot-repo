...
def token_card(info: dict, contract: str, ud: dict, sc: dict = None) -> str:
    ...
    if held:
        ...
        # Avg cost expressed as MC at buy time
        avg_mc = h.get("avg_cost_mc", 0)
        avg_mc_line = ("  |  Avg MC: *" + mc_str(avg_mc) + "*") if avg_mc else ""
        sold_total  = h.get("total_sold", 0)
        sold_line   = ("\n💸 Sold: *" + money(sold_total) + "*") if sold_total > 0 else ""
        pos_line = (
            "\n━━━━━━━━━━━━━━━━\n"
            "📌 *YOUR POSITION*\n"
            "💼 Balance: *" + money(cv) + "*  |  *" + str(round(cx, 2)) + "x*\n"
            + pnl_e + " PnL: *" + pstr(ppnl) + "*\n"
    ...
    try:
        # ── Security / audit line ─────────────────────────────────────────────
        top10   = info.get("top10_pct", None)
        insider = info.get("insider_pct", None)
        no_mint = info.get("no_mint", None)
        bl      = info.get("blacklist", None)
        burn    = info.get("burn_pct", None)
        sec_parts = []
        if top10    is not None: sec_parts.append("Top10: *" + str(top10) + "%*")
        if insider  is not None: sec_parts.append("Insiders: *" + str(insider) + "%*")
        if no_mint  is not None: sec_parts.append("✅ No Mint" if no_mint else "🚨 Mint")
        if bl       is not None: sec_parts.append("✅ Blacklist" if bl else "⚠️ No BL")
        if burn     is not None: sec_parts.append("🔥 Burn *" + str(burn) + "%*")
        sec_line = ("\n━━━━━━━━━━━━━━━━\n🔒 *Audit:* " + "  |  ".join(sec_parts)) if sec_parts else ""
        ...
        return (
            ...
            + "  💧 *Liq:* " + money(info.get("liq",0)) + " (" + str(info.get("liq_pct",0)) + "%)\n"
            ...
            + "━━━━━━━━━━━━━━━━\n"
            + "📈 *Vol 24h:* " + money(info.get("vol_h24",0)) + "  ⚡ *" + pressure + "*\n"
