...
async def get_token(contract: str, force: bool = False) -> dict | None:
    ...
    try:
        ...
        if not header_image and chain_id_raw in ("solana", "sol"):
            header_image = f"https://dd.dexscreener.com/ds-data/tokens/solana/{best.get('baseToken',{}).get('address','')}/header.png"
        twitter = ""
        telegram = ""
        website  = ""
...
def token_card(info: dict, contract: str, ud: dict, sc: dict = None) -> str:
    ...
    try:
        ...
        # ── RugCheck rate-limited fallback — show placeholder instead of silence ──
        _rc_rate_limited_flag = info.get("rc_rate_limited", False)
        _is_solana_token      = str(info.get("chain", "")).lower() in ("solana", "sol")
...
async def send_token_card(
    ...
    if is_query:
        ...
        # Try to send banner image first (same style as AURACLE/other scanners).
        # Uses DexScreener header image if available. Silently skips if 404/error.
        ...
        if header_url:
            try:
                ...
                pass   # no banner available — continue to text card
