                        ...
                        if len(history) >= 6:
                            ...
                            if len(prices) >= 6:
                                ...
                                if _sa_score >= 4:
                                    ...
                                    _notes_display = "\n".join(_sa_notes[:4]) if _sa_notes else "See logs for detail"
                                    try:
                                        await app.bot.send_message(
                                            chat_id=uid, parse_mode="Markdown",
                                            text=(
                                                ...
                                                "─────────────────────\n"
                                                + _notes_display + "\n\n"
                                                "✅ *Decision: RETRACE — holding " + str(_hold_dur) + "s*\n"
                                                "_Score: " + str(_sa_score) + "/15 — uptrend intact_"
                                            ),
                                            reply_markup=main_menu_kb()
                                        )
                                    except Exception:
                                    ...
                                    # Structure confirms breakdown — skip Helius, exit faster
...
async def btn(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # ── APEX MANUAL CONTROL (from token card button) ──────────────────────────
        ...
        ladder_1_3_pct  = round(ep["ladder_1_3x_pct"] * 100)
        ...
        text = (
            ...
            "🔒 At *1.3x* — sells " + str(ladder_1_3_pct) + "% to lock break-even\n"
            "💰 At *2x* — sells " + str(ladder_2x_pct) + "% (capital returned)\n"
            "📈 At *1.5x+* — trail stop activates (" + str(trail_early_pct) + "% early / " + str(trail_mid_pct) + "% mid)\n"
            "🚨 If dump/rug detected — emergency exit\n"
            "🔍 If price dips — checks if it's a retrace or collapse\n\n"
            "_Tap a buy button below — APEX takes over the moment you're in._"
        )
        kb = InlineKeyboardMarkup([
