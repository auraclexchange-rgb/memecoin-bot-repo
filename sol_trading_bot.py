...
def _card_stats_block(info: dict, contract: str, first_scan_line: str = "") -> str:
    """
    ...
    # ── Age display ───────────────────────────────────────────────────────────
    if age_h < 1:    age_display = str(round(age_h * 60)) + "m"
    elif age_h < 24: age_display = str(round(age_h, 1)) + "h"
    else:            age_display = str(round(age_h / 24, 1)) + "d"
    ...
    # ── X search ─────────────────────────────────────────────────────────────
    # 4 search links: All (combined), CA, Name, $Ticker
    # Parens in URL must be percent-encoded so Telegram Markdown doesn't break
    ...
    # Combined query — encode parens so they don't break Markdown link parsing
...
def group_token_card(info: dict, contract: str) -> str:
    ...
    Same design as bot card but no position block and no user-specific data.
    """
    name      = str(info.get("name", "Unknown"))
    symbol    = str(info.get("symbol", "???"))
    chain     = str(info.get("chain", "SOL")).upper()
...
async def _check_challenge(bot, uid: int, ud: dict, daily: bool = False) -> None:
    ...
    Equity = balance + manual open positions + savings.

    Duration fix: we store an explicit end_date (ISO date string) so the
    challenge runs for exactly the number of days chosen, measured from
    midnight-to-midnight, regardless of what time of day it was started.
    """
    ch = ud.get("challenge")
    if not ch or ch.get("ended"):
        return

    # ── Equity (savings included; vault excluded) ─────────────────────────────
    manual_invested = sum(
        h.get("total_invested", 0)
        for h in ud.get("holdings", {}).values()
        if h.get("mood") not in ("APEX", "AI-Sniper", "APEX-DCA")
    ...
    # ── Duration: use explicit end_date if present, else migrate on-the-fly ──
    # Old challenges only stored "started" (a datetime ISO string).
    # New challenges also store "end_date" (a date ISO string = started.date + days).
    # This way remaining days are always calculated correctly from midnight.
    ...
    if ch.get("end_date"):
        ...
        # Migrate: compute end_date from started datetime
        try:
            started_dt = datetime.fromisoformat(ch["started"])
            start_date = started_dt.date()
        except Exception:
            start_date = today
        # end_date is inclusive: a 30-day challenge started today ends on day 29
    ...
    remaining    = (end_date - today).days  # 0 on the last day, negative after
    ...
    # ── 4. Time expired — only fires the day AFTER end_date ──────────────────
    # remaining < 0 means today is past the end date
    if remaining < 0:
        ch["ended"]      = True
    ...
    if daily:
        ...
        today_manual = [
            ...
            and (lambda v: (v.date() if isinstance(v, datetime) else
                 (datetime.fromisoformat(v).date() if isinstance(v, str) else None))
                 )(t.get("closed_at")) == today
        ]
        ...
        pace_emoji = "🟢" if days_elapsed > 0 and progress_pct_clamped / days_elapsed * days_total >= 100 else "🟡" if progress_pct_clamped > 30 else "🔴"
...
async def _broadcast_to_channel(bot, channel_id: int, info: dict, sc: dict, ai: dict, contract: str, uid: int = 0) -> bool:
    ...
    try:
        ...
        # disable_web_page_preview=False so hidden image link renders
        _has_img = "\u200b" in text
        msg = await bot.send_message(
            chat_id=channel_id, text=text, parse_mode="HTML",
            reply_markup=InlineKeyboardMarkup(rows),
            disable_web_page_preview=not _has_img,
        )
            ...
            for uid, ud in active_users:
                try:
                    ...
                    async def _safe_tw():
                        try:
                            _hc = await get_http()
                            return await enrich_twitter_momentum(info, _hc)
...
async def btn(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # ── CHALLENGE MODE ─────────────────────────────────────────────────────
        ...
        if ch and not ch.get("ended"):
            ...
            # Resolve end_date (migrate old challenges on-the-fly)
            if ch.get("end_date"):
            ...
            progress_bar= "█" * bar_filled + "░" * (10 - bar_filled)
            # Check expiry on open (only after end_date has passed)
            if remaining < 0:
