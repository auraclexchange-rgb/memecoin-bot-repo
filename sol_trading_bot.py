...
def token_card(info: dict, contract: str, ud: dict, sc: dict = None) -> str:
    ...
    try:
        ...
        if ath_price and ath_price > price and price > 0:
            ath_down_pct = round((1 - price / ath_price) * 100, 1)
            ath_line = (
                "🏆 ATH  $" + str(ath_price)
                + ("  |  " + mc_str(ath_mc) + " MC" if ath_mc else "")
                + "  (" + str(ath_down_pct) + "% from ATH)\n"
            )

        # ── pressure bar (text) ───────────────────────────────────────────────
        filled  = round(buy_pct / 10)        # 0-10 blocks
        empty   = 10 - filled
        bar     = "█" * filled + "░" * empty
        if buy_pct >= 60:
        ...
        # ── liq warning ───────────────────────────────────────────────────────
        liq_warn = "\n🚨 WARNING: LOW LIQUIDITY — HIGH RISK" if liq < 50_000 else ""
        ...
        if contract in ud.get("holdings", {}):
            ...
            sold_total= h.get("total_sold", 0)
            pos_block = (
                "┌─────────────────────────┐\n"
                "│  📌 YOUR POSITION\n"
                "│  Balance  " + money(cv).ljust(10) + "  " + cx_e + " " + str(round(cx, 2)) + "x\n"
                "│  " + pnl_e + " PnL     " + pstr(ppnl) + "\n"
                "│  Bought   " + money(h.get("total_invested", 0)) + "\n"
                + ("│  Sold     " + money(sold_total) + "\n" if sold_total > 0 else "")
                + "│  Holding  " + str(round(h["amount"], 4)) + " tokens\n"
        ...
        if sc:
            ...
            verdict = sc["verdict"]
            if score >= 70:   score_e = "🟢"
            elif score >= 45: score_e = "🟡"
            else:             score_e = "🔴"
            sc_block = (
                "┌─────────────────────────┐\n"
                "│  " + score_e + " AURACLE SCORE: " + str(score) + "/100\n"
                "│  " + verdict + "\n"
            )
            if sc.get("strengths"):
                sc_block += "│\n│  💪 Strengths:\n"
        ...
        return (
            ...
            + "💲 Price   *$" + str(price) + "*\n"
            + "📊 MC      *" + mc_str(mc) + "*\n"
            + "💧 Liq     *" + money(liq) + "*  (" + str(liq_pct) + "% of MC)\n"
            + age_e + "  Age     *" + age_display + "*\n"
            + (ath_line if ath_line else "")
            + "─────────────────────────\n"
...
async def send_token_card(
    ...
    """
    Send (or edit) a token card with the price chart attached as a photo
    and the token card text as the caption.
    Falls back to plain text if chart generation fails.
    """
    ...
    if len(card_txt) > 1024:
        card_txt = card_txt[:1020] + "…"
    ...
    try:
        ...
        logger.warning(f"Chart generation failed in send_token_card: {e}")

    if chart_buf:
        if is_query:
            # Can't edit a text message into a photo — delete old and send new photo
            try:
            ...
            # New message context — delete "Scanning..." stub if present
        ...
        # No chart — fall back to plain text message
        if is_query:
            await target.edit_message_text(card_txt, parse_mode="Markdown", reply_markup=kb)
        else:
            await target.reply_text(card_txt, parse_mode="Markdown", reply_markup=kb)
...
def _ai_report_text(info: dict, sc: dict, ai: dict) -> str:
    ...
    # Sniper score hard flags
    hard_flags_line = ""
    if sc.get("flags"):
        hard_flags_line = "\n🚨 *Hard Flags:*\n" + "\n".join("  " + f for f in sc["flags"]) + "\n"

    # Sniper score strengths
    strengths_line = ""
    if sc.get("strengths"):
    ...
    return (
        ...
        "⏱ Age: *" + str(round(info.get("age_h",0), 1)) + "h*  |  Liq: *" + money(info.get("liq",0)) + "* (" + str(info.get("liq_pct",0)) + "% of MC)\n\n"
        + verdict_emoji + " *Verdict: " + ai["verdict"] + "*\n"
        "🎯 Confidence: *" + str(ai["confidence"]) + "/10*  `" + conf_bar + "`\n"
        "💵 Suggested Buy: *" + money(ai["suggested_amount"]) + "*\n\n"
        "📝 *Thesis:*\n" + ai["thesis"] + "\n\n"
        "━━━━━━━━━━━━━━━━\n"
        "🧠 *Sniper Score: " + str(sc["score"]) + "/100* — " + sc["verdict"] + "\n"
        + hard_flags_line
        + strengths_line
...
async def text_handler(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    async def _clean(keep_prompt_id: int | None = None):
        ...
        try:
            ...
            pass
        try:
            prompt_id = keep_prompt_id or ud.get("_last_prompt_msg_id")
            if prompt_id:
    ...
    if p and p.get("action", ""):
        ...
        if action == "set_balance":
            ...
            try:
                ...
                pending.pop(u.id, None)
                await _clean()
                msg = "🔔 Alert set: notify when price goes *" + direction + "* " + money(target)
                if prompt_id:
                    await ctx.bot.edit_message_text(chat_id=u.id, message_id=prompt_id,
                        text=msg, parse_mode="Markdown",
                        reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("◀ Back to Token", callback_data="btt_" + contract)]]))
                else:
                    await message.reply_text(msg, parse_mode="Markdown", reply_markup=back_main())
            except Exception:
                await message.reply_text("❌ Enter a price like 0.015", reply_markup=cancel_kb())
            return
...
async def btn(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # ── COMPETITION BETS UPDATE ────────────────────────────────────────────
        ...
        pending[u.id] = {"action": "wl_target_price", "contract": pending.get(u.id, {}).get("contract", ""), "_prompt_msg_id": q.message.message_id}
        await q.edit_message_text("👁 Enter target PRICE to alert:\nExample: 0.00005", reply_markup=cancel_kb())

    elif cb == "wl_add_mc":
        pending[u.id] = {"action": "wl_target_mc", "contract": pending.get(u.id, {}).get("contract", ""), "_prompt_msg_id": q.message.message_id}
    ...
    # ── SNIPER MODE v2 ─────────────────────────────────────────────────────────
        ...
        # Show last 10 as clickable buttons
    ...
    # ── DCA BY MARKET CAP ──────────────────────────────────────────────────────
    elif cb.startswith("dca_") and not cb.startswith("dca_cancel_") and not cb.startswith("dca_confirm_") and not cb.startswith("dca_addmore_") and not cb.startswith("dca_setmc_") and not cb.startswith("dca_setamt_") and cb != "v_dca":
        contract = cb[4:]
        info = await get_token(contract)
        sym    = info["symbol"] if info else "?"
