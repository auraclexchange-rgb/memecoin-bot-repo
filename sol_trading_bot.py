...
def get_user(uid: int, uname: str) -> dict:
    if uid not in users:
        users[uid] = {
            ...
            "apex_learn_score_min":   55,   # raised from 45 — only quality setups
        ...
        for contract, call_data in list(calls.items()):
            try:
                ...
                while current_x >= next_milestone and posted < 3:
                    # Header style scales with achievement
                    if next_milestone >= 100:
                        ...
                        rockets = "🚀" * min(next_milestone, 5)
                        header  = f"{rockets} {next_milestone}x {rockets}"
...
def main():
    ...
    for _uid, _ud in users.items():
        ...
        if _ud.get("apex_learn_threshold", 7) < 7:
            ...
            _changed = True
        if _ud.get("apex_learn_score_min", 55) < 55:
            _ud["apex_learn_score_min"] = 55
            _changed = True
