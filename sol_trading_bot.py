...
def token_card(info: dict, contract: str, ud: dict, sc: dict = None) -> str:
    ...
    # Build 3 separate X search links
    import urllib.parse
    name   = str(info.get("name",""))
    symbol = str(info.get("symbol",""))
    q_ca     = urllib.parse.quote(contract)
    q_name   = urllib.parse.quote(name)
    q_symbol = urllib.parse.quote("$" + symbol)
    base_url = "https://x.com/search?q={}&src=typed_query&f=live"
    x_ca     = f"[CA]({base_url.format(q_ca)})"
    x_name   = f"[{name[:15]}]({base_url.format(q_name)})"
    x_symbol = f"[${symbol}]({base_url.format(q_symbol)})"
    x_line   = f"\n━━━━━━━━━━━━━━━━\n🔍 *Search 𝕏:* {x_ca}  •  {x_name}  •  {x_symbol}"
        ...
        for code in my_codes:
            ...
            for uid_str, m in members.items():
                try:
                    ...
                    if m_ud:
                        curr_eq = m_ud["balance"] + sum(h["total_invested"] for h in m_ud["holdings"].values())
                    else:
                        curr_eq = m.get("start_eq", m.get("start_bal", 0))
                    start = m.get("start_eq", m.get("start_bal", 0))
                    pct = round((curr_eq - start) / start * 100, 1) if start > 0 else 0
                    rankings.append((m.get("username","?"), curr_eq, pct))
                except Exception:
                    pass
            ...
            txt += "🏆 *Leaderboard:*\n"
            medals = ["🥇","🥈","🥉"]
            for i, (name, eq, pct) in enumerate(rankings[:5]):
                medal = medals[i] if i < 3 else f"{i+1}."
                sign = "+" if pct >= 0 else ""
                txt += f"  {medal} @{name}  {money(eq)}  ({sign}{pct}%)\n"
            txt += "\n"
        await q.edit_message_text(
