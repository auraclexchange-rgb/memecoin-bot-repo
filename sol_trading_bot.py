...
def _channel_card_text(info: dict, sc: dict, ai: dict, contract: str) -> str:
    ...
    # ── Socials (only rendered when links exist) ──────────────────────────
    soc_parts = []
    if info.get("twitter"):
        soc_parts.append("🐦 Twitter")
    if info.get("telegram"):
        soc_parts.append("✈️ Telegram")
    if info.get("website"):
        soc_parts.append("🌐 Website")
    socials_line = ("🔗 " + "  ·  ".join(soc_parts) + "\n") if soc_parts else ""

    # ── Search footer ─────────────────────────────────────────────────────
    search_line = f"🔍  [𝕏 ${symbol}](https://x.com/search?q=%24{symbol})  ·  [CA](https://dexscreener.com/search?q={contract})  ·  [Name](https://www.google.com/search?q={symbol}+crypto)  ·  [BirdEye](https://birdeye.so/find-gems?type=token&symbol={symbol})"
...
def _channel_card_kb(
    ...
    rows.append([InlineKeyboardButton("🔍 View Token Live ↗", url=gt_url)])
    return InlineKeyboardMarkup(rows)
...
async def _broadcast_to_channel(bot, channel_id: int, info: dict, sc: dict, ai: dict, contract: str, uid: int = 0) -> bool:
    ...
    try:
        ...
        kb        = _channel_card_kb(contract, gt_url, bot_url, info, expanded=False)
...
async def btn(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # aic_<contract>  = collapse (hide AI panel)
        ...
        contract  = cb[5:]       # strip "aiex_" or "aic_"
        ...
        kb = _channel_card_kb(contract, gt_url, bot_url, info_c, expanded=expanding)
        try:
            await q.edit_message_text(
                full_text,
                parse_mode="Markdown",
