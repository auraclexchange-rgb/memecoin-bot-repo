...
APEX_TRAIL_PCT_EARLY    = 0.15   # 1.5x-2x:  was 0.18 — tightened, protect early gains
                ...
                try:
                    ...
                    if ud.get("apex_mode") and _apex_verdict_ok:
                        ...
                        if _apex_phase == "learning":
                            ...
                            elif apex_is_daily_loss_halted(ud):                  _ok = False  # halt learning too if vault is wiped
...
async def apex_process_entry_queue(app, uid: int, ud: dict) -> None:
    ...
    async with _get_user_lock(uid):
      for contract, entry in list(queue.items()):
        ...
        h["apex_threat"]       = "CLEAR"
        h["apex_vault_locked"] = {}
        # Hunter: write the Phase-0 floor immediately at entry using the SL pct.
        # Without this, apex_hunter_floor is 0.0 until cx >= ratchet_1x, meaning
        # the first position manager cycle has no floor and falls back to the raw
        # SL pct block — which is fine, but the floor check fires on 0.0 and
        # skips the hunter_floor_breach path entirely on a fast dump.
        if _hp:
            _entry_floor = info_post["price"] * (1.0 - _apex_sl / 100.0)
            h["apex_hunter_floor"] = round(_entry_floor, 10)
        else:
