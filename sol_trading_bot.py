...
def _mtv_block(info: dict) -> str:
    """Compact 3-row M·T·V block."""
    def _dot(p): return "🟢" if p>=60 else ("🟡" if p>=52 else "🔴")
    def _vol(v):
        if v>=1_000_000: return f"${v/1_000_000:.1f}M"
        if v>=1_000:     return f"${v/1_000:.1f}K"
        return f"${v:.0f}"
...
def _ai_report_text(info: dict, sc: dict, ai: dict, contract: str = "", expanded: bool = True) -> str:
    """Channel card v3. No header. Score in holder block. Socials/X inline. AI at footer."""
    ...
    no_mint   = info.get("no_mint")
    no_freeze = info.get("no_freeze")
    lp_str    = (f"LP: <b>{lp_burn}%</b> 🔥") if lp_burn is not None else "LP: —"
    mint_str  = ("✅ No Mint" if no_mint else "🚨 Mint!") if no_mint is not None else ""
    frz_str   = ("✅ No Freeze" if no_freeze else "⚠️ Freeze") if no_freeze is not None else ""

    # Pump.fun
    pf_curve = info.get("pf_curve")
    pf_grad  = info.get("pf_graduated", False)
    pf_rep   = info.get("pf_replies", 0) or 0
    pf_line  = ""
    if pf_curve is not None:
        grad    = " 🎓 <b>GRADUATED</b>" if pf_grad else f" Curve: <b>{pf_curve}%</b>"
        rep     = f"   💬 {pf_rep}" if pf_rep > 0 else ""
        pf_line = "🟣 Pump.fun:" + grad + rep + "\n"
    ...
    sniper_cnt = info.get("sniper_count")

    def _flag(v, warn, danger):
        if v is None: return "—"
        v = float(v)
    ...
    r1 = []
    if maker_n    is not None: r1.append(f"👥 Holders: <b>{maker_n}</b>")
    if top10      is not None: r1.append(f"Top10: {_flag(top10, 25, 40)}")
    if top20      is not None: r1.append(f"Top20: {_flag(top20, 35, 55)}")
    if sniper_cnt is not None: r1.append(f"🎯 Snipers: <b>{sniper_cnt}</b>")
    r2 = []
    if insider is not None: r2.append(f"🐀 Insiders: {_flag(insider, 5, 15)}")
    r2.append(f"👤 DEV: <b>{dev_pct}%</b>" if dev_pct is not None else "👤 DEV: —")
    r2.append("⚡ DEX PAID: " + ("✅" if boost >= 1 else "❌"))

    holder_lines = ""
    if r1: holder_lines += "  ".join(r1) + "\n"
    if r2: holder_lines += "  ·  ".join(r2) + "\n"

    holder_block = (
        "▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬\n"
        "👨\u200d👩\u200d👧 <b>Holder Intel</b>\n"
        + holder_lines
        + f"🎯 Score: <b>{score}/100</b>  <code>{score_bar}</code>  {score_tag}\n"
        "▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬\n"
    )
    ...
    # Inline socials
    soc_links = []
    if info.get("twitter"):  soc_links.append(f'<a href="{info["twitter"]}">🐦 Twitter</a>')
    if info.get("telegram"): soc_links.append(f'<a href="{info["telegram"]}">✈️ Telegram</a>')
    if info.get("website"):  soc_links.append(f'<a href="{info["website"]}">🌐 Website</a>')
    socials_line = ("🔗 " + "  ·  ".join(soc_links) + "\n") if soc_links else ""

    # Inline X search
    name     = info.get("name", symbol)
    x_base   = "https://twitter.com/search?q="
    x_links  = (
        f'<a href="{x_base}%24{symbol}">𝕏 ${symbol}</a>'
        f'  ·  <a href="{x_base}{contract}">CA</a>'
        f'  ·  <a href="{x_base}{name.replace(" ", "%20")}">Name</a>'
        f'  ·  <a href="{x_base}{symbol}">Symbol</a>'
    )
    x_line = "🔍 " + x_links + "\n"

    # AI section at footer
    ai_section = ""
    if expanded:
        ...
        ai_section = (
            ...
            + f"🚩 <b>Red Flags:</b>\n{red_l}\n\n"
            + f"💚 <b>Green Flags:</b>\n{grn_l}\n\n"
            + f"{rug_icon} Rug Risk: <b>{rug_risk}</b>   {mom_icon} Momentum: <b>{momentum}</b>\n\n"
            + f"💸 <b>Suggested Entry: {money(ai.get('suggested_amount', 0))}</b>\n"
        )

    return (
        f"🪙 <b>${symbol}</b>  ·  {chain}  ·  {verdict_icon} <b>{verdict}</b>\n"
        + "\n"
        ...
        + lp_str
        + ("   " + mint_str  if mint_str  else "")
        + ("   " + frz_str   if frz_str   else "") + "\n"
        + pf_line
        + holder_block
        + "\n"
        + mtv
...
async def _broadcast_to_channel(bot, channel_id: int, info: dict, sc: dict, ai: dict, contract: str, uid: int = 0) -> bool:
    ...
    try:
        ...
        def _build_kb(expanded: bool) -> InlineKeyboardMarkup:
            ...
            label = "🧠 Collapse AI ▲" if expanded else "🧠 Show AI Analysis ▼"
        ...
        for item in unique_items:
            ...
            for uid, ud in active_users:
                try:
                    ...
                    # 2. Score threshold (tighter default: 60)
                    elif sc["score"] < int(sf.get("min_score", 60)):
                        skip_reason = f"Score too low ({sc['score']}/100)"

                    # 3. Liquidity — pump.fun tokens still need $5K minimum (bonding curve has some liq)
                    elif is_pumpfun and liq < 5_000 and pf_curve_val < 10:
                        skip_reason = f"Pump.fun too early — liq ${liq:,.0f} / curve {pf_curve_val}%"
                    elif not is_pumpfun and liq < float(sf.get("min_liq", 15_000)):
                        skip_reason = f"Liq too low (${liq:,.0f})"

                    # 4. MC range
                    elif not (float(sf.get("min_mc", 20_000)) <= mc <= float(sf.get("max_mc", 3_000_000))):
                        skip_reason = f"MC out of range ({mc_str(mc)})"

                    # 5. Age
                    elif age_h > float(sf.get("max_age_h", 5)):
                        skip_reason = f"Too old ({round(age_h,1)}h)"

                    # 6. Activity — pump.fun tokens need at least 20 buys (not fully exempt anymore)
                    elif buys_h1 < int(sf.get("min_buys_h1", 30)) and (not is_pumpfun or buys_h1 < 20):
                        skip_reason = f"Low activity H1 ({buys_h1} buys)"

                    # 7. Buy pressure — tightened to 55%
                    elif buy_pct_h1 < int(sf.get("min_buy_pct", 55)):
                        skip_reason = f"Sell pressure H1 ({buy_pct_h1}% buys)"

                    # 8. Wash trading — tightened to 5x
                    elif vol_mc_ratio > float(sf.get("max_vol_mc_ratio", 5.0)):
                    ...
                    # 10. Too few unique holders (if data available)
...
async def btn(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # ── CHANNEL CARD AI TOGGLE ────────────────────────────────────────────────
        ...
        if not card:
            try: await q.answer("Card expired — bot restarted. New cards will work.", show_alert=True)
        ...
        channel_id = card["channel_id"];  message_id = card["message_id"]
        bot_url    = card.get("bot_url");  view_url = card.get("view_url") or card.get("dex_url","")
        ...
        try:
            ...
            try: await q.answer("Couldn't update.", show_alert=False)
            except Exception: pass
