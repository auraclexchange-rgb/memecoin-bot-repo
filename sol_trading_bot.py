...
APEX_DAILY_LOSS_LIMIT      = 0.25    # raised from 0.20 — one rug was triggering 20h halt
APEX_SINGLE_TRADE_MAX_LOSS = 150.0   # max $ loss on any single trade before large-loss counter increments
APEX_CONFIRM_WAIT_S     = 45
APEX_DRAWDOWN_1_MULT    = 0.70
APEX_DRAWDOWN_2_MULT    = 0.50
APEX_DRAWDOWN_3_PAUSE   = 30
APEX_MIN_CONFIDENCE     = 6    # minimum AI confidence to enter
...
APEX_PROFILES = {
    ...
    "hunter": {
        ...
        "trail_activate_x":  2.0,    # trail only after capital returned at 2x
...
# ══════════════════════════════════════════════════════════════════════════════
# MARKET WAVE PHASE DETECTION
# Liquidity wave cycle: SILENCE → STIRRING → WAVE → CRASH → RESET
# ══════════════════════════════════════════════════════════════════════════════
WAVE_VOL_HIGH_MULT     = 1.5    # vol > 1.5x baseline = HIGH regime (wave active)
WAVE_VOL_LOW_MULT      = 0.6    # vol < 0.6x baseline = LOW regime (silence)
WAVE_PHASE_CYCLES      = 6      # rolling baseline window (6 cycles = 30 min)

# ══════════════════════════════════════════════════════════════════════════════
# RECOVERY SCORING — post-migration tokens with bounce potential
# ══════════════════════════════════════════════════════════════════════════════
RECOVERY_PICO_DROP_MIN  = 0.60  # must have dropped 60%+ from ATH to qualify
RECOVERY_BOUNCE_MIN     = 0.05  # must be bouncing 5%+ from pico bottom

# ══════════════════════════════════════════════════════════════════════════════
# NARRATIVE POSITION SCORING
# ══════════════════════════════════════════════════════════════════════════════
NARRATIVE_FIRST_BONUS   = 20    # score bonus: first token in narrative = banger
NARRATIVE_SECOND_BONUS  = 5     # small bonus: second token = maybe
NARRATIVE_LATE_PENALTY  = 20    # score penalty: 3rd+ token = likely rug

# ══════════════════════════════════════════════════════════════════════════════
# EXIT PARAMETERS PER WAVE PHASE
# APEX position manager reads these every cycle and adjusts trail/ladder behaviour
# ══════════════════════════════════════════════════════════════════════════════
APEX_EXIT_PARAMS = {
    "WAVE": {
        # Jeets active — extract fast, tight trails, aggressive ladders
        "trail_activate_x": 1.3,
        "trail_pct_early":  0.12,
        "trail_pct_mid":    0.08,
        "trail_pct_high":   0.06,
        "trail_pct_moon":   0.05,
        "ladder_1_3x_pct":  0.35,   # sell 35% at 1.3x (was 25%)
        "ladder_2x_pct":    0.35,
        "dca_enabled":      False,  # no DCA — jeets dump into your average
        "vol_exit_mult":    4.0,    # sell remainder when vol spikes 4x
        "momentum_decay_h": 0.5,    # exit dead hold after 30 min
    },
    "STIRRING": {
        # Wave forming — standard settings
        "trail_activate_x": 1.5,
        "trail_pct_early":  0.22,
        "trail_pct_mid":    0.12,
        "trail_pct_high":   0.10,
        "trail_pct_moon":   0.07,
        "ladder_1_3x_pct":  0.25,
        "ladder_2x_pct":    0.25,
        "dca_enabled":      True,
        "vol_exit_mult":    4.0,
        "momentum_decay_h": 1.0,
    },
    "SILENCE": {
        # Accumulation — patience mode, wide trails, slow exits
        "trail_activate_x": 2.0,
        "trail_pct_early":  0.30,
        "trail_pct_mid":    0.20,
        "trail_pct_high":   0.15,
        "trail_pct_moon":   0.10,
        "ladder_1_3x_pct":  0.10,   # small 1.3x sell — protect position size
        "ladder_2x_pct":    0.15,
        "dca_enabled":      True,   # dips are real support — DCA allowed
        "vol_exit_mult":    3.0,    # lower threshold — sell into wave when it arrives
        "momentum_decay_h": 3.0,    # hold dead positions up to 3h
    },
    "CRASH": {
        # Market crashing — emergency tight settings
        "trail_activate_x": 1.3,
        "trail_pct_early":  0.08,
        "trail_pct_mid":    0.06,
        "trail_pct_high":   0.05,
        "trail_pct_moon":   0.04,
        "ladder_1_3x_pct":  0.50,
        "ladder_2x_pct":    0.50,
        "dca_enabled":      False,
        "vol_exit_mult":    2.0,
        "momentum_decay_h": 0.25,
    },
    "RESET": {
        # Market stabilising — standard settings, no new entries
        "trail_activate_x": 1.5,
        "trail_pct_early":  0.22,
        "trail_pct_mid":    0.12,
        "trail_pct_high":   0.10,
...
# ══════════════════════════════════════════════════════════════════════════════
# ADVISORY ENGINE CONSTANTS
# ══════════════════════════════════════════════════════════════════════════════
ADVISORY_MIN_RECOVERY_SCORE    = 45    # minimum recovery score to generate signal
ADVISORY_MAX_SIGNALS_PER_CYCLE = 3     # max signals posted per advisory cycle
ADVISORY_SIGNAL_EXPIRY_H       = 4.0   # Mode 2 approval expires after 4h
...
_token_cache: dict = {}
CACHE_TTL = 20.0   # seconds — raised from 12s to reduce DexScreener 429s on position checks
...
# ── Helius API result cache (10 min TTL) ──────────────────────────────────────
# Helius maker/buyer data doesn't change meaningfully in 10 minutes.
# Without this, the same contract gets queried every 5-min sniper cycle
# that passes pre-filters — the primary cause of credit drain in production.
_helius_maker_cache: dict = {}
HELIUS_MAKER_CACHE_TTL = 600.0   # 10 minutes

# ── Ticker origin cache (1 hour TTL) ─────────────────────────────────────────
# Stores "FIRST" or "COPY" per contract. Origin never changes so 1h is safe.
_ticker_origin_cache: dict = {}
...
_market_wave_phase: str   = "RESET"   # SILENCE / STIRRING / WAVE / CRASH / RESET
_market_vol_history: list = []        # rolling avg vol per cycle (for baseline calc)

# ── NARRATIVE FIRST-TOKEN TRACKING ───────────────────────────────────────────
_narrative_first_tokens: dict = {}    # keyword → first contract seen in that narrative

# ── ADVISORY ENGINE ───────────────────────────────────────────────────────────
_advisory_seen: dict = {}             # uid → {contract → timestamp} dedup per user

# ── MANUAL APEX POSITIONS ─────────────────────────────────────────────────────
# Contracts entered via token card "APEX Control" button — full APEX intelligence
# (wave-aware trail, euphoria exit, break-even floor) applied to these positions.
_apex_manual_positions: set = set()
            ...
            try:
                ...
                if _rc_fresh:
                    ...
                    if rc_r.status_code == 429:
                        if _rc_cached:
                            # Serve stale cache — any age is better than nothing
                            rc_data = _rc_cached["data"]
                            logger.debug(f"RugCheck 429 — serving stale cache for {contract[:12]}")
                        else:
                            # No cache at all — flag so user can see it in sniper log
                ...
                if rc_data:
                    ...
                    if holders:
                        ...
                        # RugCheck tags each holder as: insider, sniper, fresh,
                        # smartMoney, bot, etc. We aggregate by type to get the
                        # combined % held by each category — much more precise
                        # than just checking total insider %.
                        _sniper_pct   = 0.0
                        _fresh_pct    = 0.0
                        _smart_pct    = 0.0
                        _bundle_pct   = 0.0
                        ...
                        for _h in holders:
                            ...
                            if "fresh" in _htags or "new_wallet" in _htags:
                                _fresh_pct    += _hpct
                            # Smart money / known profitable wallets
                            if "smart_money" in _htags or "smartmoney" in _htags or "smart" in _htags:
                                _smart_pct   += _hpct
                                _smart_count += 1
                            # Bundle bags (snipers that got in at launch as a group)
                            if "bundle" in _htags or ("sniper" in _htags and _hins):
                                _bundle_pct  += _hpct
                        # Store extracted wallet intelligence
...
def sniper_score(info: dict) -> dict:
    ...
    elif insider_pct is not None and insider_pct > 15:
        warnings.append(f"⚠️ Insider holds {insider_pct:.1f}% — elevated risk")
    if dev_pct > 40:
        flags.append(f"🚨 DEV wallet holds {dev_pct:.1f}% — extreme dump risk")
    elif dev_pct > 20:
    ...
    if vol_mc_ratio > 15:
        ...
        warnings.append(f"Vol/MC={vol_mc_ratio:.1f}x — elevated (check for bots)")
    if maker_count > 0 and maker_count < 15:
...
def get_user(uid: int, uname: str) -> dict:
    if uid not in users:
        users[uid] = {
            ...
            "apex_learn_score_min":   45,   # minimum sniper score to enter
...
def sell_core(ud: dict, uid: int, contract: str, usd: float, price: float, reason: str = "manual") -> dict:
    ...
    # ── Track cumulative realized PnL on the holding itself ──────────────────
    # Used by trade log on close to report TOTAL pnl across all partial sells,
    # not just the last sell's pnl.
    h["cumulative_realized"] = round(h.get("cumulative_realized", 0.0) + realized, 4)

    # ── Large-loss counter (for daily halt guard) ─────────────────────────────
    # Counts trades exceeding APEX_SINGLE_TRADE_MAX_LOSS independently of the
    # cumulative daily PnL check. Resets only on the daily boundary (in
    # apex_reset_daily), not on individual wins — a $0.01 win should not
    # clear the counter after three $200 losses on the same day.
    if realized < -APEX_SINGLE_TRADE_MAX_LOSS:
        ud["apex_large_loss_count"] = ud.get("apex_large_loss_count", 0) + 1
    ...
    if h["amount"] < 0.000001:
        # total_invested: h["total_invested"] was already reduced by cost above,
        # so add cost back to get the full original invested amount.
        total_invested_full = h["total_invested"] + cost
        total_returned_full = h.get("total_sold", 0.0)
        # total realized PnL across ALL partial sells (not just this last one)
        total_realized_pnl  = h.get("cumulative_realized", realized)
        # x = gross proceeds / gross invested (before fees) for accurate multiplier
        # Use total_invested_full (fees not deducted from cost basis)
        ...
        trade_log.setdefault(uid, []).append({
            ...
            "realized_pnl":  total_realized_pnl,   # total across all partial sells
            "x":             x_val,
...
def token_card(info: dict, contract: str, ud: dict, sc: dict = None) -> str:
    ...
    try:
        ...
        if any(x is not None for x in [no_mint, no_freeze, lp_burn, top10, insider]):
            ...
            _rc_avail      = info.get("no_mint") is not None  # RugCheck responded
            _wi_parts = []
            if insider is not None:
                _ins_icon = "🐭" if insider == 0 else ("🟡" if insider < 15 else "🔴")
                _wi_parts.append(f"{_ins_icon} Insiders: {insider:.0f}%")
            elif _rc_avail:
                _wi_parts.append("🐭 Insiders: 0%")
            else:
                _wi_parts.append("🐭 Insiders: N/A")
            if _wi_sniper_cnt > 0:
                _snp_icon = "🎯" if _wi_sniper_p < 10 else ("🟡" if _wi_sniper_p < 25 else "🔴")
                _wi_parts.append(f"{_snp_icon} Snipers: {_wi_sniper_cnt}")
            if _wi_fresh_p > 0:
                _fr_icon = "🫧" if _wi_fresh_p < 10 else ("🟡" if _wi_fresh_p < 20 else "🔴")
                _wi_parts.append(f"{_fr_icon} Fresh: {_wi_fresh_p:.0f}%")
            if _wi_smart_cnt > 0:
                _wi_parts.append(f"💰 SmartWallets: {_wi_smart_cnt}")
            if _wi_bundle_p >= 5:
                _bdl_icon = "🟡" if _wi_bundle_p < 15 else "🔴"
                _wi_parts.append(f"{_bdl_icon} Bundle: {_wi_bundle_p:.0f}%")
            _wi_line = "  |  ".join(_wi_parts)
            if _wi_fresh_p > 0:
                _fr_icon = "🫧" if _wi_fresh_p < 10 else ("🟡" if _wi_fresh_p < 20 else "🔴")
                _wi_parts.append(f"{_fr_icon} Fresh: {_wi_fresh_p:.0f}%")
            if _wi_smart_cnt > 0:
...
async def send_token_card(
    ...
    """
    1. Delete old token card message (if refresh)
    2. Send token card as text message with all buttons
    Chart intentionally removed — was adding 2–6s latency per open.
    DexScreener button in the card covers charting needs.
    """
    ...
    # ── Step 2: Delete old token card message (refresh case) ────────────────
    # Chart removed: fetch_ohlcv + generate_price_chart added ~2–6s latency
    # per token card open (GeckoTerminal API call + matplotlib render).
    # Token card now opens instantly. DexScreener link in buttons covers charting.
    if is_query:
        try:
    ...
    # ── Step 3: Send fresh token card with buttons ────────────────────────────
...
def buy_kb(contract: str, ud: dict) -> InlineKeyboardMarkup:
    ...
    # APEX Control button — shows management status if already under APEX
    _is_apex_managed = (
        contract in _apex_manual_positions or
        ud.get("holdings", {}).get(contract, {}).get("mood") in ("APEX", "AI-Sniper", "APEX-DCA")
    )
    _apex_ctrl_lbl = "🤖 APEX Managing ✅" if _is_apex_managed else "🤖 APEX Control"
    _apex_ctrl_cb  = "apex_manual_status_" + contract if _is_apex_managed else "apex_manual_" + contract
    ...
    rows = [
        ...
        [InlineKeyboardButton(_apex_ctrl_lbl,      callback_data=_apex_ctrl_cb)],
...
async def run_checker(app: Application):
    ...
    if all_contracts:
        # Batched fetches — firing all at once causes DexScreener 429s
        _cj_contracts = list(all_contracts)
        _cj_batch = 6
        for _cj_i in range(0, len(_cj_contracts), _cj_batch):
            await _asyncio.gather(*[get_token(c, force=True) for c in _cj_contracts[_cj_i:_cj_i+_cj_batch]])
            if _cj_i + _cj_batch < len(_cj_contracts):
    ...
    for uid, ud in list(users.items()):
        ...
        for hca, h in list(ud.get("holdings", {}).items()):
            try:
                ...
                # ── MILESTONE ALERTS — every integer x sequentially ───────────
                # last_milestone_x = highest integer multiple already notified.
                # Starts at 1. Each cycle: announce from last+1 up to floor(current_x).
                # Max 2 DMs per cycle to avoid flooding the user.
                if ud.get("milestone_notif", True):
                    _ms_last   = h.get("last_milestone_x", 1)
                    _ms_next   = _ms_last + 1
                    invested   = h.get("total_invested", 0)
                    cur_value  = h["amount"] * cur_price
                    _ms_posted = 0

                    while current_x >= _ms_next and _ms_posted < 2:
...
async def survival_report_job(ctx: ContextTypes.DEFAULT_TYPE):
    """
    Runs every morning at 9:00 UTC.
    Looks at every token the sniper evaluated 20–28 hours ago and checks
    what actually happened to it — survived, pumped, or rugged.

    Posts a summary to the dump log channel (if set), otherwise DM.

    This tells you:
      • Tokens you SKIPPED that pumped (filters too strict?)
      • Tokens APEX bought that rugged quickly (entry quality)
      • Overall survival rate of tokens the sniper sees
    """
    now_ts = _time.time()
    # Window: tokens scanned between 28h and 20h ago (yesterday's full scan batch)
    window_start = now_ts - (28 * 3600)
    window_end   = now_ts - (20 * 3600)

    for uid, ud in list(users.items()):
        if ud.get("balance") is None:
            continue
        # Only users who have sniper/apex active or a dump log channel set
        if not (ud.get("sniper_advisory") or ud.get("sniper_auto") or
                ud.get("apex_mode") or ud.get("sniper_log_channel")):
            continue
        ...
        # Pull entries from the scan window
        window_entries = []
        seen_contracts = set()
        for entry in log:
            try:
        ...
        # Fetch current data for each token (batched to avoid rate limits)
        survived   = []   # liq still > $3K
        pumped_2x  = []   # MC at least 2x from scan time
        pumped_5x  = []   # MC at least 5x
        rugged     = []   # liq < $1K or price unavailable
        skipped_pumped = []  # skipped by bot but pumped 2x+
        bought_rugged  = []  # bot bought but it rugged

        BATCH = 6
        for i in range(0, len(window_entries), BATCH):
            batch = window_entries[i:i + BATCH]
            infos = await _asyncio.gather(
                *[get_token(e["contract"]) for e in batch],
                return_exceptions=True
            )
            for entry, info in zip(batch, infos):
                contract   = entry.get("contract", "")
                entry_mc   = entry.get("mc", 0)
                entry_liq  = entry.get("liq", 0)
                entry_score= entry.get("score", 0)
                verdict    = entry.get("verdict", "SKIP")
                was_bought = entry.get("bought", False)
                symbol     = entry.get("symbol", "?")

                if isinstance(info, Exception) or not info:
                    # Can't fetch = likely dead/rugged
                    rugged.append({
                        "symbol": symbol, "contract": contract,
                        "entry_mc": entry_mc, "score": entry_score,
                        "verdict": verdict, "bought": was_bought,
                    })
                    if was_bought:
                        bought_rugged.append(symbol)
                    continue

                cur_mc  = info.get("mc", 0)
                cur_liq = info.get("liq", 0)
                mc_x    = round(cur_mc / entry_mc, 2) if entry_mc > 0 else 0

                rec = {
                    "symbol": symbol, "contract": contract,
                    "entry_mc": entry_mc, "cur_mc": cur_mc,
                    "mc_x": mc_x, "cur_liq": cur_liq,
                    "score": entry_score, "verdict": verdict,
                    "bought": was_bought,
                }
                ...
                if cur_liq < 1_000:
                    ...
                    survived.append(rec)
                    if mc_x >= 5.0:
                        pumped_5x.append(rec)
                        if not was_bought:
                            skipped_pumped.append(rec)
                    elif mc_x >= 2.0:
                        pumped_2x.append(rec)
                        if not was_bought:
                            skipped_pumped.append(rec)
        ...
        surv_pct  = round(len(survived) / total * 100)
        rug_pct   = round(len(rugged)   / total * 100)

        # ── Build the report ──────────────────────────────────────────────────
        lines = [
            "📊 *24H SURVIVAL REPORT*",
            f"_{datetime.utcnow().strftime('%b %d, %Y')} — based on yesterday's scans_\n",
            f"Tokens scanned: *{total}*",
            f"Survived (liq > $1K): *{len(survived)}* ({surv_pct}%)",
            f"Rugged / dead: *{len(rugged)}* ({rug_pct}%)",
            f"Pumped 2x+: *{len(pumped_2x)}*",
            f"Pumped 5x+: *{len(pumped_5x)}*\n",
        ]

        # Tokens skipped but pumped 2x+ — filter insight
        if skipped_pumped:
            skipped_pumped.sort(key=lambda x: x["mc_x"], reverse=True)
            lines.append("🟡 *SKIPPED — but pumped:*")
            for rec in skipped_pumped[:5]:
                ds_url = f"https://dexscreener.com/solana/{rec['contract']}"
                lines.append(
                    f"  [${_md(rec['symbol'])}]({ds_url})  *{rec['mc_x']}x*"
                    f"  score:{rec['score']}  {rec['verdict']}"
                )
            if len(skipped_pumped) > 5:
                lines.append(f"  _...and {len(skipped_pumped) - 5} more_")
            lines.append("")

        # Tokens bot bought that rugged
        if bought_rugged:
            lines.append("🔴 *APEX BOUGHT — then rugged:*")
            for rec in (r for r in rugged if r.get("bought")):
                ds_url = f"https://dexscreener.com/solana/{rec['contract']}"
                lines.append(f"  [${_md(rec['symbol'])}]({ds_url})")
            lines.append("")

        # Top survivors by MC gain
        if survived:
            top_surv = sorted(survived, key=lambda x: x["mc_x"], reverse=True)[:8]
            lines.append("🟢 *TOP SURVIVORS (24h):*")
            for rec in top_surv:
                x_str  = f"*{rec['mc_x']}x*" if rec["mc_x"] > 0 else "~1x"
                ds_url = f"https://dexscreener.com/solana/{rec['contract']}"
                lines.append(
                    f"  [${_md(rec['symbol'])}]({ds_url})  {x_str}"
                    f"  liq:{money(rec['cur_liq'])}"
                    f"  score:{rec['score']}"
                )
            lines.append("")

        # Filter calibration hint
        if skipped_pumped:
            avg_skipped_score = round(
                sum(r["score"] for r in skipped_pumped) / len(skipped_pumped)
            )
            lines.append(
                f"💡 *Filter insight:* {len(skipped_pumped)} skipped tokens pumped 2x+\n"
                f"   Avg score of missed pumps: *{avg_skipped_score}/100*\n"
                f"   _(If avg score > your threshold, consider lowering it slightly)_"
            )
        ...
        # Convert Markdown to HTML for clickable inline links
        # Telegram Markdown v1 doesn't support [text](url) in all contexts
        import re as _re2
        def _md_to_html(text: str) -> str:
            # Convert [text](url) → <a href="url">text</a>
            text = _re2.sub(r'\[([^\]]+)\]\(([^)]+)\)', r'<a href="\2">\1</a>', text)
            # Convert *bold* → <b>bold</b>
            text = _re2.sub(r'\*([^*]+)\*', r'<b>\1</b>', text)
            # Convert _italic_ → <i>italic</i>
            text = _re2.sub(r'_([^_]+)_', r'<i>\1</i>', text)
            return text

        report_html = _md_to_html(report_text)

        # Send to dump log channel if set, else DM
        target = ud.get("sniper_log_channel") or uid
        try:
            await ctx.bot.send_message(
                chat_id=int(target),
                text=report_html,
            ...
            logger.info(f"24h survival report sent for uid={uid} ({total} tokens)")
        except Exception as _sre:
            logger.warning(f"Survival report send failed uid={uid}: {_sre}")
            if target != uid:
                try:
                    await ctx.bot.send_message(
                        chat_id=uid, text=report_html,
                        parse_mode="HTML",
                        disable_web_page_preview=True,
                    )
                except Exception:
...
async def check_ticker_origin(symbol: str, contract: str, pair_created_ms: int) -> str:
    """
    Check if this token is the first with this ticker on Solana or a copycat.
    Returns "FIRST", "COPY", or "" (unknown — couldn't determine).
    Uses DexScreener search — free, no API key, no credits.
    Cached for 1 hour since origin status never changes.
    """
    _cache_key = contract[:32]
    _cached = _ticker_origin_cache.get(_cache_key)
    if _cached and (_time.time() - _cached["ts"]) < 3600:
    ...
    try:
        ...
        # Filter to same chain, same symbol, exclude current contract
        same_sym = [
            p for p in pairs
            if p.get("baseToken", {}).get("symbol", "").upper() == symbol.upper()
            and p.get("chainId", "").lower() in ("solana", "sol")
            and p.get("baseToken", {}).get("address", "") != contract
            and p.get("pairCreatedAt")
        ]

        if not same_sym:
            # No other token with this symbol on Solana
            result = "FIRST"
        else:
            # Find oldest pair among others
            oldest_other = min(same_sym, key=lambda p: p.get("pairCreatedAt", 9e18))
            oldest_ts = oldest_other.get("pairCreatedAt", 0)
            if pair_created_ms and pair_created_ms <= oldest_ts:
                result = "FIRST"
            else:
                result = "COPY"

    except Exception as e:
        logger.debug(f"check_ticker_origin error: {e}")
        result = ""

    _ticker_origin_cache[_cache_key] = {"result": result, "ts": _time.time()}
    # Trim cache
    if len(_ticker_origin_cache) > 2000:
        _oldest = sorted(_ticker_origin_cache.keys(),
                         key=lambda k: _ticker_origin_cache[k]["ts"])[:200]
        for _k in _oldest:
            _ticker_origin_cache.pop(_k, None)
    return result
                ...
                if isinstance(pf_data, list):
                    ...
                    for coin in pf_data:
                        ...
                        out[mint] = {
                            ...
                            "_pf_image":    coin.get("image_uri", "") or coin.get("image", ""),
                        }
                    return out
        except Exception as e:
...
def _sniper_daily_reset(ud: dict):
    ...
    Compares as ISO string to avoid date vs string mismatch after Supabase reload.
    """
    today_str = datetime.now().date().isoformat()   # always "2026-03-20" string
    stored    = ud.get("sniper_daily_date")
    # Normalise stored value to string for comparison
    if hasattr(stored, "isoformat"):
        stored = stored.isoformat()
    if stored != today_str:
        ud["sniper_daily_spent"] = 0.0
        ud["sniper_daily_date"]  = today_str
        ud.pop("_budget_alert_sent", None)   # reset alert flag for new day
...
def _compact_pill_text(info: dict, sc: dict, ai: dict) -> str:
    ...
    origin   = info.get("ticker_origin", "")
    origin_tag = "  `✅ FIRST`" if origin == "FIRST" else ("  `⚠️ COPY`" if origin == "COPY" else "")
    ...
    return (
        ...
        f"MC: *{_mf(mc)}*{origin_tag}  |  Liq: *{_mf(liq)}*  |  Age: *{age_str}*\n"
...
def _ai_report_text(info: dict, sc: dict, ai: dict, contract: str = "", expanded: bool = True) -> str:
    ...
    return (
        ...
        f"MC: *{_mf(mc)}*{('  `✅ FIRST`' if info.get('ticker_origin') == 'FIRST' else ('  `⚠️ COPY`' if info.get('ticker_origin') == 'COPY' else ''))}  |  Liq: *{_mf(liq)}* ({round(liq_pct,1)}%)\n"
...
def _channel_card_text(info: dict, sc: dict, ai: dict, contract: str) -> str:
    ...
    stats_block = (
        f"💰 MC: <b>{_mf(mc)}</b>{('  <code>✅ FIRST</code>' if info.get('ticker_origin') == 'FIRST' else ('  <code>⚠️ COPY</code>' if info.get('ticker_origin') == 'COPY' else ''))}  ·  Liq: <b>{_mf(liq)}</b> <i>({round(liq_pct, 1)}%)</i>\n"
...
async def _broadcast_to_channel(bot, channel_id: int, info: dict, sc: dict, ai: dict, contract: str, uid: int = 0) -> bool:
    """Post channel card with token image if available, otherwise plain text."""
    try:
        ...
        # ── Send with token image if available (like GemTools) ───────────────
        # Send image first as standalone photo, then full card text separately.
        # This avoids Telegram's 1024-char caption limit cutting off flags/links.
        image_url = info.get("pf_image", "")
        msg = None

        if image_url:
            try:
                # ── Cross-reference token with active narratives ───────────────
                # Check if any active narrative keyword appears in the token
                # name or symbol. Shows context like "📡 Narrative: AI (12 tokens)"
                _tok_words = set(
                    (info.get("symbol","") + " " + info.get("name","")).lower().split()
                )
                _narrative_line = ""
                if _narrative_tracker:
                    _active_kws = {
                        kw: cnt for kw, cnt in _narrative_tracker.items()
                        if cnt >= 5 and kw in _tok_words
                    }
                    if _active_kws:
                        # Pick the strongest matching keyword
                        _best_kw  = max(_active_kws, key=lambda k: _active_kws[k])
                        _best_cnt = _active_kws[_best_kw]
                        _narrative_line = f"\n📡 Narrative: <b>{_best_kw.upper()}</b> ({_best_cnt} tokens this cycle)"

                # Send image with ticker label + narrative context
                await bot.send_photo(
                    chat_id=channel_id,
                    photo=image_url,
                    caption=f"<b>${symbol}</b>  ·  {name}{_narrative_line}",
                    parse_mode="HTML",
                    disable_notification=True,
                )
            except Exception as _img_err:
                logger.debug(f"Token image send failed: {_img_err}")

        # Always send full card text with buttons (no truncation)
...
async def channel_milestone_job(ctx: ContextTypes.DEFAULT_TYPE):
    """
    Runs every 5 minutes. Posts EVERY integer x milestone sequentially —
    x2, x3, x4 ... x14, x15, x16 ... just like GemTools.

    Uses last_milestone (int) to track the highest announced multiple.
    Each cycle: announce everything from last_milestone+1 up to floor(current_x).
    Max 3 posts per cycle per token to avoid spam if bot was offline.
    ...
    for uid, calls in list(_channel_calls.items()):
        for contract, call_data in list(calls.items()):
            try:
                ...
                # last_milestone = highest integer x already announced (starts at 1)
                last_milestone = call_data.get("last_milestone", 1)
                next_milestone = last_milestone + 1

                if current_x < next_milestone:
                    continue
                ...
                # Post each missed milestone sequentially, max 3 per cycle
                posted = 0
                while current_x >= next_milestone and posted < 3:
                    # Header style scales with achievement — single emoji only
                    if next_milestone >= 100:
                        header = f"🏆 {next_milestone}x"
                    ...
                    ms_text = (
                        ...
                        f"⏱ Time to {next_milestone}x: <b>{elapsed_str}</b>\n"
                    )
                    ...
                    try:
                        ...
                        logger.info(f"Milestone {next_milestone}x posted for ${symbol} to channel {ch_id}")
                        call_data["last_milestone"] = next_milestone
...
async def sniper_job(ctx: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # Batched: fire 8 at a time with 0.3s between batches.
        # Firing all 42 simultaneously was causing 429s on every single token,
        # meaning the entire sniper cycle ran blind with no data.
        _pf_items   = unique_items
        _pf_batch   = 8
        _pf_delay   = 0.3
        for _pf_i in range(0, len(_pf_items), _pf_batch):
            _pf_chunk = _pf_items[_pf_i:_pf_i + _pf_batch]
            await _asyncio.gather(
                *[get_token(c["tokenAddress"], force=True) for c in _pf_chunk],
                return_exceptions=True
            )
            if _pf_i + _pf_batch < len(_pf_items):
                await _asyncio.sleep(_pf_delay)
        ...
        for item in unique_items:
            ...
            try:
                ...
                info["pf_image"]     = item.get("_pf_image", "")   # token logo from pump.fun
                ...
                # ── Ticker origin: FIRST or COPY ─────────────────────────────
                # Only check Solana tokens — free DexScreener search, no credits.
                # Result stored on info so all display functions can read it.
                info["ticker_origin"] = ""
                if token_chain == "solana":
                    try:
                        _pair_created_ms = 0
                        if info.get("age_h") is not None:
                            _pair_created_ms = int(
                                (_time.time() - info["age_h"] * 3600) * 1000
                            )
                        info["ticker_origin"] = await check_ticker_origin(
                            info.get("symbol", ""), contract, _pair_created_ms
                        )
                    except Exception as _toe:
                        logger.debug(f"ticker_origin check failed: {_toe}")
                ...
                try:
                    ...
                    if skip_reason:
                        ...
                        if _reason_key == "score":
                            _skip_ttl = 1800          # 30 min — score rarely improves meaningfully in 5 min
                    ...
                    if not _advisory_only and spent + max_buy > budget:
                        ...
                        # ── Alert user once per day when budget is exhausted ──
                        if not ud.get("_budget_alert_sent"):
                            ud["_budget_alert_sent"] = True
                            try:
                                await ctx.bot.send_message(
                                    chat_id=uid,
                                    parse_mode="Markdown",
                                    text=(
                                        "💰 *DAILY SNIPER BUDGET EXHAUSTED*\n\n"
                                        "Spent: *" + money(spent) + "* of *" + money(budget) + "* today\n\n"
                                        "APEX will not enter new positions until tomorrow (resets midnight UTC) "
                                        "or until you raise the budget.\n\n"
                                        "_Sniper → 💰 Budget to adjust_"
                                    ),
                                    reply_markup=InlineKeyboardMarkup([[
                                        InlineKeyboardButton("💰 Raise Budget", callback_data="sniper_budget_cfg")
                                    ]])
                                )
                            except Exception as _bae:
                                logger.debug(f"Budget alert send failed: {_bae}")
                        continue
                    ...
                    # Cache results for 10 min — same token shouldn't be re-queried
                    # every 5-min scan cycle (primary cause of credit drain).
                    if helius_key and info.get("chain","").lower() in ("solana","sol"):
                        try:
                            _hm_cached = _helius_maker_cache.get(contract)
                            _hm_fresh  = _hm_cached and (_time.time() - _hm_cached["ts"]) < HELIUS_MAKER_CACHE_TTL
                            if _hm_fresh:
                                helius_data = _hm_cached["data"]
                            else:
                                helius_data = await get_helius_maker_pct(contract, helius_key)
                                if helius_data:
                                    _helius_maker_cache[contract] = {"data": helius_data, "ts": _time.time()}
                                    # Trim cache if oversized
                                    if len(_helius_maker_cache) > 500:
                                        _oldest_hm = sorted(_helius_maker_cache.keys(),
                                            key=lambda k: _helius_maker_cache[k]["ts"])[:100]
                                        for _k in _oldest_hm:
                                            _helius_maker_cache.pop(_k, None)
                            if helius_data:
                    ...
                    if ud.get("apex_mode") and _apex_verdict_ok:
                        ...
                        # ── HARD RUG PREVENTION — runs before score/confidence ─────────
                        # Non-negotiable. No score or confidence overrides these.
                        # Based on rugged tokens in trade history: Pssh, DonTzu, RAYTRUMP,
                        # LOW INCOME, EPIK — all had at least one of these signals at entry.
                        _rug_block    = None
                        _e_liq        = info.get("liq", 0)
                        _e_mc         = info.get("mc", 1)
                        _e_liq_pct    = info.get("liq_pct", 0)
                        _e_vol_h1     = info.get("vol_h1", 0)
                        _e_no_mint    = info.get("no_mint")     # None=unknown, True=safe, False=ACTIVE
                        _e_lp_burn    = info.get("lp_burn")     # None=unknown, float=pct burned
                        _e_top10      = info.get("top10_pct")
                        _e_dev_pct    = float(info.get("pf_dev_pct") or info.get("dev_pct_rc") or 0)
                        _e_insider    = info.get("insider_pct")
                        _e_vol_mc     = (_e_vol_h1 / _e_mc) if _e_mc > 0 else 0
                        _e_hardflags  = sc.get("flags", [])

                        if _e_hardflags:
                            # Only block on the most critical hard flags — honeypot, bundle, dev sold
                            _critical = [f for f in _e_hardflags if any(
                                kw in f.lower() for kw in
                                ["honeypot", "bundle", "dev already sold", "mint authority", "copycat"]
                            )]
                            if _critical:
                                _rug_block = f"Hard flag: {_critical[0]}"
                        if not _rug_block and _e_liq < 5_000:
                            _rug_block = f"Liq ${_e_liq:,.0f} — too thin"
                        elif not _rug_block and _e_liq_pct < 2 and _e_mc > 80_000:
                            _rug_block = f"Liq {_e_liq_pct:.1f}% of MC — drain risk"
                        elif not _rug_block and _e_vol_mc > 20:
                            _rug_block = f"Vol/MC {round(_e_vol_mc,1)}x — wash trading"
                        elif not _rug_block and _e_no_mint is False:
                            _rug_block = "Mint authority active — supply can inflate"
                        elif not _rug_block and _e_lp_burn is not None and _e_lp_burn < 30 and not info.get("pf_curve"):
                            _rug_block = f"LP only {_e_lp_burn:.0f}% burned"
                        elif not _rug_block and _e_top10 is not None and _e_top10 > 60:
                            _rug_block = f"Top10 hold {_e_top10:.0f}% — whale trap"
                        elif not _rug_block and _e_dev_pct > 15:
                            _rug_block = f"Dev holds {_e_dev_pct:.1f}%"
                        elif not _rug_block and _e_insider is not None and _e_insider > 20:
                            _rug_block = f"Insiders hold {_e_insider:.1f}%"

                        if _rug_block:
                            _ok = False
                            logger.info(f"APEX rug gate blocked {info.get('symbol','?')}: {_rug_block}")
                            ud.setdefault("sniper_log", []).append({
                                "contract": contract, "symbol": info.get("symbol","?"),
                                "chain": info.get("chain","?"), "mc": info.get("mc",0),
                                "liq": _e_liq, "score": sc.get("score",0),
                                "verdict": "SKIP", "confidence": 0, "rug_risk": "HIGH",
                                "thesis": f"APEX rug gate: {_rug_block}",
                                "red_flags": [_rug_block], "green_flags": [],
                                "timestamp": datetime.now().isoformat(),
                                "bought": False, "skip_stage": "apex_rug_gate",
                            })
                    ...
                    # ════════════════════════════════════════════════════
                    if ud.get("sniper_advisory") and _market_regime != "DEAD":
...
def apex_check_threat(info: dict, h: dict) -> str:
    ...
    # The data shows 30 exits at 0.80–1.0x from "threat red" — these are
    # normal micro-cap entry dips, not real rugs. Only exit RED on hard
    # on-chain signals: LP drain, extreme panic, or confirmed coordinated dump.
    # Price-based RED (drop%) is handled purely by stop_loss_pct, not threat.

    # 1. Liq drain — strongest rug signal, always exit
    if liq_drop >= 25:
        return "RED"

    # 2. Extreme buy pressure collapse — requires TWO confirmations
    if buy_pct_m5 < 10:
        # Absolute panic (<10% buys) AND liq draining — confirmed rug
        if liq_drop >= 8:
            return "RED"
        # Absolute panic AND price already well below entry
        if cx_now < 0.80:
            return "RED"
    elif buy_pct_m5 < 20:
        # Severe panic + liq drain = real rug
        if liq_drop >= 15:
            return "RED"
        # Severe panic + position well below entry + liq also dropping
        if cx_now < 0.85 and liq_drop >= 8:
            return "RED"

    # 3. Coordinated dump: significant liq drain + heavy sell pressure
    if liq_drop >= 18 and buy_pct_m5 < 35:
        return "RED"

    # 4. Bundle/whale dump: massive volume spike + overwhelming sell pressure
    if avg_5m_vol > 0 and vol_m5 >= avg_5m_vol * 5 and buy_pct_m5 < 25:
        return "RED"

    # ══ SMART PRICE RED — only when drop exceeds the full SL threshold ════════
    # No combined-signal RED below entry (that was causing 30 false exits).
    # The stop_loss_pct mechanism handles gradual price-only exits cleanly.
...
def apex_reset_daily(ud: dict) -> None:
    ...
    if ud.get("apex_daily_date") != today:
        ...
        ud["apex_large_loss_count"]   = 0   # reset daily — not on individual wins
...
def apex_is_daily_loss_halted(ud: dict) -> bool:
    ...
    # Check 1: cumulative daily loss % limit
    if ud.get("apex_daily_pnl", 0.0) <= -(start * _limit):
        return True
    # Check 2: 3+ large single-trade losses in one day
    if ud.get("apex_large_loss_count", 0) >= 3:
        return True
    return False
    ...
    async with _get_user_lock(uid):
        for contract, h in list(ud.get("holdings", {}).items()):
            ...
            try:
                ...
                # ── RED: emergency exit ───────────────────────────────────────────
                # Require 2 consecutive RED cycles before exiting in ALL modes.
                # Data shows RED fires on normal micro-cap volatility within 5 minutes
                # (avg hold = 5.8 min, avg exit = 0.84x). One bad candle should not
                # kill a position. Helius-confirmed rugs (LP drained) bypass this gate.
                _is_red = (threat == "RED" or info.get("_helius_rug"))
                if _is_red:
                    _hunter_requires_2x = _hp and cx >= _hp["red_2x_start"]
                    _default_requires_2x = (not _hp) and not info.get("_helius_rug")
                    if (_hunter_requires_2x or _default_requires_2x) and _prev_threat != "RED":
                        # First RED — record it but don't exit yet. Wait for confirmation.
                        h["apex_threat"] = "RED"
                        save_user(uid, ud)
                        continue
                    ...
                    # ── STOP LOSS CHECK — runs first, nothing overrides it ─────────
                    # The investigation/structure analysis can say "hold" all it wants.
                    # If the SL threshold is breached, we exit. Full stop.
                    # Pssh bug: investigation kept renewing _pullback_hold_until every
                    # 8s so SL never fired. Token dropped to 0.73x with 22% SL (should
                    # have exited at 0.78x) but investigation kept blocking it.
                    _sl_pct_pre   = h.get("stop_loss_pct")
                    _avg_pre      = h.get("avg_price", price)
                    _trail_act_pre = _hp["trail_activate_x"] if _hp else ud.get("apex_trail_activate_x_learned", APEX_TRAIL_ACTIVATE_X)
                    if _sl_pct_pre and _avg_pre > 0 and cx < _trail_act_pre:
                        _drop_pre = (price - _avg_pre) / _avg_pre * 100
                        if _drop_pre <= -_sl_pct_pre:
                            # SL breached — exit now, skip ALL investigation
                            _cv_pre = h["amount"] * price
                            if _cv_pre >= 0.01:
                                _res_pre = sell_core(ud, uid, contract, _cv_pre, price, "stop_loss")
                                ud["apex_daily_pnl"] = (ud.get("apex_daily_pnl") or 0.0) + (_res_pre.get("realized") or 0.0)
                                if _res_pre["realized"] < 0:
                                ...
                                apex_learn_record(uid, _apex_rich_record(h, cx, peak, avg, _res_pre["realized"], "stop_loss", ud))
                                logger.info(f"SL override: {h.get('symbol','?')} at {round(cx,2)}x drop {round(_drop_pre,1)}% — exiting despite hold decision")
                                try:
                                    await app.bot.send_message(
                                        chat_id=uid, parse_mode="Markdown",
                                        text=(
                                            "🛑 *APEX — STOP LOSS*\n\n"
                                            "*$" + _md(h["symbol"]) + "*  SL -" + str(_sl_pct_pre) + "%\n"
                                            "Exit: *" + str(round(cx, 2)) + "x*\n"
                                            "PnL: *" + pstr(_res_pre["realized"]) + "*"
                                        ),
                                        reply_markup=main_menu_kb()
                                    )
                                except Exception:
                    ...
                    # ── Respect a previous hold decision for up to 30 seconds ─────
                    _prev_hold_ts = h.get("_pullback_hold_until", 0)
                    if _time.time() < _prev_hold_ts:
                        _pi_skip_exit = True
                        logger.debug(f"Pullback [{h.get('symbol','?')}]: respecting previous hold decision ({round(_prev_hold_ts - _time.time(),1)}s remaining)")

                    # ══════════════════════════════════════════════════════════════
                    # STRUCTURE ANALYSIS — runs before Helius investigation
                    # Uses sr_history (price snapshots every 8s) to determine:
                    #   Is this a retrace inside an uptrend? → HOLD
                    #   Is this a genuine breakdown?         → EXIT
                    #
                    # Concepts checked:
                    #   1. Higher lows — each dip bottoms higher than the last
                    #   2. Volume pattern — selling vol < buying vol on this move
                    #   3. MC vs entry — still well above where we bought?
                    #   4. Retrace depth — how far from peak? Normal = 20-40%
                    #   5. Buy pressure recovery — are buyers returning?
                    #   6. Wallet funding clustering — same-source wallets selling?
                    # ══════════════════════════════════════════════════════════════
                    if not _pi_skip_exit and not info.get("_helius_rug"):
                        _sa_hold   = False   # True = structure says retrace, not collapse
                        _sa_reason = ""
                        _sa_score  = 0       # positive = retrace, negative = breakdown
                        _sa_notes  = []

                        history = h.get("sr_history") or h.get("price_history") or []

                        if len(history) >= 6:
                            prices   = [s.get("price", 0) for s in history if s.get("price", 0) > 0]
                            vols     = [s.get("vol", 0)   for s in history]
                            buys     = [s.get("buy_pct", 50) for s in history]
                            mc_hist  = [s.get("mc", 0)    for s in history if s.get("mc", 0) > 0]

                            if len(prices) >= 6:
                                cur_p    = prices[-1]
                                peak_p   = max(prices)
                                peak_idx = prices.index(peak_p)

                                # ── 1. HIGHER LOWS ────────────────────────────────
                                # Split history into thirds — guard against empty slices.
                                # With exactly 6 prices and third=2: [0:2],[2:4],[4:6] → fine.
                                # With 7+ it's always fine. The guard below ensures no
                                # empty slice reaches min() which would raise ValueError.
                                third = max(2, len(prices) // 3)
                                _slice_early  = prices[:third]
                                _slice_mid    = prices[third:third*2]
                                _slice_recent = prices[third*2:]

                                if _slice_early and _slice_mid and _slice_recent:
                                    lows_early  = min(_slice_early)
                                    lows_mid    = min(_slice_mid)
                                    lows_recent = min(_slice_recent)

                                    if lows_recent > lows_mid > lows_early * 0.95:
                                        _sa_score += 3
                                        _sa_notes.append("Higher lows intact")
                                    elif lows_recent < lows_early * 0.85:
                                        _sa_score -= 3
                                        _sa_notes.append("Lower lows — breakdown")

                                # ── 2. RETRACE DEPTH ─────────────────────────────
                                # Normal healthy retrace: 20-45% from peak
                                # Collapse: >60% from peak in a single move
                                if peak_p > 0 and cur_p > 0:
                                    retrace_pct = (peak_p - cur_p) / peak_p
                                    if retrace_pct <= 0.45:
                                        _sa_score += 2
                                        _sa_notes.append(f"Normal retrace {round(retrace_pct*100)}% from peak")
                                    elif retrace_pct <= 0.60:
                                        _sa_notes.append(f"Deep retrace {round(retrace_pct*100)}% from peak")
                                    else:
                                        _sa_score -= 3
                                        _sa_notes.append(f"Collapse depth {round(retrace_pct*100)}% from peak")

                                # ── 3. MC vs ENTRY ────────────────────────────────
                                if cx >= 1.20:
                                    _sa_score += 2
                                    _sa_notes.append(f"Still {round(cx,2)}x — retrace not breakdown")
                                elif cx >= 1.0:
                                    _sa_score += 1
                                    _sa_notes.append(f"Near entry {round(cx,2)}x — monitoring")
                                else:
                                    _sa_score -= 2
                                    _sa_notes.append(f"Below entry {round(cx,2)}x — loss territory")

                                # ── 4. VOLUME PATTERN ────────────────────────────
                                # On a healthy retrace: volume DROPS on red candles
                                # On a real dump: volume SPIKES on red candles
                                # Guard: peak_idx must not be the last snapshot —
                                # if peak is most recent there's no pullback data yet.
                                if len(vols) >= 4 and 0 < peak_idx < len(vols) - 1:
                                    pump_vols     = vols[:peak_idx+1]
                                    pullback_vols = vols[peak_idx+1:]  # strictly AFTER peak
                                    avg_pump      = sum(pump_vols) / len(pump_vols) if pump_vols else 0
                                    avg_pullback  = sum(pullback_vols) / len(pullback_vols) if pullback_vols else 0

                                    if avg_pump > 0:
                                        vol_ratio = avg_pullback / avg_pump
                                        if vol_ratio < 0.60:
                                            _sa_score += 3
                                            _sa_notes.append(f"Pullback vol only {round(vol_ratio*100)}% of pump — healthy")
                                        elif vol_ratio < 1.0:
                                            _sa_score += 1
                                            _sa_notes.append(f"Pullback vol moderate {round(vol_ratio*100)}%")
                                        elif vol_ratio >= 1.5:
                                            _sa_score -= 3
                                            _sa_notes.append(f"Sell vol {round(vol_ratio*100)}% of pump — real dump")
                                        else:
                                            _sa_score -= 1
                                            _sa_notes.append(f"Pullback vol elevated {round(vol_ratio*100)}%")

                                # ── 5. BUY PRESSURE RECOVERY ─────────────────────
                                if len(buys) >= 4:
                                    recent_buys  = buys[-3:]
                                    earlier_buys = buys[-6:-3] if len(buys) >= 6 else buys[:3]
                                    avg_recent   = sum(recent_buys)  / len(recent_buys)
                                    avg_earlier  = sum(earlier_buys) / len(earlier_buys)

                                    if avg_recent > avg_earlier and avg_recent >= 48:
                                        _sa_score += 2
                                        _sa_notes.append(f"Buy% recovering {round(avg_earlier)}% → {round(avg_recent)}%")
                                    elif avg_recent >= 50:
                                        _sa_score += 1
                                        _sa_notes.append(f"Buy% holding {round(avg_recent)}%")
                                    elif avg_recent < 35:
                                        _sa_score -= 2
                                        _sa_notes.append(f"Buy% collapsing {round(avg_recent)}%")

                                # ── 6. MC HISTORY LOWS ───────────────────────────
                                if len(mc_hist) >= 5:
                                    mc_lows = [min(mc_hist[i:i+3]) for i in range(0, len(mc_hist)-2, 2)]
                                    if len(mc_lows) >= 2 and mc_lows[-1] > mc_lows[-2] * 0.95:
                                        _sa_score += 1
                                        _sa_notes.append("MC holding above recent lows")
                                    elif len(mc_lows) >= 2 and mc_lows[-1] < mc_lows[-2] * 0.80:
                                        _sa_score -= 2
                                        _sa_notes.append("MC making lower lows")

                                # ── STRUCTURE VERDICT ─────────────────────────────
                                _sa_notes_str = " | ".join(_sa_notes[:3]) if _sa_notes else "insufficient data"
                                logger.info(
                                    f"Structure [{h.get('symbol','?')}] "
                                    f"score={_sa_score} cx={round(cx,2)}x | "
                                    + _sa_notes_str
                                )
                                ...
                                if _sa_score >= 4:
                                    _sa_hold = True
                                    _pi_skip_exit = True
                                    _hold_dur = 24 if _sa_score >= 6 else 16
                                    h["_pullback_hold_until"] = _time.time() + _hold_dur
                                    h["_structure_score"]     = _sa_score
                                    # Only notify ONCE — runs every 8s so without flag = spam
                                    if not h.get("_structure_notified"):
                                        h["_structure_notified"] = True
                                        _notes_display = "\n".join(_sa_notes[:4]) if _sa_notes else "Uptrend intact"
                                        try:
                                            await app.bot.send_message(
                                                chat_id=uid, parse_mode="Markdown",
                                                text=(
                                                    "📐 *APEX — STRUCTURE ANALYSIS*\n\n"
                                                    "*$" + _md(h["symbol"]) + "*  " + str(round(cx, 2)) + "x\n"
                                                    "─────────────────────\n"
                                                    + _notes_display + "\n\n"
                                                    "✅ *Decision: RETRACE — holding*\n"
                                                    "_Score: " + str(_sa_score) + "/15_"
                                                ),
                                                reply_markup=main_menu_kb()
                                            )
                                        except Exception:
                                    ...
                                    h.pop("_structure_notified", None)
                                    _sa_reason = "Structure breakdown confirmed"
                                    logger.info(f"Structure [{h.get('symbol','?')}]: breakdown confirmed (score={_sa_score}), proceeding to exit")

                    if not _pi_skip_exit and _pi_key and not info.get("_helius_rug") and not _hp:
                        # Only investigate in Default mode — Hunter has ratchet floor
                        # Don't investigate Helius-confirmed rugs (LP drained)
                        # Don't investigate if structure analysis already gave a verdict
                        ...
                        if _time.time() - _pi_last_ts > _pi_cooldown:
                            ...
                            try:
                                ...
                                if _pi_data:
                                    ...
                                    # ── BUNDLE CLUSTERING CHECK ───────────────────────────
                                    # NOTE: enrich_wallet_clustering requires holder_distribution
                                    # on the info dict. During live position management, info
                                    # comes from get_token() (DexScreener) which never includes
                                    # holder_distribution — that's only populated during the
                                    # sniper scan. So this check uses the ENTRY-TIME data
                                    # stored on the holding, which is the best we can do.
                                    # Sophisticated bundles with intermediate wallet hops
                                    # cannot be detected regardless.
                                    _pi_cluster_pct = 0.0
                                    _entry_holders  = h.get("_entry_holder_distribution", [])
                                    if _entry_holders:
                                        try:
                                            _clust = await enrich_wallet_clustering(
                                                contract, _entry_holders, _pi_key
                                            )
                                            _pi_cluster_pct = _clust.get("cluster_pct", 0.0)
                                            if _clust.get("cluster_detected") and _pi_cluster_pct >= 20:
                                                _pi_maker = min(_pi_maker, 40)
                                                logger.info(
                                                    f"Pullback [{h.get('symbol','?')}]: "
                                                    f"entry-time cluster {_pi_cluster_pct}% "
                                                    f"same-source — treating as concentrated"
                                                )
                                        except Exception as _ce:
                                            logger.debug(f"Clustering check failed: {_ce}")
                                    ...
                                    # ── Case C: Whale profit-take ─────────────────────
                                        ...
                                        _pi_skip_exit = True
                                        h["_pullback_hold_until"] = _time.time() + 30  # hold for 30s
                                        ...
                                        logger.info(f"Pullback [{h['symbol']}]: whale profit-take, trail tightened, holding 30s")
                                        if not h.get("_whale_notified"):
                                            h["_whale_notified"] = True
                                            try:
                                                    await app.bot.send_message(
                                                        chat_id=uid, parse_mode="Markdown",
                                                        text=(
                                    ...
                                    # ── Case D: Organic thin-book pressure ────────────
                                        ...
                                        logger.info(f"Pullback [{h['symbol']}]: organic thin-book pressure, holding 30s")
                                        if not h.get("_organic_notified"):
                                            h["_organic_notified"] = True
                                            try:
                                                await app.bot.send_message(
                                                chat_id=uid, parse_mode="Markdown",
                                                text=(
                    ...
                    if _pi_skip_exit:
                        # Investigation says hold on RED exit — but SL ALWAYS runs.
                        # Bug was: this continue skipped the SL block entirely.
                        # $Pssh at 0.73x with 22% SL was never cut because investigation
                        # said "whale profit-take" and bypassed the stop loss.
                        # Fix: check SL right here before continuing.
                        _sl_pct_now = h.get("stop_loss_pct")
                        _avg_now    = h.get("avg_price", price)
                        if _sl_pct_now and _avg_now > 0:
                            _drop_now = (price - _avg_now) / _avg_now * 100
                            if _drop_now <= -_sl_pct_now:
                                # SL hit — exit regardless of investigation verdict
                                _cv_sl = h["amount"] * price
                                if _cv_sl >= 0.01:
                                    _res_sl = sell_core(ud, uid, contract, _cv_sl, price, "stop_loss")
                                    ud["apex_daily_pnl"] = (ud.get("apex_daily_pnl") or 0.0) + (_res_sl.get("realized") or 0.0)
                                    if _res_sl["realized"] < 0:
                                        ud["apex_consec_losses"] = ud.get("apex_consec_losses", 0) + 1
                                    else:
                                        ud["apex_consec_losses"] = 0
                                        ud["apex_total_wins"] = ud.get("apex_total_wins", 0) + 1
                                    ud["apex_total_trades"] = ud.get("apex_total_trades", 0) + 1
                                    apex_learn_record(uid, _apex_rich_record(h, cx, peak, avg, _res_sl["realized"], "stop_loss", ud))
                                    try:
                                        await app.bot.send_message(
                                            chat_id=uid, parse_mode="Markdown",
                                            text=(
                                                "🛑 *APEX — STOP LOSS*\n\n"
                                                "*$" + _md(h["symbol"]) + "*  SL -" + str(_sl_pct_now) + "%\n"
                                                "Exit: *" + str(round(cx, 2)) + "x*\n"
                                                "PnL: *" + pstr(_res_sl["realized"]) + "*\n"
                                                "_(SL overrides hold decision)_"
                                            ),
                                            reply_markup=main_menu_kb()
                                        )
                                    except Exception:
                                        pass
                                continue  # position closed, move on
                        # SL not hit — safe to hold as investigation suggested
                        save_user(uid, ud)
                        continue
                ...
                #   Before 2x : SL stays fully active for capital protection
                #               RED threat handles genuine rugs at any point
                #   At 2x     : sell 50% → returns initial capital to vault
                #               trail activates on remaining 50% (house money)
                #               SL removed — you cannot lose on house money
                #   After 2x  : trail manages the rest to 5x, 10x etc.
                if not _hp:
                    # ── 2x sell: return initial capital + activate trail ─────────
                    if cx >= 2.0 and not h.get("apex_ladder_sold_2x"):
                        _ladder_amt2 = h.get("amount", 0)
                        _ladder_cv2  = _ladder_amt2 * price
                        if _ladder_cv2 >= 0.02:
                            # Sell exactly 50% — returns original capital
                            _sell_val2 = _ladder_cv2 * 0.50
                            if _sell_val2 >= 0.01:
                                _lad_result2 = sell_core(ud, uid, contract, _sell_val2, price, "apex_ladder_2x")
                                if isinstance(_lad_result2, dict):
                                    h["apex_ladder_sold_2x"] = True
                                    ud["apex_daily_pnl"] = (ud.get("apex_daily_pnl") or 0.0) + (_lad_result2.get("realized") or 0.0)
                                    _lad_pnl2 = _lad_result2.get("realized", 0)

                                    # ── Token intelligence: set hold target for remaining 50% ──
                                    # APEX uses entry data to decide how hard to hold house money.
                                    _score      = h.get("apex_entry_score", 50)
                                    _conf       = h.get("apex_entry_conf", 5)
                                    _tok_type   = h.get("apex_token_type", "organic")
                                    _graduated  = h.get("apex_entry_rug", "LOW") == "LOW"
                                    _wave       = h.get("apex_wave_phase", _market_wave_phase)
                                    _narr_pos   = 3  # default conservative

                                    # High conviction: score 75+, high conf, TECH or kol
                                    if _score >= 75 and _conf >= 8 and _tok_type in ("tech", "kol", "graduated"):
                                        _hold_target = 10.0
                                        _trail_mode  = "MOON"
                                        _hold_label  = "🚀 10x target — wide trail, riding house money"
                                    # Good conviction: score 60+, graduated pico
                                    elif _score >= 60 and _conf >= 6 and _tok_type in ("graduated", "kol"):
                                        _hold_target = 5.0
                                        _trail_mode  = "HOLD"
                                        _hold_label  = "💎 5x target — standard trail on house money"
                                    # WAVE phase — extract rest faster
                                    elif _wave == "WAVE":
                                        _hold_target = 3.0
                                        _trail_mode  = "FAST"
                                        _hold_label  = "⚡ 3x target — WAVE market, extracting"
                                    # MEME + late narrative — don't overstay
                                    elif _tok_type == "pumpfun" and _score < 60:
                                        _hold_target = 3.0
                                        _trail_mode  = "FAST"
                                        _hold_label  = "🎭 3x target — MEME token, taking profits fast"
                                    else:
                                        _hold_target = 5.0
                                        _trail_mode  = "HOLD"
                                        _hold_label  = "📈 5x target — standard trail"

                                    # Store hold target on position for trail logic
                                    h["apex_hold_target"] = _hold_target
                                    h["apex_trail_mode"]  = _trail_mode

                                    try:
                                        await app.bot.send_message(
                                            chat_id=uid, parse_mode="Markdown",
                                            text=(
                                                "💰 *APEX — CAPITAL RETURNED (2x)*\n\n"
                                                "*$" + _md(h["symbol"]) + "*  hit *2x*\n"
                                                "Sold *50%* → *" + money(_sell_val2) + "* back to vault\n"
                                                "Initial capital: *fully returned* ✅\n\n"
                                                "Remaining *50%* = pure profit\n"
                                                "_" + _hold_label + "_"
                                            ),
                                            reply_markup=main_menu_kb()
                                        )
                ...
                if _hunter_floor > 0 and price < _hunter_floor:
                    # Floor set — either by Hunter ratchet OR by ladder-sell break-even fix.
                    # Enforced regardless of profile: Default positions that hit 1.3x and
                    # got the BE floor set must also be protected here.
                    _sl_triggered = True
                ...
                # ── SL only disabled once trail activates at 2x ──────────────────
                # Before 2x: SL stays active — it protects capital if the token fails
                # At 2x: we sell 50% (capital returned), trail activates on remainder
                # After 2x: playing with house money, SL no longer needed
                if cx >= _trail_activate and h.get("stop_loss_pct") is not None:
                ...
                if cx >= _trail_activate:
                    if _hp:
                        ...
                        # ── Token intelligence: widen or tighten trail based on
                        # hold target set at 2x. High conviction = wide trail for
                        # 10x. MEME/fast = tighter trail. House money after 2x.
                        _trail_mode   = h.get("apex_trail_mode", "HOLD")
                        _hold_target  = h.get("apex_hold_target", 5.0)
                        _capital_back = h.get("apex_ladder_sold_2x", False)

                        if _capital_back:  # only adjust once capital is returned
                            if _trail_mode == "MOON":
                                # High conviction 10x — very wide trail, let it breathe
                                tpct = max(tpct, 0.30) if cx < 5.0 else max(tpct, 0.20)
                            elif _trail_mode == "FAST":
                                # MEME / WAVE — tighter, lock profits faster
                                tpct = min(tpct, 0.15) if cx < 3.0 else min(tpct, 0.10)
                            # HOLD mode = standard tpct unchanged
    ...
    async with _get_user_lock(uid):
      for contract, entry in list(queue.items()):
        ...
        if mc_runup > 2.0:
            ...
            continue
        # ── RUG PROTECTION — parabolic pump guard ─────────────────────────────
        # Uses live data at 45s confirmation to catch tokens that pumped
        # parabolically after signal ($RAYTRUMP pattern: +2000% in minutes).
        _ch_h1_live  = info_live.get("ch_h1", 0)
        _ch_m5_live  = info_live.get("ch_m5", 0)
        _vol_mc_live = (info_live.get("vol_h1", 0) / max(info_live.get("mc", 1), 1))
        if _ch_h1_live >= 400:
            logger.info(f"APEX queue: {contract} rejected — parabolic ch_h1={round(_ch_h1_live)}%")
            continue
        if _ch_h1_live >= 300:
            sc["score"] = min(sc.get("score", 0), 50)
            if sc["score"] < 50:
                logger.info(f"APEX queue: {contract} score capped — ch_h1={round(_ch_h1_live)}%")
                continue
        if _ch_m5_live > 80:
            logger.info(f"APEX queue: {contract} rejected — M5 still pumping {round(_ch_m5_live)}%")
            continue
        if _vol_mc_live > 15:
            logger.info(f"APEX queue: {contract} rejected — vol/MC {round(_vol_mc_live,1)}x (rug pump)")
            continue
        # ─────────────────────────────────────────────────────────────────────
        ...
        try:
            ...
            h["apex_ladder_sold_1_3x"] = False   # floor locked at 1.3x
            h["apex_ladder_sold_2x"]   = False   # 50% sold at 2x (capital returned)
            # Hold target — set at entry based on token intelligence, refined at 2x
            h["apex_hold_target"] = 5.0           # default: ride to 5x
            h["apex_trail_mode"]  = "HOLD"        # MOON / HOLD / FAST
            h["apex_wave_phase"]  = _market_wave_phase  # wave at time of entry
            ...
            # Store entry-time holder distribution for pullback investigation.
            # get_token() during live management never includes holder_distribution
            # (that's only fetched during sniper scan enrichment). Saving it here
            # gives the pullback clustering check real data to work with.
            _entry_hd = (entry.get("info") or info_post).get("holder_distribution", [])
            if _entry_hd:
                h["_entry_holder_distribution"] = _entry_hd
...
# ══════════════════════════════════════════════════════════════════════════════
# WAVE PHASE DETECTION ENGINE
# ══════════════════════════════════════════════════════════════════════════════

def _detect_market_wave_phase(scanned_tokens: list) -> tuple:
    """
    Analyses vol_h1 across all scanned tokens this cycle to detect
    the current liquidity wave phase.

    Returns: (wave_phase, vol_regime, avg_vol_now, avg_vol_baseline)

    Phases (from Latuche's Liquidity Wave Theory):
      SILENCE  — vol consistently below baseline (accumulation — buy the FUD)
      STIRRING — vol rising from LOW (wave forming — early positioning window)
      WAVE     — vol significantly above baseline (euphoria — extract fast)
      CRASH    — vol collapsing after HIGH spike (do not buy falling knives)
      RESET    — vol stabilising after crash (wait for next SILENCE)
    """
    global _market_vol_history, _market_wave_phase

    vols = []
    for item in scanned_tokens:
        contract = item.get("tokenAddress", "")
        cached   = _token_cache.get(contract, {}).get("data", {})
        vol      = cached.get("vol_h1", 0) or 0
        if vol > 0:
            vols.append(vol)

    if not vols:
        return _market_wave_phase, "NORMAL", 0, 0

    avg_vol_now = sum(vols) / len(vols)
    _market_vol_history.append(avg_vol_now)
    if len(_market_vol_history) > WAVE_PHASE_CYCLES * 2:
        _market_vol_history = _market_vol_history[-WAVE_PHASE_CYCLES:]

    if len(_market_vol_history) < 3:
        return "RESET", "NORMAL", avg_vol_now, avg_vol_now

    baseline_vols = _market_vol_history[:-1]
    avg_baseline  = sum(baseline_vols) / len(baseline_vols)
    if avg_baseline <= 0:
        return "RESET", "NORMAL", avg_vol_now, avg_vol_now

    ratio = avg_vol_now / avg_baseline

    if ratio >= WAVE_VOL_HIGH_MULT:
        vol_regime = "HIGH"
    elif ratio <= WAVE_VOL_LOW_MULT:
        vol_regime = "LOW"
    else:
        vol_regime = "NORMAL"

    prev_phase = _market_wave_phase
    if vol_regime == "HIGH":
        new_phase = "WAVE"
    elif vol_regime == "LOW":
        if prev_phase == "WAVE":
            new_phase = "CRASH"
        elif prev_phase == "CRASH":
            new_phase = "RESET"
        else:
            new_phase = "SILENCE"
    else:
        if prev_phase in ("SILENCE", "RESET"):
            new_phase = "STIRRING"
        elif prev_phase == "WAVE":
            new_phase = "CRASH"
        else:
            new_phase = prev_phase

    _market_wave_phase = new_phase
    if new_phase != prev_phase:
        logger.info(
            f"Wave phase: {prev_phase} → {new_phase} | "
            f"vol ${avg_vol_now:,.0f} vs baseline ${avg_baseline:,.0f} ({round(ratio,2)}x)"
        )
    return new_phase, vol_regime, avg_vol_now, avg_baseline


def _get_exit_params(wave_phase: str) -> dict:
    """Return APEX exit parameters dict for the current wave phase."""
    return APEX_EXIT_PARAMS.get(wave_phase, APEX_EXIT_PARAMS["STIRRING"])


# ══════════════════════════════════════════════════════════════════════════════
# TOKEN INTELLIGENCE HELPERS
# ══════════════════════════════════════════════════════════════════════════════

def _classify_token_type(info: dict) -> str:
    """Returns 'TECH' or 'MEME'. TECH = patient hold, MEME = extract fast."""
    combined = (
        (info.get("name")             or "") + " " +
        (info.get("symbol")           or "") + " " +
        (info.get("meta_description") or "")
    ).lower()

    tech_kw = ["ai","agent","depin","defi","rwa","chain","protocol","network",
               "compute","data","oracle","gpu","llm","model","inference",
               "node","validator","layer","zk","rollup"]
    meme_kw = ["dog","cat","pepe","frog","inu","elon","moon","baby","meme",
               "trump","shib","wojak","chad","based","degen","bonk","wif",
               "nub","pnut","giga","sigma","hamster","mouse"]

    tech_hits = sum(1 for k in tech_kw if k in combined)
    meme_hits = sum(1 for k in meme_kw if k in combined)
    return "TECH" if (tech_hits > meme_hits and tech_hits >= 2) else "MEME"


def _get_narrative_position(info: dict, contract: str) -> int:
    """
    Returns 1 (first in narrative), 2 (second), or 3+ (late copycat).
    Uses the global _narrative_tracker and _narrative_first_tokens dicts.
    """
    global _narrative_first_tokens
    symbol = (info.get("symbol") or "").lower()
    name   = (info.get("name")   or "").lower()
    active = {kw: cnt for kw, cnt in _narrative_tracker.items() if cnt >= 3}
    best   = 3
    for kw in active:
        if kw in symbol or kw in name:
            if kw not in _narrative_first_tokens:
                _narrative_first_tokens[kw] = contract
                best = 1
            elif _narrative_first_tokens[kw] == contract:
                best = 1
            else:
                best = min(best, 2)
    return best
...
def _recovery_score(info: dict, contract: str) -> tuple:
    """
    Scores a post-migration token on recovery potential 0-100.
    Based on Latuche's 3-step process:
      Step 1: Already migrated (market validated it)
      Step 2: Pico bottom confirmed (dropped hard from ATH)
      Step 3: Why would it pump again? (community, socials, dev, narrative)
    ...
    ch_m5     = info.get("ch_m5",     0)
    liq       = info.get("liq",       0)
    replies   = info.get("pf_replies",0) or 0
    graduated = info.get("pf_graduated", False)
    twitter   = info.get("twitter",  "")
    telegram  = info.get("telegram", "")
    rug_risks = info.get("rug_risks", []) or []
    ...
    if graduated:
        score += 25
        reasons.append("✅ Migrated to Raydium — market demand confirmed")
    else:
        score -= 10
        reasons.append("⚠️ Not yet migrated — demand unproven")

    # Step 2 — Pico bottom confirmed and bouncing
    if ath_mc > 0 and mc > 0:
        drop_from_ath = (ath_mc - mc) / ath_mc
        if drop_from_ath >= RECOVERY_PICO_DROP_MIN:
            score += 20
            reasons.append(
                f"✅ Deep pico confirmed — {round(drop_from_ath*100)}% from ATH "
                f"({mc_str(ath_mc)} → {mc_str(mc)})"
            )
            if ch_h1 > 0 and ch_m5 > 0:
                score += 15
                reasons.append(f"✅ Bouncing — H1: +{round(ch_h1,1)}%  M5: +{round(ch_m5,1)}%")
            elif ch_h1 > 0:
                score += 8
                reasons.append(f"⚠️ Early recovery — H1: +{round(ch_h1,1)}% (M5 not yet)")
            else:
                reasons.append("⚠️ Pico confirmed but no bounce yet — watch")
        elif drop_from_ath >= 0.40:
            score += 8
            reasons.append(f"⚠️ Moderate dip — {round(drop_from_ath*100)}% from ATH")
        else:
            reasons.append("❌ Still near ATH — not a suppressed entry")
    else:
        reasons.append("⚠️ No ATH data — cannot assess pico depth")

    # Step 3a — Community still alive
    if replies >= 200:
        score += 15
        reasons.append(f"✅ Strong community — {replies} pump.fun replies")
    elif replies >= 80:
        score += 8
        reasons.append(f"⚠️ Moderate community — {replies} replies")
    elif replies >= 20:
        score += 3
    else:
        reasons.append("❌ No meaningful community activity")

    # Step 3b — Socials
    if twitter and telegram:
        score += 10
        reasons.append("✅ Full socials — Twitter + Telegram present")
    elif twitter or telegram:
        score += 5
        reasons.append("⚠️ Partial socials only")

    # Step 3c — Dev not sold
    dev_sold = any(
        "deployer sold" in r.lower() or "creator sold" in r.lower()
        for r in rug_risks if isinstance(r, str)
    )
    if not dev_sold:
        score += 10
        reasons.append("✅ Dev has not sold — still committed")
    else:
        score -= 15
        reasons.append("❌ Dev already sold — major red flag")

    # Step 3d — Liquidity healthy
    if liq >= 20_000:
        score += 10
        reasons.append(f"✅ Healthy liquidity — {money(liq)}")
    elif liq >= 8_000:
        score += 5
        reasons.append(f"⚠️ Thin liquidity — {money(liq)}")
    else:
        score -= 10
        reasons.append(f"❌ Very thin liquidity — {money(liq)}")

    # Step 3e — Narrative position (first = banger, late = copycat rug)
    narr_pos = _get_narrative_position(info, contract)
    if narr_pos == 1:
        score += NARRATIVE_FIRST_BONUS
        reasons.append("✅ FIRST in narrative — banger potential")
    elif narr_pos == 2:
        score += NARRATIVE_SECOND_BONUS
        reasons.append("⚠️ Second token in narrative — proceed carefully")
    else:
        score -= NARRATIVE_LATE_PENALTY
        reasons.append("❌ Late narrative entry — likely copycat rug")

    return max(0, min(100, score)), reasons


# ══════════════════════════════════════════════════════════════════════════════
# ADVISORY SIGNAL BUILDER + CARD FORMATTERS
# ══════════════════════════════════════════════════════════════════════════════

def _build_advisory_signal(info: dict, contract: str, sc: dict, wave_phase: str) -> dict:
    """Assembles a complete advisory signal package for channel + APEX engine."""
    r_score, r_reasons = _recovery_score(info, contract)
    token_type         = _classify_token_type(info)
    narr_pos           = _get_narrative_position(info, contract)
    exit_params        = _get_exit_params(wave_phase)

    phase_meta = {
        "SILENCE":  ("ACCUMULATION", "🔵", "ACCUMULATION CALL", "Days–weeks", "Medium"),
        "STIRRING": ("EARLY_WAVE",   "🟡", "EARLY WAVE CALL",   "Hours–days", "Low-Med"),
        "WAVE":     ("MOMENTUM",     "🟢", "MOMENTUM CALL",     "Hours — extract fast", "Medium"),
        "CRASH":    ("NO_CALL",      "🔴", "NO CALL",           "N/A", "N/A"),
        "RESET":    ("NO_CALL",      "🔴", "NO CALL",           "N/A", "N/A"),
    }
    sig_type, sig_icon, sig_label, hold_time, risk_level = phase_meta.get(
        wave_phase, ("EARLY_WAVE", "🟡", "EARLY WAVE CALL", "Hours–days", "Low-Med")
    )
    ...
    return {
        ...
        "recovery_score":   r_score,
        "recovery_reasons": r_reasons,
        "token_type":       token_type,
        "narrative_pos":    narr_pos,
        "sniper_score":     sc.get("score", 0),
...
def _advisory_channel_card(signal: dict) -> str:
    """Full HTML signal card for the call channel."""
    sym       = signal["symbol"].upper()
    name      = signal["name"]
    mc        = signal["mc"]
    ath_mc    = signal["ath_mc"]
    liq       = signal["liq"]
    wave      = signal["wave_phase"]
    sig_icon  = signal["signal_icon"]
    sig_label = signal["signal_label"]
    r_score   = signal["recovery_score"]
    r_reasons = signal["recovery_reasons"]
    tok_type  = signal["token_type"]
    narr_pos  = signal["narrative_pos"]
    hold_time = signal["hold_time"]
    risk      = signal["risk_level"]
    replies   = signal["pf_replies"]
    twitter   = signal["twitter"]
    telegram  = signal["telegram"]
    s_score   = signal["sniper_score"]
    graduated = signal["graduated"]
    ...
    wave_ctx = {
        "SILENCE":  "🔵 Market is quiet. Accumulation phase — buy the FUD before the wave.",
        "STIRRING": "🟡 Wave is forming. First movers entering. Early positioning window.",
        "WAVE":     "🟢 Wave is active. Crowd entering. Extract fast — sell into the FOMO.",
    }.get(wave, "")

    narr_line = {1: "🥇 <b>FIRST in narrative</b> — banger potential\n",
                 2: "🥈 Second in narrative — proceed carefully\n"
                 }.get(narr_pos, "⚠️ Late narrative entry — higher risk\n")

    type_line = ("🤖 TECH — patient hold\n" if tok_type == "TECH"
                 else "🎭 MEME — extract fast at euphoria\n")

    ath_line = ""
    if ath_mc > 0 and mc > 0 and ath_mc > mc:
        drop = round((ath_mc - mc) / ath_mc * 100)
        ath_line = f"🏔 ATH: <b>{_mf(ath_mc)}</b>  (−{drop}% from peak)\n"

    soc_parts = []
    if twitter:  soc_parts.append(f'<a href="{twitter}">🐦 Twitter</a>')
    if telegram: soc_parts.append(f'<a href="{telegram}">💬 Telegram</a>')
    soc_line = "  ·  ".join(soc_parts) if soc_parts else "<i>No socials</i>"

    top_reasons = "\n".join(f"  {r}" for r in r_reasons[:4])

    dex_ch = {"solana":"solana","ethereum":"ethereum","base":"base","bsc":"bsc"}.get(chain, chain)
    _ds    = f"https://dexscreener.com/{dex_ch}/{contract}"
    _pf    = f"https://pump.fun/{contract}"
    dex_links = f'<a href="{_ds}">DexScreener</a>'
    if chain in ("solana", "sol"):
        dex_links += f'  ·  <a href="{_pf}">PumpFun</a>'

    return (
        f"{sig_icon} <b>{sig_label}</b>\n"
        f"━━━━━━━━━━━━━━━━━━━━━━\n"
        f"<b>${sym}</b>  ·  {name}  ·  {chain.upper()}\n"
        f"<code>{contract}</code>\n\n"
        f"<i>{wave_ctx}</i>\n\n"
        f"━━━━━━━━━━━━━━━━━━━━━━\n"
        f"💰 MC: <b>{_mf(mc)}</b>\n"
        f"{ath_line}"
        f"💧 Liq: <b>{_mf(liq)}</b>  ·  "
        f"{'✅ Migrated' if graduated else '⚠️ Not migrated'}\n"
        f"💬 Community: <b>{replies} replies</b>\n\n"
        f"━━━━━━━━━━━━━━━━━━━━━━\n"
        f"📊 Recovery Score: <b>{r_score}/100</b>\n"
        f"🧠 Sniper Score:   <b>{s_score}/100</b>\n\n"
        f"{narr_line}"
        f"{type_line}\n"
        f"<b>Analysis:</b>\n{top_reasons}\n\n"
        f"━━━━━━━━━━━━━━━━━━━━━━\n"
        f"⏱ Hold: <b>{hold_time}</b>  ·  ⚠️ Risk: <b>{risk}</b>\n\n"
        f"🔗 {soc_line}\n"
        f"{dex_links}"
    )


def _advisory_dm_card(signal: dict) -> str:
    """Compact Markdown card for Mode 2 user approval DM."""
    sym      = signal["symbol"].upper()
    mc       = signal["mc"]
    ath_mc   = signal["ath_mc"]
    r_score  = signal["recovery_score"]
    ...
    narr_txt = {1: "🥇 First", 2: "🥈 Second"}.get(narr_pos, "⚠️ Late")
    ath_txt  = f"  |  ATH: {_mf(ath_mc)}" if ath_mc > 0 else ""

    return (
        f"{sig_icon} *ADVISORY — ${_md(sym)}*\n\n"
        f"Wave: *{wave}*  |  Recovery: *{r_score}/100*\n"
        f"MC: *{_mf(mc)}*{ath_txt}\n"
        f"Type: *{tok_type}*  |  Narrative: *{narr_txt}*\n\n"
        f"_Tap View Analysis for full research card_"
    )


async def advisory_job(ctx: ContextTypes.DEFAULT_TYPE) -> None:
    """
    Advisory Engine — runs every 5 minutes.

    Strategy (Latuche + FEXIR framework):
      1. Detect wave phase from vol_h1 rolling average across all scanned tokens
      2. Scan post-migration tokens (Latuche Step 1: already migrated = validated)
      3. Score each on recovery: pico bottom + community + dev + narrative position
      4. Post signal card to broadcast channel
      5. Mode 1 (auto): queue into APEX entry queue with wave-adjusted size
         Mode 2 (approval): send compact DM for user to approve/reject
    """
    global _market_wave_phase

    try:
        active_users = [
            (uid, ud) for uid, ud in users.items()
            if (ud.get("apex_mode") or ud.get("sniper_advisory"))
            and ud.get("balance") is not None
        ]
        if not active_users:
            return
        ...
        # Detect wave phase
        wave_phase, vol_regime, vol_now, vol_baseline = _detect_market_wave_phase(raw_tokens)

        # No calls during crash or reset
        if wave_phase in ("CRASH", "RESET"):
            logger.info(f"Advisory: {wave_phase} — no new signals")
            for uid, ud in active_users:
                if ud.get("apex_mode") and not ud.get("_wave_pause_notified"):
                    ud["_wave_pause_notified"] = True
                    try:
                        await ctx.bot.send_message(
                            chat_id=uid, parse_mode="Markdown",
                            text=(
                                "🌊 *MARKET WAVE UPDATE*\n\n"
                                f"Phase: *{wave_phase}*\n"
                                "Advisory signals paused — market not in entry window.\n"
                                "APEX is protecting existing positions.\n"
                                "_Signals resume automatically when market stabilises._"
                            )
                        )
                    except Exception:
                        pass
            ...
            for uid, ud in active_users:
                ud.pop("_wave_pause_notified", None)

        # Pre-fetch all token data in parallel
        await _asyncio.gather(
            *[get_token(c["tokenAddress"], force=True) for c in raw_tokens],
            return_exceptions=True
        )
        ...
        for item in raw_tokens:
            if signals_sent >= ADVISORY_MAX_SIGNALS_PER_CYCLE:
                break
            ...
            try:
                ...
                # Enrich with pump.fun metadata from scan item
                info["pf_curve"]     = item.get("_pf_curve")
                info["pf_replies"]   = item.get("_pf_replies", 0)
                info["pf_graduated"] = item.get("_pf_graduated", False)
                info["pf_dev_pct"]   = item.get("_pf_dev_pct")
                ...
                # Advisory focuses exclusively on migrated tokens (Latuche Step 1)
                if not info.get("pf_graduated"):
                    continue

                # Recovery score gate
                r_score, r_reasons = _recovery_score(info, contract)
                if r_score < ADVISORY_MIN_RECOVERY_SCORE:
                    continue

                # Still need a clean sniper score (hard flags block regardless)
                sc = sniper_score(info)
                if sc.get("flags"):
                    continue

                signal = _build_advisory_signal(info, contract, sc, wave_phase)
                signals_sent += 1

                for uid, ud in active_users:
                    seen = _advisory_seen.setdefault(uid, {})
                    if contract in seen:
                        if _time.time() - seen[contract] < ADVISORY_SIGNAL_EXPIRY_H * 3600:
                            continue
                    seen[contract] = _time.time()

                    ch_id = ud.get("sniper_broadcast_channel")

                    # Post full card to channel
                    if ch_id:
                        try:
                            html  = _advisory_channel_card(signal)
                            chain_raw = info.get("chain", "solana").lower()
                            dex_ch    = {"solana":"solana","ethereum":"ethereum",
                                         "base":"base","bsc":"bsc"}.get(chain_raw, chain_raw)
                            ds_url = f"https://dexscreener.com/{dex_ch}/{contract}"
                            bot_url = (
                                f"https://t.me/{_bot_username}?start={contract}"
                                if _bot_username else ds_url
                            )
                            await ctx.bot.send_message(
                                chat_id=int(ch_id), text=html, parse_mode="HTML",
                                reply_markup=InlineKeyboardMarkup([
                                    [InlineKeyboardButton("🤖 Trade on APEX", url=bot_url)],
                                    [InlineKeyboardButton("🔍 View Live",     url=ds_url)],
                                ]),
                                disable_web_page_preview=True
                            )
                        except Exception as _ce:
                            logger.warning(f"Advisory channel post failed: {_ce}")

                    # Feed to APEX engine
                    if ud.get("apex_mode"):
                        base_amt  = ud.get("sniper_filters", {}).get("buy_amount", 50.0)
                        size_mult = round(r_score / 100, 2)
                        adj_amt   = max(1.0, round(base_amt * size_mult, 2))

                        if ud.get("apex_advisory_mode2", False):
                            # Mode 2 — send DM for approval
                            try:
                                await ctx.bot.send_message(
                                    chat_id=uid, parse_mode="Markdown",
                                    text=_advisory_dm_card(signal),
                                    reply_markup=InlineKeyboardMarkup([
                                        [InlineKeyboardButton(
                                            "✅ Approve", callback_data=f"adv_approve_{contract}")],
                                        [InlineKeyboardButton(
                                            "👁 Full Analysis", callback_data=f"adv_view_{contract}"),
                                         InlineKeyboardButton(
                                            "❌ Reject", callback_data=f"adv_reject_{contract}")],
                                    ])
                                )
                                ud.setdefault("_pending_advisory_signals", {})[contract] = signal
                            except Exception as _de:
                                logger.error(f"Advisory DM error {uid}: {_de}")
                        else:
                            # Mode 1 — auto-queue into APEX entry queue
                            if contract not in _apex_entry_queue.get(uid, {}):
                                _apex_entry_queue.setdefault(uid, {})[contract] = {
                                    "info":      info,
                                    "sc":        sc,
                                    "ai": {
                                        "verdict":          signal["signal_type"],
                                        "confidence":       min(10, round(r_score / 10)),
                                        "thesis":           next(
                                            (r for r in r_reasons if r.startswith("✅")),
                                            "Recovery signal detected."
                                        ),
                                        "suggested_amount": adj_amt,
                                        "rug_risk":         "LOW" if r_score >= 70 else "MEDIUM",
                                        "momentum":         "STRONG" if wave_phase == "WAVE" else "MODERATE",
                                        "social_score":     "GOOD" if signal["pf_replies"] > 100 else "PARTIAL",
                                        "red_flags":        [],
                                        "green_flags":      [r for r in r_reasons if r.startswith("✅")][:3],
                                    },
                                ...
                                logger.info(
                                    f"Advisory queued ${signal['symbol']} "
                                    f"recovery={r_score} wave={wave_phase} uid={uid}"
                                )

            except Exception as _te:
                logger.warning(f"Advisory token error {contract}: {_te}")

    except Exception as e:
        logger.error(f"advisory_job crashed: {e}", exc_info=True)
...
async def enrich_holder_distribution(contract: str, chain: str, http_client) -> dict:
    ...
    if chain_lower in ("solana", "sol"):
        try:
            ...
            # Returning empty holders list — holder data comes from Helius maker_pct instead.
            return []  # solscan_dead
...
async def do_buy_core(ud: dict, uid: int, contract: str, usd_amount: float, planned: bool = True, mood: str = "", vault_buy: bool = False) -> str | tuple:
    ...
    # tokens = how many tokens you actually receive after fees
    # cost basis = usd_amount (what you spent before fees — used for PnL calc)
    _effective_spend = usd_amount - _fee_cost   # net capital deployed into the token
    tokens = _effective_spend / info["price"] if _effective_spend > 0 else usd_amount / info["price"]

    if vault_buy:
        ud["apex_vault"] = round(ud.get("apex_vault", 0.0) - usd_amount, 4)
    else:
        ud["balance"] = round(ud["balance"] - usd_amount, 4)
    ud["daily_trades"] += 1
...
async def btn(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
            ...
            for uid_str, m in members.items():
                try:
                    ...
                    if m_ud:
                        ...
                        comp_trades = [t for t in logs if _safe_dt(t.get("closed_at")) >= join_dt]
    ...
    # ── APEX VAULT MENU ───────────────────────────────────────────────────────
        ...
        _mode2_on = ud.get("apex_advisory_mode2", False)
        _mode2_lbl = "🤖 Mode: Manual Approval ✅" if _mode2_on else "🤖 Mode: Auto-Trade ✅"
        _mode2_sub = "(tap each signal to approve)" if _mode2_on else "(APEX buys automatically)"
        await q.edit_message_text(
            "⚙️ *APEX SETTINGS*\n\n"
            "Min confidence: *" + str(conf_thr) + "/10*\n"
            "Min score: *" + str(score_min) + "/100*\n"
            ...
            "🤖 *Entry Mode:* " + ("Manual Approval" if _mode2_on else "Auto-Trade") + "\n"
            "_" + _mode2_sub + "_\n\n"
            "🎛 *Risk Profile:* " + _hp_label + "\n"
            "_Self-learning engine suggests adjustments after daily trades._",
            parse_mode="Markdown",
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("🎛 Change Risk Profile",   callback_data="apex_profile_menu")],
                [InlineKeyboardButton(_mode2_lbl,                 callback_data="apex_mode2_toggle")],
    ...
    elif cb == "apex_mode2_toggle":
        ud["apex_advisory_mode2"] = not ud.get("apex_advisory_mode2", False)
        mode2_on = ud["apex_advisory_mode2"]
        save_user(u.id, ud)
        await q.edit_message_text(
            "🤖 *APEX ENTRY MODE*\n\n"
            + ("*Manual Approval Mode* ✅\n\n"
               "Each advisory signal will be sent to your DM.\n"
               "You tap Approve or Reject — APEX only trades on your say-so."
               if mode2_on else
               "*Auto-Trade Mode* ✅\n\n"
               "APEX automatically buys qualifying advisory signals.\n"
               "Recovery score scales position size — stronger signals = bigger bet."),
            parse_mode="Markdown",
            reply_markup=InlineKeyboardMarkup([[
    ...
    # ── ADVISORY APPROVAL CALLBACKS ───────────────────────────────────────────

    elif cb.startswith("adv_approve_"):
        contract = cb[12:]
        signal   = ud.get("_pending_advisory_signals", {}).get(contract)
        if not signal:
            await q.edit_message_text(
                "⏰ Signal expired — please wait for the next scan cycle.",
                reply_markup=back_main()
            )
            return
        # Queue into APEX entry queue
        base_amt  = ud.get("sniper_filters", {}).get("buy_amount", 50.0)
        r_score   = signal.get("recovery_score", 50)
        size_mult = round(r_score / 100, 2)
        adj_amt   = max(1.0, round(base_amt * size_mult, 2))
        wave_phase = signal.get("wave_phase", "STIRRING")
        info_adv  = await get_token(contract)
        if not info_adv:
            await q.edit_message_text("Token data unavailable.", reply_markup=back_main())
            return
        sc_adv = sniper_score(info_adv)
        _apex_entry_queue.setdefault(u.id, {})[contract] = {
            "info":      info_adv,
            "sc":        sc_adv,
            "ai": {
                "verdict":          signal.get("signal_type", "EARLY_WAVE"),
                "confidence":       min(10, round(r_score / 10)),
                "thesis":           next(
                    (r for r in signal.get("recovery_reasons", []) if r.startswith("✅")),
                    "Approved advisory signal."
                ),
                "suggested_amount": adj_amt,
                "rug_risk":         "LOW" if r_score >= 70 else "MEDIUM",
                "momentum":         "STRONG" if wave_phase == "WAVE" else "MODERATE",
                "social_score":     "GOOD" if signal.get("pf_replies", 0) > 100 else "PARTIAL",
                "red_flags":        [],
                "green_flags":      [r for r in signal.get("recovery_reasons", []) if r.startswith("✅")][:3],
            },
            "queued_at":  datetime.now(),
            "base_amount": adj_amt,
            "wave_phase":  wave_phase,
        ...
        ud.get("_pending_advisory_signals", {}).pop(contract, None)
        await q.edit_message_text(
            "✅ *Advisory Signal Approved*\n\n"
            "*$" + _md(signal.get("symbol", "?")) + "* queued for APEX entry.\n"
            "Position size: *" + money(adj_amt) + "* "
            "(recovery score " + str(r_score) + "/100)\n\n"
            "_APEX will confirm entry in ~45 seconds._",
            parse_mode="Markdown",
            reply_markup=InlineKeyboardMarkup([[
                InlineKeyboardButton("⚡ APEX Menu", callback_data="apex_menu"),
    ...
    elif cb.startswith("adv_reject_"):
        contract = cb[11:]
        sym = ud.get("_pending_advisory_signals", {}).get(contract, {}).get("symbol", "?")
        ud.get("_pending_advisory_signals", {}).pop(contract, None)
        # Suppress this contract for ADVISORY_SIGNAL_EXPIRY_H
        _advisory_seen.setdefault(u.id, {})[contract] = _time.time()
        await q.edit_message_text(
            "❌ *Signal Rejected*\n\n"
            "*$" + _md(sym) + "* suppressed for " + str(int(ADVISORY_SIGNAL_EXPIRY_H)) + "h.",
            parse_mode="Markdown",
            reply_markup=back_main()
        )

    elif cb.startswith("adv_view_"):
        contract = cb[9:]
        signal   = ud.get("_pending_advisory_signals", {}).get(contract)
        if not signal:
            await q.edit_message_text("⏰ Signal expired.", reply_markup=back_main())
            return
        wave_phase = signal.get("wave_phase", "STIRRING")
        r_score    = signal.get("recovery_score", 0)
        reasons    = signal.get("recovery_reasons", [])
        tok_type   = signal.get("token_type", "MEME")
        narr_pos   = signal.get("narrative_pos", 3)
        narr_txt   = {1: "🥇 First", 2: "🥈 Second"}.get(narr_pos, "⚠️ Late")
        reasons_txt = "\n".join("  " + r for r in reasons[:6])
        await q.edit_message_text(
            "👁 *FULL ADVISORY ANALYSIS*\n\n"
            "*$" + _md(signal.get("symbol", "?")) + "*  " + signal.get("chain","SOL").upper() + "\n"
            "`" + contract + "`\n\n"
            "🌊 *Wave Phase:* " + wave_phase + "\n"
            "📊 *Recovery Score:* " + str(r_score) + "/100\n"
            "🎭 *Token Type:* " + tok_type + "\n"
            "🏷 *Narrative Position:* " + narr_txt + "\n\n"
            "📋 *Analysis:*\n" + reasons_txt,
            parse_mode="Markdown",
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("✅ Approve", callback_data="adv_approve_" + contract)],
                [InlineKeyboardButton("❌ Reject",  callback_data="adv_reject_"  + contract)],
                [InlineKeyboardButton("🪙 View Token Card", callback_data="btt_" + contract)],
            ])
        )

    # ── APEX MANUAL CONTROL (from token card button) ──────────────────────────

    elif cb.startswith("apex_manual_status_"):
        contract = cb[19:]
        h = ud.get("holdings", {}).get(contract, {})
        trail_stop = h.get("apex_trail_stop")
        threat     = h.get("apex_threat", "CLEAR")
        cx_now     = 0.0
        info_m = await get_token(contract)
        if info_m and h.get("avg_price", 0) > 0:
            cx_now = round(info_m["price"] / h["avg_price"], 3)
        threat_icon = {"CLEAR": "🟢", "YELLOW": "🟡", "ORANGE": "🟠", "RED": "🔴"}.get(threat, "⚪")
        wave_info   = f"Wave: *{_market_wave_phase}*"
        await q.edit_message_text(
            "🤖 *APEX MANAGING THIS POSITION*\n\n"
            "*$" + _md(h.get("symbol", "?")) + "*\n"
            "Current: *" + str(cx_now) + "x*\n"
            "Threat: " + threat_icon + " *" + threat + "*\n"
            "Trail stop: *" + (money(trail_stop) if trail_stop else "not yet active") + "*\n"
            + wave_info + "\n\n"
            "_APEX is actively managing this position.\n"
            "Remove APEX control to manage manually._",
            parse_mode="Markdown",
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("🗑 Remove APEX Control",
                                      callback_data="apex_manual_remove_" + contract)],
                [InlineKeyboardButton("◀ Back to Token", callback_data="btt_" + contract)],
            ])
        )

    elif cb.startswith("apex_manual_remove_"):
        contract = cb[19:]
        _apex_manual_positions.discard(contract)
        if contract in ud.get("holdings", {}):
            ud["holdings"][contract]["mood"] = ""
        sym = ud.get("holdings", {}).get(contract, {}).get("symbol", contract[:8])
        await q.edit_message_text(
            "✅ *APEX Control Removed*\n\n"
            "*$" + _md(sym) + "* is now in manual mode.\n"
            "APEX will no longer manage exits for this position.",
            parse_mode="Markdown",
            reply_markup=InlineKeyboardMarkup([[
                InlineKeyboardButton("◀ Back to Token", callback_data="btt_" + contract)
            ]])
    ...
    elif cb.startswith("apex_manual_"):
        # Token card APEX Control button — not a status or remove action
        contract = cb[12:]
        if not ud.get("apex_mode"):
            try:
                await q.edit_message_text(
                    "⚡ *APEX Control*\n\n"
                    "APEX Mode is currently OFF.\n\n"
                    "Enable it from Sniper → APEX Engine to use wave-intelligent position management.",
                    parse_mode="Markdown",
                    reply_markup=InlineKeyboardMarkup([
                        [InlineKeyboardButton("⚡ Enable APEX", callback_data="apex_toggle")],
                        [InlineKeyboardButton("◀ Back",        callback_data="btt_" + contract)],
                    ])
                )
            except Exception:
                await ctx.bot.send_message(
                    chat_id=q.message.chat_id,
                    text="⚡ APEX Mode is OFF. Enable it from Sniper → APEX Engine first.",
                    reply_markup=InlineKeyboardMarkup([
                        [InlineKeyboardButton("⚡ Enable APEX", callback_data="apex_toggle")],
                    ])
                )
        ...
        info_m = await get_token(contract)
        sym    = info_m["symbol"] if info_m else contract[:8]
        wave   = _market_wave_phase

        # ── Plain-English wave descriptions ──────────────────────────────────
        wave_plain = {
            "SILENCE":  ("😴", "Quiet market", "Volume is low across the board. APEX holds patiently and uses wider exits so it doesn't get shaken out of a position that hasn't moved yet."),
            "STIRRING": ("👀", "Market waking up", "Activity is picking up. APEX uses standard exits — trail activates at 1.5x, partial sell at 1.3x to lock profit early."),
            "WAVE":     ("🌊", "Active wave — extract fast", "Market is moving. APEX tightens exits to lock gains quickly before momentum fades. Trail activates earlier, sells more at each level."),
            "CRASH":    ("🚨", "Market crashing — defensive", "Broad market is dumping. APEX is in defensive mode — very tight stops, exits faster than normal. New buys are higher risk."),
            "RESET":    ("⏳", "Between cycles", "The last wave finished. APEX is waiting for the next one to form. Standard exits apply. Good time to enter — next wave usually comes within 30–60 min."),
        }
        wave_icon, wave_short, wave_detail = wave_plain.get(wave, ("⚡", wave, "Standard APEX management."))

        ep = _get_exit_params(wave)
        trail_early_pct = round(ep["trail_pct_early"] * 100)
        trail_mid_pct   = round(ep["trail_pct_mid"] * 100)
        ladder_2x_pct   = round(ep["ladder_2x_pct"] * 100)

        # Current position info if already holding
        holding_line = ""
        if contract in ud.get("holdings", {}):
            h_m = ud["holdings"][contract]
            cx_m = (info_m["price"] / h_m["avg_price"]) if info_m and h_m.get("avg_price", 0) > 0 else 0
            managed = h_m.get("mood") in ("APEX", "AI-Sniper", "APEX-DCA") or contract in _apex_manual_positions
            holding_line = (
                "\n📌 *You hold this token*  (" + str(round(cx_m, 2)) + "x)\n"
                + ("🤖 APEX is managing it\n" if managed else "⚠️ APEX is NOT managing it — tap Activate below\n")
            )

        text = (
            "🤖 *APEX CONTROL — $" + _md(sym) + "*\n\n"
            + wave_icon + " *Market Phase: " + wave_short + "*\n"
            "_" + wave_detail + "_\n"
            + holding_line + "\n"
            "━━━━━━━━━━━━━━━━\n"
            "*What APEX will do for this position:*\n\n"
            "🛡 Before *2x* — SL active, protecting your capital\n"
            "💰 At *2x* — sells *50%* → initial capital returned to vault\n"
            "📈 At *2x+* — trail activates on remaining 50% (pure profit)\n"
            "   _" + str(trail_early_pct) + "% early  /  " + str(trail_mid_pct) + "% mid  /  tightens the higher it goes_\n"
            "🚨 Real rug detected (LP drain) — emergency exit at any point\n"
            "🔍 Price dips — structure analysis before deciding to exit\n\n"
            "_Tap a buy button — APEX manages from the moment you're in._"
        )
        kb = InlineKeyboardMarkup([
            [InlineKeyboardButton("⚡ Buy $25  + APEX",  callback_data="apex_mbuy_25_"  + contract),
             InlineKeyboardButton("⚡ Buy $50  + APEX",  callback_data="apex_mbuy_50_"  + contract)],
            [InlineKeyboardButton("⚡ Buy $100 + APEX",  callback_data="apex_mbuy_100_" + contract),
             InlineKeyboardButton("⚡ Buy $250 + APEX",  callback_data="apex_mbuy_250_" + contract)],
            [InlineKeyboardButton("🤖 Activate on Existing Position",
                                  callback_data="apex_mactivate_" + contract)],
            [InlineKeyboardButton("◀ Back", callback_data="btt_" + contract)],
        ])
        try:
            await q.edit_message_text(text, reply_markup=kb)
        except Exception:
            await ctx.bot.send_message(chat_id=q.message.chat_id, text=text, reply_markup=kb)

    elif cb.startswith("apex_mbuy_"):
        # Buy + activate APEX control in one tap
        rest     = cb[10:]
        amt_str, contract = rest.split("_", 1)
        amount   = float(amt_str)
        if amount > ud.get("balance", 0):
            await q.edit_message_text(
                "❌ Insufficient balance — " + money(ud.get("balance", 0)) + " available.",
                reply_markup=InlineKeyboardMarkup([[
                    InlineKeyboardButton("◀ Back", callback_data="apex_manual_" + contract)
                ]])
            )
            return
        result = await do_buy_core(ud, u.id, contract, amount, planned=True, mood="APEX")
        if isinstance(result, str):
            await q.edit_message_text(result, reply_markup=back_main())
            return
        info_b, _ = result
        _apex_manual_positions.add(contract)
        # Apply wave-aware exit params to the holding
        ep   = _get_exit_params(_market_wave_phase)
        h    = ud.get("holdings", {}).get(contract, {})
        if h:
            h["apex_trail_stop"]         = None
            h["apex_peak_price"]         = info_b["price"]
            h["apex_threat"]             = "CLEAR"
            h["apex_vault_locked"]       = {}
            h["apex_ladder_sold_1_3x"]   = False
            h["apex_ladder_sold_2x"]     = False
            h["apex_entry_score"]        = 50
            h["apex_entry_conf"]         = 5
            h["liq_at_buy"]              = info_b.get("liq", 0)
            h["apex_entry_rug"]          = "LOW"
            h["sr_history"]              = []
            h["apex_profile_at_entry"]   = "default"
            h["stop_loss_pct"]           = 22.0  # default LOW rug risk SL
            h["apex_hunter_floor"]       = 0.0
            h["apex_dca_count"]          = 0
            h["apex_last_dca_ts"]        = 0.0
            h["apex_wave_phase"]         = _market_wave_phase
        save_user(u.id, ud)
        await q.edit_message_text(
            "✅ *APEX CONTROL ACTIVATED*\n\n"
            "*$" + _md(info_b["symbol"]) + "*\n"
            "Bought: *" + money(amount) + "*\n"
            "Wave: *" + _market_wave_phase + "*\n"
            "APEX will manage this position with wave-aware exits.",
            parse_mode="Markdown",
            reply_markup=InlineKeyboardMarkup([
    ...
    elif cb.startswith("apex_mactivate_"):
        # Activate APEX on an already-open position
        contract = cb[15:]
        if contract not in ud.get("holdings", {}):
            await q.edit_message_text("Position not found.", reply_markup=back_main())
            return
        h = ud["holdings"][contract]
        h["mood"]                    = "APEX"
        h["apex_trail_stop"]         = None
        h["apex_peak_price"]         = h.get("peak_price", h.get("avg_price", 0))
        h["apex_threat"]             = "CLEAR"
        h["apex_vault_locked"]       = {}
        h["apex_ladder_sold_1_3x"]   = False
        h["apex_ladder_sold_2x"]     = False
        h["apex_entry_score"]        = 50
        h["apex_entry_conf"]         = 5
        h["liq_at_buy"]              = h.get("liq_at_buy", 0)
        h["apex_entry_rug"]          = "LOW"
        h["sr_history"]              = h.get("sr_history", [])
        h["apex_profile_at_entry"]   = "default"
        if not h.get("stop_loss_pct"):
            h["stop_loss_pct"]       = 22.0
        h["apex_hunter_floor"]       = 0.0
        h["apex_dca_count"]          = h.get("apex_dca_count", 0)
        h["apex_last_dca_ts"]        = h.get("apex_last_dca_ts", 0.0)
        h["apex_wave_phase"]         = _market_wave_phase
        _apex_manual_positions.add(contract)
        save_user(u.id, ud)
        await q.edit_message_text(
            "✅ *APEX Activated on Existing Position*\n\n"
            "*$" + _md(h.get("symbol","?")) + "*\n"
            "Wave: *" + _market_wave_phase + "*\n"
            "SL: *22%* (default LOW risk)\n\n"
            "APEX will manage exits from now on.",
            parse_mode="Markdown",
            reply_markup=InlineKeyboardMarkup([[
                InlineKeyboardButton("📊 View Position", callback_data="btt_" + contract)
            ]])
        ...
        await q.edit_message_text(
            ...
            "• Entry: -20% SL (same as Default — no wide stops on losers)\n"
            "• At 1.3x: floor moves to -5% from entry\n"
            "• At 1.6x: floor locks to break-even — *can no longer lose*\n"
            "• Trail: 30–38% wide once in profit zone\n"
            "• After 1.3x: needs 2× RED signal to exit (no wick-outs)\n"
...
async def dump_log_drain_job(ctx: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    for uid, queue in list(_dl_queue.items()):
        ...
        ud = users.get(uid)
        sent = 0
        real_fails = 0   # only non-flood failures count toward disable
...
def main():
    ...
    # advisory_job removed — was spamming MARKET WAVE CRASH every 5 min
    app.job_queue.run_repeating(kol_tracker_job,       interval=300,  first=120)
    app.job_queue.run_repeating(channel_milestone_job, interval=300,  first=150)
    app.job_queue.run_repeating(autosave_job,          interval=120,  first=120)
    app.job_queue.run_repeating(dump_log_drain_job,    interval=DL_DRAIN_EVERY, first=5)
    ...
    app.job_queue.run_daily(survival_report_job,         time=__import__("datetime").time(9, 15))  # 24h scan survival report
