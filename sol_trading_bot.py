            ...
            for uid, ud in active_users:
                try:
                    ...
                    if helius_key and info.get("chain","").lower() in ("solana","sol"):
                        ...
                        # ── Sajad S1-02: SOL balance uniformity check ─────────
                        try:
                            _top_h2 = info.get("holder_distribution", [])
                            _sol_bal = await check_holder_sol_balances(_top_h2, helius_key)
                            info.update(_sol_bal)
                        except Exception as _sbe:
                            logger.debug(f"SOL balance check error: {_sbe}")

                        # ── Sajad S1-02: Global fees monitor ─────────────────
                        try:
                            _gfees = await check_global_fees(contract, helius_key)
                            info.update(_gfees)
                        except Exception as _gfe:
                            logger.debug(f"Global fees error: {_gfe}")

                        # ── Sajad S1-02: Pro trader count ─────────────────────
                        try:
                            _top_h3 = info.get("holder_distribution", [])
                            _pro = await check_pro_traders(contract, _top_h3, helius_key)
                            info.update(_pro)
                        except Exception as _pte:
                            logger.debug(f"Pro trader check error: {_pte}")
