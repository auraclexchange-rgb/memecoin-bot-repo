                    ...
                    if cx >= 1.3 and not h.get("apex_ladder_sold_1_3x") and _ladder_cv >= 0.02:
                        ...
                        if _sell_val >= 0.01:
                            ...
                            if isinstance(_lad_result, dict):
                                ...
                                # ── BREAK-EVEN FLOOR FIX ──────────────────────
                                # Lock floor to entry price immediately when 1.3x
                                # ladder fires. Without this, stop_loss_pct can
                                # still trigger below avg and wipe the gain.
                                # Use hunter_floor — position manager respects it
                                # over stop_loss_pct in all exit paths.
                                _old_sl = h.get("stop_loss_pct")
                                h["apex_hunter_floor"] = avg * 1.002  # entry + 0.2% buffer
                                h["stop_loss_pct"]     = None          # disable pct SL
                                h.setdefault("stop_loss_history", []).append({
                                    "old":    _old_sl,
                                    "new":    None,
                                    "source": "apex_ladder_be_floor",
                                    "cx":     round(cx, 3),
                                    "ts":     _time.time(),
                                })
                                # ─────────────────────────────────────────────
                                try:
                                    await app.bot.send_message(
                                        ...
                                        text=(
                                            ...
                                            "Sold *25%* (~" + money(_lad_sold_usd) + ") → locked *" + pstr(_lad_pnl) + "*\n"
                                            "🛡 Break-even floor locked ✅\n"
                                            "_(cannot lose money on this position now)_\n"
                        ...
                        if _ladder_cv2 >= 0.02:
                            ...
                            if _sell_val2 >= 0.01:
                                ...
                                if isinstance(_lad_result2, dict):
                                    ...
                                    _lad_sold_usd2 = round(_sell_val2, 2)
                                    try:
                                        await app.bot.send_message(
                                            chat_id=uid, parse_mode="Markdown",
                                            text=(
                                                "💰 *APEX — LADDER SELL (2x)*\n\n"
                                                "*$" + _md(h["symbol"]) + "*  hit *2x*\n"
                                                "Sold *25%* more (~" + money(_lad_sold_usd2) + ") → locked *" + pstr(_lad_pnl2) + "*\n"
...
async def btn(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # ── APEX MANUAL CONTROL (from token card button) ──────────────────────────
        ...
        mc_now = mc_str(info_m["mc"]) if info_m else "unknown"
        wave   = _market_wave_phase
        ep     = _get_exit_params(wave)
        wave_desc = {
            "SILENCE":  "Quiet market — accumulation phase. Patient hold strategy.",
            "STIRRING": "Wave forming — early positioning. Standard exits.",
            "WAVE":     "Wave active — extract fast. Tight trail, ladder sells.",
            "CRASH":    "Market crashing — defensive mode only.",
            "RESET":    "Market resetting — wait for SILENCE phase.",
        }.get(wave, "")
        await q.edit_message_text(
            "🤖 *APEX CONTROL — $" + _md(sym) + "*\n\n"
            "Current wave: *" + wave + "*\n"
            "_" + wave_desc + "_\n\n"
            "Exit parameters for this wave:\n"
            "  Trail activates: *" + str(ep["trail_activate_x"]) + "x*\n"
            "  Trail width: *" + str(round(ep["trail_pct"]*100)) + "%*\n"
            "  Ladder sell: *" + str(round(ep["ladder_pct"]*100)) + "%* at " + str(ep["ladder_x"]) + "x\n\n"
            "APEX will manage this position with wave-aware exits.\n"
            "Buy first — then APEX takes over.",
            parse_mode="Markdown",
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("⚡ Buy $25  + APEX",  callback_data=f"apex_mbuy_25_{contract}"),
                 InlineKeyboardButton("⚡ Buy $50  + APEX",  callback_data=f"apex_mbuy_50_{contract}")],
                [InlineKeyboardButton("⚡ Buy $100 + APEX",  callback_data=f"apex_mbuy_100_{contract}"),
                 InlineKeyboardButton("⚡ Buy $250 + APEX",  callback_data=f"apex_mbuy_250_{contract}")],
                [InlineKeyboardButton("🤖 Activate on Existing Position",
                                      callback_data=f"apex_mactivate_{contract}")],
                [InlineKeyboardButton("◀ Back", callback_data="btt_" + contract)],
            ])
        )
