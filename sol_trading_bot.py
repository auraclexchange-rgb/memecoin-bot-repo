...
APEX_HUNTER_SUSPENDED = os.getenv("APEX_HUNTER_SUSPENDED", "true").lower() == "true"
...
def generate_trade_card(symbol: str, chain: str, pnl_str: str, x_val: str, held_h: str, bought_str: str, position_str: str, username: str, pnl_pct: str, pnl_positive: bool, closed_at: datetime | None = None, bought_label: str = "Bought", position_label: str = "Position") -> "io.BytesIO | None":
    ...
    try:
        ...
        for y in range(H):
            _t = y / H
            draw.line([(0,y),(W,y)], fill=(int(8+_t*4), int(10+_t*6), int(18+_t*14)))
...
def sniper_score(info: dict) -> dict:
    ...
    # pf_replies already assigned at top of function with `or 0` guard.
...
def sell_core(ud: dict, uid: int, contract: str, usd: float, price: float, reason: str = "manual") -> dict:
    ...
    # Snapshot total_invested BEFORE subtraction so total_invested_full at close
    # is always the correct full cost basis, regardless of partial-sell history.
    _total_invested_snapshot = h["total_invested"]
    ...
    if realized > 0 and ud.get("auto_save_pct"):
        ...
        # On APEX trades only the profit split (50% of realized) lands in balance.
        # Cap save_amt to that split so we never overdraw the main balance.
        if _apex_mood:
            _split_pct = ud.get("apex_vault_profit_split", 0.50)
            save_amt = min(save_amt, realized * _split_pct)
    ...
    if h["amount"] < 0.000001:
        total_invested_full = _total_invested_snapshot     # full cost basis before this sell's deduction
        ...
        for order in list(ud.get("limit_orders", [])):
            ...
            if order["type"] == "buy" and price <= order["target_price"]:
                ...
                if ud["balance"] >= order["amount"]:
                    ...
                    tokens = amt / price
                    c = order["contract"]
                    # ── Simulated trading costs (mirrors do_buy_core) ─────────
                    _lb_fee = 0.0
                    if SIM_FEES_ENABLED:
                        _lb_fee = round(amt * SIM_TOTAL_PCT + SIM_GAS_USD, 4)
                        _lb_fee = min(_lb_fee, max(0.0, ud["balance"] - amt))
                        ud["balance"] = round(ud["balance"] - _lb_fee, 4)
                        ud["total_fees_paid"] = round(ud.get("total_fees_paid", 0.0) + _lb_fee, 4)
...
async def sniper_job(ctx: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        unique_items: list = []   # guard: always bound before the main loop
                    ...
                    if skip_reason:
                        ...
                        if _dl_ch_skip and ud.get("sniper_log_channel_on", True):
                            try:
                                ...
                                await ctx.bot.send_message(
                                    chat_id=int(_dl_ch_skip), text=_dl_text,
...
async def apex_process_entry_queue(app, uid: int, ud: dict) -> None:
    ...
    async with _get_user_lock(uid):
        for contract, entry in list(queue.items()):
            ...
            if not h:
                continue
            # ── Post-buy setup: wrapped in try/except so a setup error never
            # leaves a position without a stop-loss (auramaxxing incident fix).
            try:
                ...
                _tt = "organic"
                _info_for_type = entry.get("info", info_post)
    ...
    for uid, watchlist in list(_apex_watchlist.items()):
        ...
        for contract, rec in list(watchlist.items()):
            try:
                ...
                # Save before deleting so reversal_alerted/status survive if autosave hasn't run yet
                save_user(uid, ud)
                del watchlist[contract]
...
async def enrich_twitter_momentum(info: dict, http_client) -> dict:
    ...
    nitter_instances = [
        ...
        # "https://nitter.poast.org",  # 503 dead — removed
    ]
    profile_html = None
...
async def enrich_holder_distribution(contract: str, chain: str, http_client) -> dict:
    ...
    # ── EVM: DexScreener token page ────────────────────────────────────────────
        ...
        try:
            ...
            # io.dexscreener.com/dex/log is a private DexScreener endpoint (403).
            # Skipping — this was an experimental call that never worked publicly.
            return None  # dexscreener_private_endpoint
            url = f"https://io.dexscreener.com/dex/log/amm/v2/update/tokens/{contract}"  # noqa: dead
    ...
    for follower_id, follower in list(users.items()):
        if follower.get("copy_trading") == uid and not follower.get("copy_paused"):
            if follower.get("balance", 0) >= usd_amount * 0.5:
                ...
                # ── Simulated trading costs for copy trade (mirrors do_buy_core) ──
                _ct_fee = 0.0
                if SIM_FEES_ENABLED:
                    _ct_fee = round(copy_amt * SIM_TOTAL_PCT + SIM_GAS_USD, 4)
                    _ct_fee = min(_ct_fee, max(0.0, follower["balance"] - copy_amt))
                    follower["balance"] = round(follower["balance"] - _ct_fee, 4)
                    follower["total_fees_paid"] = round(follower.get("total_fees_paid", 0.0) + _ct_fee, 4)
    ...
    if result["closed"]:
        ...
        try:
            ...
            if h_remaining:
                ...
                # usd_amount is the gross proceeds of this partial sell.
                # Use it as the sold basis — avoids double-counting already-reduced total_invested.
                pnl_pct_val    = round((realized_so_far / max(usd_amount, 0.01)) * 100, 2)
                card = generate_trade_card(
