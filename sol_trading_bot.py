...
async def _broadcast_to_channel(bot, channel_id: int, info: dict, sc: dict, ai: dict, contract: str, uid: int = 0) -> bool:
    """Post channel card v4. Clean card, no AI toggle button, MTV block, clickable socials."""
    try:
        chain_raw = info.get("chain", "solana").lower().replace(" ", "")
        dex_ch    = {"solana":"solana","ethereum":"ethereum","base":"base","bsc":"bsc","arbitrum":"arbitrum"}.get(chain_raw, chain_raw)
        symbol    = info.get("symbol", "TOKEN")
        ...
        dt_url   = "https://www.dextools.io/app/en/" + dex_ch + "/pair-explorer/" + contract

        # ── Helpers ────────────────────────────────────────────────────────────
        def _dot(pct: float) -> str:
            return "🟢" if pct >= 60 else "🟡" if pct >= 45 else "🔴"

        def _vol_fmt(v: float) -> str:
            if v >= 1_000_000: return f"${v/1_000_000:.1f}M"
            if v >= 1_000:     return f"${v/1_000:.1f}K"
            return f"${v:.0f}"

        age_str_ = (str(round(age_h * 60)) + "m") if age_h < 1 else (str(round(age_h, 1)) + "h")

        verdict_icon = {"SNIPE": "🟢", "WAIT": "🟡", "SKIP": "🔴"}.get(verdict, "🟡")
        rug_icon     = {"LOW": "🟢", "MEDIUM": "🟡", "HIGH": "🔴"}.get(rug_risk, "🟡")
        conf_bar     = "█" * confidence + "░" * (10 - confidence)

        # ── Score bar ──────────────────────────────────────────────────────────
        score_filled = round(score / 10)
        score_bar    = "█" * score_filled + "░" * (10 - score_filled)

        # ── M·T·V block ───────────────────────────────────────────────────────
        bpm5  = info.get("buy_pct_m5",  buy_pct)
        bph1  = info.get("buy_pct_h1",  buy_pct)
        bph24 = buy_pct
        txm5  = info.get("buys_m5", 0) + info.get("sells_m5", 0)
        txh1  = info.get("buys_h1", 0) + info.get("sells_h1", 0)
        txh24 = info.get("buys", 0)    + info.get("sells", 0)
        mtv_block = (
            "📊 <b>M · T · V Intelligence</b>\n"
            f"M5   T: {txm5} {_dot(bpm5)}{bpm5}%   V: {_vol_fmt(vol_m5)} {_dot(bpm5)}{bpm5}%\n"
            f"H1   T: {txh1} {_dot(bph1)}{bph1}%   V: {_vol_fmt(vol_h1)} {_dot(bph1)}{bph1}%\n"
            f"H24  T: {txh24} {_dot(bph24)}{bph24}%   V: {_vol_fmt(vol_h24)} {_dot(bph24)}{bph24}%\n"
        )
        ...
        if telegram_: soc_parts.append(f'<a href="{telegram_}">✈️ Telegram</a>')
        if website:  soc_parts.append(f'<a href="{website}">🌐 Website</a>')
        social_line = "Socials: " + "  ·  ".join(soc_parts) if soc_parts else "Socials: None"

        # ── Search CA on X ────────────────────────────────────────────────────
        import urllib.parse as _up
        x_url  = "https://x.com/search?f=live&q=" + _up.quote(contract) + "&src=typed_query"
        x_line = f'🔍 <a href="{x_url}">Search CA on 𝕏</a>'

        # ── Flags ──────────────────────────────────────────────────────────────
        gf_lines = "\n".join(["  ✅ " + f for f in green_flags]) if green_flags else "  <i>None detected</i>"
        rf_lines = "\n".join(["  ⚠️ " + f for f in red_flags])   if red_flags   else "  <i>None detected</i>"

        # ── Entry + chart links ───────────────────────────────────────────────
        entry_line = (f'\n💰 <b>Suggested Entry:</b> ${suggested:,.2f}' if suggested > 0 else "")
        links_line = f'\n🔗 <a href="{dex_url}">DS</a>  ·  <a href="{gt_url}">GT</a>  ·  <a href="{dt_url}">DT</a>'

        # ── Full card ──────────────────────────────────────────────────────────
        text = (
            f"<b>${symbol}</b>  ·  {name}  ·  {chain_raw.upper()}\n"
            f"<code>{contract}</code>\n\n"
            f"{verdict_icon} <b>VERDICT: {verdict}</b>  ·  Conf: <code>{conf_bar}</code> {confidence}/10\n"
            f"{rug_icon} Rug Risk: <b>{rug_risk}</b>  ·  Momentum: <b>{momentum}</b>  ·  Social: <b>{social_sc}</b>\n"
            + (f"<i>{thesis}</i>\n" if thesis else "")
            + "\n━━━━━━━━━━━━━━━━━━\n"
            f"📊 <b>SCORE:</b> <code>{score_bar}</code> <b>{score}/100</b> {s_icon}\n\n"
            + mtv_block
            + "\n"
            f"MC: <b>{mc_str(mc)}</b>  ·  Liq: <b>${liq:,.0f}</b> ({round(liq_pct,1)}%)\n"
            f"Age: <b>{age_str_}</b>  ·  Buy%: <b>{buy_pct}%</b>  ·  Vol/h: <b>{_vol_fmt(vol_h1)}</b>\n"
        ...
        # ── Keyboard: just Buy + View, no AI toggle ────────────────────────────
        kb_rows = []
        if bot_url:
            kb_rows.append([InlineKeyboardButton("⚡ Buy on APEX Sniper", url=bot_url)])
        kb_rows.append([InlineKeyboardButton("🔍 View Token Live ↗", url=dex_url)])
        ...
        _ch_card_cache[contract[:32]] = {
            ...
            "bot_url": bot_url, "dex_url": dex_url, "gt_url": gt_url,
        }
        # Pass message_id so milestone replies thread back to this card
        if uid: _register_channel_call(uid, contract, info, channel_id, msg.message_id)
        return True
    except Exception as e:
            ...
            try:
                ...
                for milestone_x in _MILESTONE_XS:
                    ...
                    if current_x >= milestone_x:
                        ...
                        ds_url_m  = f"https://dexscreener.com/{dex_chain}/{contract}"
                        gt_url_m  = f"https://www.geckoterminal.com/{dex_chain}/pools/{contract}"
                        dt_url_m  = f"https://www.dextools.io/app/en/{dex_chain}/pair-explorer/{contract}"
                        ...
                        try:
                            await ctx.bot.send_message(
                                ...
                                reply_markup=InlineKeyboardMarkup([
                                    ...
                                    [
                                        InlineKeyboardButton("DS ↗", url=ds_url_m),
                                        InlineKeyboardButton("GT ↗", url=gt_url_m),
                                        InlineKeyboardButton("DT ↗", url=dt_url_m),
                                    ],
                                ]),
                                disable_web_page_preview=True
                            )
...
def _register_channel_call(uid: int, contract: str, info: dict, channel_id: int, message_id: int = 0):
    ...
    message_id is the Telegram message ID of the original call card — milestones
    reply directly to it so they appear threaded in the channel (like GemTools).
    """
    ...
    if contract not in user_calls:   # don't reset if already tracking
        user_calls[contract] = {
            ...
            "message_id":     message_id,   # ← original card msg ID for reply threading
            "milestones_hit": set(),
        }
...
def apex_is_daily_loss_halted(ud: dict) -> bool:
    apex_reset_daily(ud)
    # Halt based on vault losses — vault IS the APEX capital
    vault = ud.get("apex_vault", 0.0)
    start = ud.get("apex_session_start_bal", vault)
    if start is None or start <= 0:
        return vault <= 0.0  # vault empty = halt
