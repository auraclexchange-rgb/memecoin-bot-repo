...
def _sajad_staircase_break(h: dict, current_price: float) -> bool:
    ...
    Uses sr_history first, falls back to price_history on fast positions.
    Requires at least 5 candles (not 8) so it fires on shorter-held positions.
    Only fires when position is below break-even (cx < 1.05).
...
async def btn(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # ── APEX VAULT MENU ───────────────────────────────────────────────────────
        ...
        await q.edit_message_text(
            ...
            "*Entry filters:*\n"
            "• MC > $150K, Liq > $80K, Top10 < 20%, Age < 40h\n"
