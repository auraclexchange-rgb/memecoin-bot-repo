...
async def btn(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if cb == "mm":
        ...
        if not trade:
            ...
            return
        share_text = (
            "AURACLE_XBOT - TRADE RESULT\n\n"
            "$" + trade["symbol"] + "\n"
            "Chain: " + trade.get("chain", "SOL").upper() + "\n\n"
            "Entry: " + money(trade["invested"] / (trade["returned"] / trade["invested"]) if trade["returned"] > 0 else 0) + "\n"
            "Exit PnL: " + pstr(trade["realized_pnl"]) + "\n"
            "Multiple: " + str(round(trade.get("x", 0), 2)) + "x\n"
            "Held: " + str(trade["hold_h"]) + "h\n"
            "Reason: " + trade.get("mood", "N/A") + "\n\n"
