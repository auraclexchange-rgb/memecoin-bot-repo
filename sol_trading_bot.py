...
async def btn(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # ── COMPETITION BETS UPDATE ────────────────────────────────────────────
        ...
        for code in my_codes:
            ...
            # ── Build ranked leaderboard (by realized PnL from comp period) ──
            rankings = []
            for uid_str, m in members.items():
                try:
                    ...
                    # joined_at: when they entered the competition
                    try:
                        joined_dt = datetime.fromisoformat(m.get("joined_at", "2000-01-01T00:00:00"))
                    except Exception:
                        joined_dt = datetime(2000, 1, 1)

                    if m_ud:
                        logs = trade_log.get(m_uid, [])
                        # Only manual (non-APEX, non-copy) trades made AFTER joining
                        comp_trades = [
                            t for t in logs
                            if t.get("mood") not in ("APEX", "AI-Sniper", "APEX-DCA", "Copy Trade")
                        ...
                        total_pnl    = round(sum(t.get("realized_pnl", 0) for t in comp_trades), 2)
            ...
            # Sort by Win Rate descending, PnL as tiebreaker
            rankings.sort(key=lambda x: (-x["wr"], -x["pnl"]))

            # ── Auto-pay winner when competition ends ─────────────────────────
            if is_ended and not comp.get("winner_paid") and pot > 0 and rankings:
                winner       = rankings[0]
                winner_ud    = users.get(winner["uid"])
                if winner_ud and winner["trades"] > 0:
                    winner_ud["balance"] = round(winner_ud.get("balance", 0) + pot, 4)
                    save_user(winner["uid"], winner_ud)
                    comp["winner_paid"] = True
                    comp["winner_uid"]  = str(winner["uid"])
                    comp["ended"]       = True
                    # Persist updated comp to all members
                    for uid_str2 in members:
                        try:
                            ...
                            # Notify all members
                            await ctx.bot.send_message(
                                chat_id=m_uid2, parse_mode="Markdown",
                                text=(
                                    "🏆 *COMPETITION ENDED!*\n\n"
                                    f"📋 Code: `{code}`\n\n"
                                    f"🥇 *Winner: @{winner['uname']}*\n"
                                    f"💰 Prize: *{money(pot)}* credited to their balance!\n"
                                    f"🎯 Win Rate: *{winner['wr']}%*  ({winner['wins']}/{winner['trades']} trades)\n"
                    ...
                    # No one made profit — mark ended without paying
                    comp["winner_paid"] = True
                    comp["ended"]       = True
            ...
            if is_ended and comp.get("winner_uid"):
                w_name = members.get(comp["winner_uid"], {}).get("username", "?")
                w_rank = next((r for r in rankings if str(r["uid"]) == comp["winner_uid"]), None)
                w_wr   = f"{w_rank['wr']}% WR" if w_rank else ""
                winner_line = f"\n🏆 *Winner: @{w_name}*  {w_wr}\n"

            txt += (
                f"📋 Code: `{code}`\n"
                f"{status_str}  ·  {pot_str}  ·  👥 {len(members)} players\n"
                f"📅 Ends: {end_str}\n"
                + winner_line +
                "\n🏆 *Leaderboard (by Win Rate):*\n"
            )
            if not rankings:
                txt += "  _No trades yet — be first!_\n"
            else:
                for i, r in enumerate(rankings[:5]):
                    medal   = medals[i] if i < len(medals) else f"{i+1}."
                    is_me   = r["uid"] == u.id
                    me_tag  = "  ← *you*" if is_me else ""
                    trades_str = f"{r['trades']}T  {pstr(r['pnl'])}" if r["trades"] > 0 else "no trades yet"
